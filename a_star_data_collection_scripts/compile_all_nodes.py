...
def read_compiled_total(staged_root: Path) -> int | None:
    """Read total env count from an existing compiled node dir.

    Parameters
    ----------
    staged_root
        Existing ``data_node*_compiled`` directory.

    Returns
    -------
    int | None
        Parsed ``successful_envs.json`` total, or ``None`` if unavailable.
    """
    tracker = staged_root / "successful_envs.json"
    if not tracker.exists():
        return None
    try:
        data = json.loads(tracker.read_text())
        ...
        return None
    try:
        return int(data.get("total", len(data.get("envs", []))))
    except Exception:  # noqa: BLE001
        return None


def replace_dir_atomic(src_tmp: Path, dst: Path) -> None:
    """Replace ``dst`` with ``src_tmp`` using a backup then rename sequence."""
    backup = dst.with_name(f"{dst.name}.bak_replace_{os.getpid()}")
    if backup.exists():
...
def compile_node(
    ...
    tmp_root = data_dir / f".data_node{node_key}_compiled.tmp.{os.getpid()}"
    old_total = read_compiled_total(staged_root)
    ...
    if staged_root.exists() and not args.overwrite:
    ...
    if staged_root.exists() and args.overwrite:
        print(f"[node {node_key}] overwrite enabled; building temp dir first "
              f"(existing total={old_total})")
    ...
    # Build into a temp dir. Existing compiled output is replaced only after
    # validation/copy completes, so killing this script cannot corrupt a good
    # previous compiled dir.
    write_root = tmp_root if args.overwrite else staged_root
    ...
    (write_root / "successful_envs.json").write_text(
    ...
    (write_root / "task_summary.json").write_text(json.dumps(summary,
                                                             indent=2))

    if args.overwrite:
        new_total = int(counts["ok"])
        if old_total is not None and new_total < old_total:
            print(f"[SAFE-SKIP] node {node_key}: temp compile kept "
                  f"{new_total}, existing compiled has {old_total}. "
                  "Keeping existing compiled dir and not cleaning raw.")
            shutil.rmtree(tmp_root, ignore_errors=True)
            summary["skipped"] = True
            summary["reason"] = "new_total_less_than_existing"
            return summary
        replace_dir_atomic(tmp_root, staged_root)
