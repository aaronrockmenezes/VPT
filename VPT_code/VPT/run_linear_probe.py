...
def train_linear_probe(model, train_loader, test_loader, val_loader, human_loader, criterion, optimizer, lr_scheduler, device, args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        with torch.no_grad():
            ...
            test_acc, test_loss, test_records = evaluate_linear_probe(model, test_loader, criterion, device, True, args)
...
def run_extract_features(args):
    ...
    human_dataset = datasets.ImageFolder(os.path.join(args.data_dir, 'test'), transform=transform)
    ...
    human_features, human_labels = extract_features(model, human_loader, device, return_path=not True)

    human_img_paths = None

    return train_features, train_labels, test_features, test_labels, val_features, val_labels, human_features, human_labels, human_img_paths, test_img_paths
...
def run_linear_probe(args):
    ...
    human_features, human_labels, human_img_paths, test_img_paths = run_extract_features(args)
