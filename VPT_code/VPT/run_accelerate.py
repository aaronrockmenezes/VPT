...
def extract_features(model, data_loader, accelerator, return_path=False):
    ...
    if return_path:
        all_paths = gather_object(img_path_list)

        if len(all_paths) > len(features):
            all_paths = all_paths[:len(features)]
            ...
            if val_acc > best_acc_val:
                ...
                with open(f'./logs/linear_probe_preds/{file_name}', 'w') as f:
                    ...
                    if test_img_paths is not None:
                        ...
                        labels_np = test_labels.numpy().squeeze()
                        img_paths_np = np.array(test_img_paths).squeeze()
                        records = np.vstack([img_paths_np, preds_np, labels_np]).T
...
def run_extract_features(args, accelerator):
    ...
    # Only need one test dataset — val/human/test all use the same directory
