...
import json
import csv
import re  # Added for regex path matching
...
def train_linear_probe(model, train_loader, test_loader, criterion, optimizer, device, args):
    ...
    # Try to find the labels file in the dataset directory
    json_path = os.path.join(args.data_dir, "visibility_labels.json")
    ground_truth_labels = {}
        ...
        try:
            with open(json_path, 'r') as f:
                ...
                for k, v in data.get("environments", {}).items():
                    label_val = 1 if v["label"] == "Yes" else 0
                    ground_truth_labels[str(k)] = label_val 
        except Exception as e:
            print(f"Warning: Could not load JSON labels from {json_path}: {e}")
    else:
        print(f"Warning: visibility_labels.json not found at {json_path}")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        # --- Evaluation Step ---
        with torch.no_grad():
            # test_records contains [path, pred, label] from the dataset (not yet JSON overridden)
            ...
            if test_acc > best_acc_test:
                ...
                with open(f'./logs/linear_probe_preds/{file_name}', 'w') as f:
                    ...
                    # Build rows manually to inject JSON Ground Truth
                    rows_to_write = []
                    # test_records is a numpy array of strings/mixed types
                    for row in test_records:
                        p_path = row[0]
                        p_pred = row[1]
                        
                        # Start with the dataset label as fallback
                        final_label = row[2] 
                        
                        # EXTRACT ENV ID FROM PATH
                        # Expects format like .../env_123/...
                        match = re.search(r'env_(\d+)', str(p_path))
                        
                        if match and ground_truth_labels:
                            env_id = match.group(1)
                            if env_id in ground_truth_labels:
                                final_label = ground_truth_labels[env_id]
                        
                        rows_to_write.append([p_path, p_pred, final_label])
