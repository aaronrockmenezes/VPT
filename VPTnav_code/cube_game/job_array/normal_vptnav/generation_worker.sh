...
# Build per-run --env args; skip vars that are unset/empty so the
# container does not see empty-string values that break int() parsing.
EXTRA_ENV_ARGS=()
push_env() {
  local var="$1"
  if [ -n "${!var:-}" ]; then
    EXTRA_ENV_ARGS+=(--env "${var}=${!var}")
  fi
}
for v in AGENT_SCRIPT NUM_ENVS SETTLE_STEPS \
  STRATEGY_TARGET STRATEGY_MAX_ATTEMPTS STRATEGY_VERBOSE \
  ...
  STRATEGY_RAIL_EXTENT STRATEGY_CAMERA_GOAL_DISTANCE \
  STRATEGY_MAX_CAMERA_GOAL_DISTANCE STRATEGY_MIN_CAMERA_GOAL_CLEARANCE \
  STRATEGY_MIN_SELECTED_PAIR_DISTANCE STRATEGY_MIN_AGENT_CLEARANCE \
  ...
  STRATEGY_MIN_VALID_CANDIDATES; do
  push_env "$v"
done
