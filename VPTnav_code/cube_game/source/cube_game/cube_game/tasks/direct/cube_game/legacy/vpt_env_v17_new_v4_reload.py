...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: VPTEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.goal_pixel_threshold = 100
        self.goal_pixel_threshold_occlusion = 400
        self.camera_pixel_threshold = 500
        self.verbose = 2
        ...
        self.base_path = f"/oscar/scratch/arock3/VPTnav_data/v17_new/data_v17_reload3/data_{self.GPU_ID}"
        ...
        print(f"🚀 Init VPTEnv | GPU: {self.GPU_ID} | Mode: {self.mode} | Envs: {self.num_envs}")
        ...
        self.applied_scales = {}          # {(env_idx, obj_idx): [sx, sy, sz]}
        self.applied_materials = {}       # {(env_idx, obj_idx): material_path_str}
        self.applied_mat_materials = {}   # {env_idx: material_path_str}
        self.applied_wall_colors = {}     # {env_idx: {wall_name: [r,g,b]}}
        self.applied_light_params = {}    # {env_idx: [dict, ...]}
        self.collected_agent_poses = [None] * self.num_envs  # list of [[x,y,z,w,x,y,z], ...]
    ...
    def _setup_scene(self):
        ...
        self._reference_obj = RigidObject(self.cfg.reference_obj)
        # In _setup_scene, alongside the other RigidObject registrations:
        ...
        self.scene.rigid_objects.update({
            "agent": self._agent,
            "goal": self._goal,
            "reference_obj": self._reference_obj,
            "mat": self._mat,
    ...
    def _save_env_config_to_json_new(self, env_id: int, folder_idx: int):
        """Save complete environment configuration for full reconstruction."""
        
        env_id_item = env_id.item() if torch.is_tensor(env_id) else env_id
        
        # === FETCH CACHE ===
        env_cache = getattr(self, "env_metadata_cache", {}).get(env_id_item, {})
        ...
        ref_pos_local = to_local_pos(self._reference_obj.data.root_pos_w[env_id_item])
        ref_quat = self._reference_obj.data.root_quat_w[env_id_item].cpu().numpy().tolist()
        ...
        # --- Agent poses at viewpoints ---
        agent_viewpoint_poses_local = []
        if self.collected_agent_poses[env_id_item] is not None:
            for pose in self.collected_agent_poses[env_id_item]:
                # pose is [x, y, z, qw, qx, qy, qz] in world coords
                local_pose = list(pose)  # copy
                local_pose[0] -= float(env_origin[0])
                local_pose[1] -= float(env_origin[1])
                local_pose[2] -= float(env_origin[2])
                agent_viewpoint_poses_local.append(local_pose)
        ...
        for local_idx, obj_idx in enumerate(active_indices):
            ...
            world_pos = self._vpt_objects.data.object_pos_w[env_id_item, obj_idx_item]
            pos_local = to_local_pos(world_pos)
            quat = self._vpt_objects.data.object_quat_w[env_id_item, obj_idx_item].cpu().numpy().tolist()
            bbox_dims = self.all_vpt_dims[env_id_item, obj_idx_item].cpu().numpy().tolist()

            # Scale from tracking
            applied_scale = self.applied_scales.get((env_id_item, obj_idx_item), [1.0, 1.0, 1.0])

            # Material from tracking (prefer direct tracking over cache)
            material_path = self.applied_materials.get(
                (env_id_item, obj_idx_item), 
                vpt_textures_cache.get(obj_idx_item, "unknown")
            )

            # Z-offset ratio and shape
            z_offset_ratio = self.vpt_z_offset_ratios[env_id_item, obj_idx_item].item()
            shape_id = self.vpt_shapes[env_id_item, obj_idx_item].item()

            # Base dims
            base_dims = self.vpt_base_dims[obj_idx_item].cpu().numpy().tolist()

            # Source asset info from config
            spawn_cfg = self.cfg.vpt_objects.rigid_objects[vpt_keys[obj_idx_item]].spawn
            spawn_type = type(spawn_cfg).__name__
            asset_info = {}
            if isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                asset_info["usd_path"] = spawn_cfg.usd_path
                asset_info["base_scale"] = list(spawn_cfg.scale) if spawn_cfg.scale else [1, 1, 1]
            elif isinstance(spawn_cfg, sim_utils.MeshCuboidCfg):
                asset_info["size"] = list(spawn_cfg.size)
            elif isinstance(spawn_cfg, (sim_utils.MeshCylinderCfg, sim_utils.MeshConeCfg)):
                asset_info["radius"] = float(spawn_cfg.radius)
                asset_info["height"] = float(spawn_cfg.height)

            vpt_objects_data.append({
                "obj_index": obj_idx_item,
                "config_key": vpt_keys[obj_idx_item],
        ...
        # --- Dedup hash ---
        dedup_hash = ""
        if hasattr(self, '_generate_env_dedup_hash'):
            dedup_hash = self._generate_env_dedup_hash(
                active_obj_ids=active_indices_list,
                obj_positions=[o["pos_local"] for o in vpt_objects_data],
                obj_quats=[o["quat_wxyz"] for o in vpt_objects_data],
                obj_scales=[o["dimensions_bbox"] for o in vpt_objects_data],
                obj_textures=[o["material_path"] for o in vpt_objects_data],
        ...
        # --- Lighting ---
        lighting_data = self.applied_light_params.get(env_id_item, 
                                                       env_cache.get("lighting", []))

        # --- Floor material ---
        floor_material = self.applied_mat_materials.get(
            env_id_item, 
            textures_cache.get("floor", "unknown")
        )

        # --- Wall colors ---
        wall_colors = self.applied_wall_colors.get(env_id_item, {})

        # --- Depth labels for this env ---
        env_key = f"env_{folder_idx}"
        depth_labels = self.depth_labels_data.get(env_key, {})
        ...
        config = {
            ...
            "environment": {
                "origin": env_origin.tolist(),
                "boundary_limits": list(self.cfg.boundary_limits),
                "center_to_boundary": float(self.center_to_boundary.item()) if torch.is_tensor(self.center_to_boundary) else float(self.center_to_boundary),
            ...
            "entities": {
                "goal": {
                    "pos_local": goal_pos_local,
                    "quat_wxyz": goal_quat,
                    "radius": float(self.cfg.goal_ball.spawn.radius),
                    "color": list(self.cfg.goal_ball.spawn.visual_material.diffuse_color),
                },
                "reference_obj": {
                    "pos_local": ref_pos_local,
                    "quat_wxyz": ref_quat,
                },
                "camera": {
                    "pos_local": camera_pos_local,
                    "quat_wxyz": camera_quat,
                    "color": list(self.cfg.camera_obj.spawn.visual_material.diffuse_color),
                },
                "agent": {
                    "pos_local": agent_pos_local,
                    "quat_wxyz": agent_quat,
                    "default_height": float(self._agent.data.default_root_state[env_id_item, 2].item()),
                },
            },
            "vpt_objects": {
                "total_pool_size": self.num_objs,
            ...
            "viewpoints": {
                ...
                "agent_poses_at_viewpoints": {
                    "count": len(agent_viewpoint_poses_local),
                    "poses_local": agent_viewpoint_poses_local,
                },
            },
        ...
        if self.verbose >= 2:
            print(f"  💾 Saved config v3: {config_filepath}")

    def load_env_from_json(self,
                           config_path: str,
                           target_env_id: int = 0,
                           apply_visuals: bool = True) -> dict:
        """
        Reconstructs an environment from a saved JSON config.

        Restores object poses, scales, materials, lighting, camera/goal poses,
        active VPT selection, viewpoint data, and depth labels.
        ...
        target_env_id : int
            Which environment slot to load into (0 to num_envs-1).
        apply_visuals : bool
            If True, applies materials, scales, colors, and lighting via USD stage ops.

        Returns
        -------
        dict
            The loaded config dict for downstream use.
        """
        with open(config_path, 'r') as f:
            config = json.load(f)

        device = self.device
        stage = get_current_stage() if apply_visuals else None
        eid = target_env_id
        eid_tensor = torch.tensor([eid], dtype=torch.long, device=device)

        version = config.get("version", "1.0")

        # Determine coordinate mode
        uses_local = version >= "2.0"
        env_origin = np.array(config.get("environment", {}).get("origin", [0, 0, 0]))
        if not uses_local:
            env_origin = self.scene.env_origins[eid].cpu().numpy()

        def to_world(local_pos_list):
            """Convert local position list to world coordinates."""
            return [local_pos_list[i] + float(env_origin[i]) for i in range(3)]

        # ==========================================================
        # 1. ACTIVE VPT INDICES
        # ==========================================================
        vpt_cfg = config["vpt_objects"]
        active_indices = torch.tensor(vpt_cfg["active_indices"], dtype=torch.long, device=device)
        self.active_vpt_indices[eid] = active_indices

        # ==========================================================
        # 2. OBJECT POSES
        # ==========================================================
        vpt_state = self._vpt_objects.data.default_object_state[eid].clone()

        # Move ALL to storage
        for obj_idx in range(self.num_objs):
            vpt_state[obj_idx, 0] = self.storage_position[0]
            vpt_state[obj_idx, 1] = self.storage_position[1]
            vpt_state[obj_idx, 2] = self.storage_position[2]
            vpt_state[obj_idx, 7:13] = 0.0

        # Place active objects
        for obj_detail in vpt_cfg["objects"]:
            idx = obj_detail["obj_index"]

            # Handle both v2 (pos_local) and v3 (pos_local) and v1 (position) formats
            if "pos_local" in obj_detail:
                pos = to_world(obj_detail["pos_local"])
            else:
                pos = obj_detail["position"]

            orient = obj_detail.get("quat_wxyz", obj_detail.get("orientation", [1, 0, 0, 0]))

            vpt_state[idx, 0] = pos[0]
            vpt_state[idx, 1] = pos[1]
            vpt_state[idx, 2] = pos[2]
            vpt_state[idx, 3] = orient[0]
            vpt_state[idx, 4] = orient[1]
            vpt_state[idx, 5] = orient[2]
            vpt_state[idx, 6] = orient[3]
            vpt_state[idx, 7:13] = 0.0

            # Restore cached dims
            bbox = obj_detail.get("dimensions_bbox", obj_detail.get("final_dims", [1, 1, 1]))
            self.all_vpt_dims[eid, idx, 0] = bbox[0]
            self.all_vpt_dims[eid, idx, 1] = bbox[1]
            self.all_vpt_dims[eid, idx, 2] = bbox[2]

            if "z_offset_ratio" in obj_detail:
                self.vpt_z_offset_ratios[eid, idx] = obj_detail["z_offset_ratio"]
            if "shape_id" in obj_detail:
                self.vpt_shapes[eid, idx] = obj_detail["shape_id"]

        self._vpt_objects.write_object_pose_to_sim(
            vpt_state[:, :7].unsqueeze(0), eid_tensor)
        self._vpt_objects.write_object_velocity_to_sim(
            torch.zeros((1, self.num_objs, 6), device=device), eid_tensor)

        # ==========================================================
        # 3. GOAL POSE
        # ==========================================================
        entities = config.get("entities", config)
        goal_cfg = entities.get("goal", config.get("goal_ball", config.get("goal", {})))
        
        g_pos = to_world(goal_cfg["pos_local"]) if "pos_local" in goal_cfg else goal_cfg["position"]
        g_quat = goal_cfg.get("quat_wxyz", goal_cfg.get("orientation", [1, 0, 0, 0]))
        
        goal_pose = torch.tensor(g_pos + g_quat, dtype=torch.float32, device=device).unsqueeze(0)
        self._goal.write_root_com_pose_to_sim(goal_pose, eid_tensor)
        self._goal.write_root_com_velocity_to_sim(torch.zeros((1, 6), device=device), eid_tensor)

        # ==========================================================
        # 4. CAMERA POSE
        # ==========================================================
        cam_cfg = entities.get("camera", config.get("camera_object", config.get("camera", {})))
        
        c_pos = to_world(cam_cfg["pos_local"]) if "pos_local" in cam_cfg else cam_cfg["position"]
        c_quat = cam_cfg.get("quat_wxyz", cam_cfg.get("orientation", [1, 0, 0, 0]))
        
        camera_pose = torch.tensor(c_pos + c_quat, dtype=torch.float32, device=device).unsqueeze(0)
        self._camera_obj.write_root_com_pose_to_sim(camera_pose, eid_tensor)
        self._camera_obj.write_root_com_velocity_to_sim(torch.zeros((1, 6), device=device), eid_tensor)

        # ==========================================================
        # 5. AGENT POSE
        # ==========================================================
        agent_cfg = entities.get("agent", {})
        
        a_pos = to_world(agent_cfg["pos_local"]) if "pos_local" in agent_cfg else agent_cfg.get("position", [0, 0, 0])
        a_quat = agent_cfg.get("quat_wxyz", agent_cfg.get("orientation", [1, 0, 0, 0]))
        
        agent_pose = torch.tensor(a_pos + a_quat, dtype=torch.float32, device=device).unsqueeze(0)
        self._agent.write_root_com_pose_to_sim(agent_pose, eid_tensor)
        self._agent.write_root_com_velocity_to_sim(torch.zeros((1, 6), device=device), eid_tensor)

        # ==========================================================
        # 5a. REFERENCE OBJECT
        # ==========================================================
        ref_cfg = entities.get("reference_obj", {})
        if ref_cfg:
            r_pos = to_world(ref_cfg["pos_local"]) if "pos_local" in ref_cfg else [0, 0, 0]
            r_quat = ref_cfg.get("quat_wxyz", [1, 0, 0, 0])

            ref_pose = torch.tensor(r_pos + r_quat, dtype=torch.float32, device=device).unsqueeze(0)
            self._reference_obj.write_root_com_pose_to_sim(ref_pose, eid_tensor)
            self._reference_obj.write_root_com_velocity_to_sim(
                torch.zeros((1, 6), device=device), eid_tensor)

        # ==========================================================
        # 5b. MAT & WALLS (write default poses so they appear)
        # ==========================================================
        static_bodies = [
            self._mat,
            self._boundary_top,
            self._boundary_bottom,
            self._boundary_left,
            self._boundary_right,
        ]
        for body in static_bodies:
            state = body.data.default_root_state[eid_tensor].clone()
            body.write_root_com_pose_to_sim(state[:, :7], eid_tensor)
            body.write_root_com_velocity_to_sim(
                torch.zeros((1, 6), device=device), eid_tensor)

        # ==========================================================
        # 6. VISUALS
        # ==========================================================
        if apply_visuals and stage is not None:
            bbox_cache = UsdGeom.BBoxCache(Usd.TimeCode.Default(), [UsdGeom.Tokens.default_])

            # 6a. Object Scales
            for obj_detail in vpt_cfg["objects"]:
                idx = obj_detail["obj_index"]
                scale = obj_detail.get("applied_scale", [1.0, 1.0, 1.0])
                prim_path = f"/World/envs/env_{eid}/obs_{idx}"
                ...
                if prim.IsValid():
                    ...
                    bbox = obj_detail.get("dimensions_bbox", [1, 1, 1])
                    z_ratio = obj_detail.get("z_offset_ratio", 0.0)
                    z_pos = bbox[2] * z_ratio
                    current_trans = translate_op.Get() or Gf.Vec3d(0, 0, 0)
                    translate_op.Set(Gf.Vec3d(current_trans[0], current_trans[1], z_pos))

                    self.applied_scales[(eid, idx)] = scale

            # 6b. Object Materials
            for obj_detail in vpt_cfg["objects"]:
                idx = obj_detail["obj_index"]
                mat_path = obj_detail.get("material_path")
                if mat_path and mat_path != "unknown":
                    prim_path = f"/World/envs/env_{eid}/obs_{idx}"
                    sim_utils.bind_visual_material(prim_path, mat_path)

            # 6c. Floor Material
            scene_cfg = config.get("scene", config.get("environment_aesthetics", {}))
            floor_mat = scene_cfg.get("floor_material", scene_cfg.get("floor_material_path"))
            if floor_mat and floor_mat != "unknown":
                sim_utils.bind_visual_material(f"/World/envs/env_{eid}/mat", floor_mat)

            # 6d. Wall Colors
            wall_colors = scene_cfg.get("wall_colors", {})
            for wall_name, color in wall_colors.items():
                wall_path = f"/World/envs/env_{eid}/{wall_name}"
                prim = stage.GetPrimAtPath(wall_path)
                if not prim.IsValid():
                    continue
                for child in Usd.PrimRange(prim):
                    if child.GetTypeName() == "Shader":
                        color_attr = child.GetAttribute("inputs:diffuseColor")
                        if color_attr:
                            color_attr.Set(Gf.Vec3f(color[0], color[1], color[2]))
                        break

            # 6e. Lighting
            lights = scene_cfg.get("lights", config.get("lighting", []))
            for light_info in lights:
                suffix = light_info.get("prim_path_suffix", "Light_A")
                light_path = f"/World/envs/env_{eid}/{suffix}"
                ...
                prim.GetAttribute("inputs:intensity").Set(light_info["intensity"])
                prim.GetAttribute("inputs:radius").Set(light_info["radius"])
                prim.GetAttribute("inputs:enableColorTemperature").Set(True)
                prim.GetAttribute("inputs:colorTemperature").Set(light_info["color_temperature"])
        ...
        # 7. VIEWPOINTS
        # ==========================================================
        vp_cfg = config.get("viewpoints", {})

        valid_positions = vp_cfg.get("valid_positions_local", 
                                      config.get("valid_viewpoints", {}).get("positions", []))
        if valid_positions:
            vp_world = []
            for vp in valid_positions:
                vp_world.append([vp[i] + float(env_origin[i]) for i in range(len(vp))])
            self.valid_viewpoint_poses[eid] = torch.tensor(vp_world, dtype=torch.float32, device=device)

        collected_positions = vp_cfg.get("collected_positions_local",
                                          config.get("collected_viewpoints", {}).get("positions", []))
        if collected_positions:
            cp_world = []
            for cp in collected_positions:
                cp_world.append([cp[i] + float(env_origin[i]) for i in range(len(cp))])
            self.selected_viewpoints_for_collection[eid] = torch.tensor(cp_world, dtype=torch.float32, device=device)

        # Agent poses at viewpoints
        agent_vp_data = vp_cfg.get("agent_poses_at_viewpoints", {})
        agent_vp_poses = agent_vp_data.get("poses_local", agent_vp_data.get("poses", []))

        # ==========================================================
        # 8. VISIBILITY LABELS
        # ==========================================================
        meta = config["metadata"]
        folder_idx = meta["folder_idx"]
        self.env_visibility_labels[folder_idx] = meta["visibility_label"]
        self.env_visibility_reasons[folder_idx] = meta["visibility_reason"]

        # ==========================================================
        # 9. DEPTH LABELS
        # ==========================================================
        depth_labels = config.get("depth_labels", {})
        if depth_labels:
            env_key = f"env_{folder_idx}"
            self.depth_labels_data[env_key] = depth_labels
        ...
        # Update OBB cache
        self.update_obb_cache(eid_tensor, vpt_state.unsqueeze(0))

        # Update camera sensor
        self._update_camera_poses(eid_tensor)
        self._occlusion_camera.update(self.sim.cfg.dt)
        self._rgb_tiled_camera.update(self.sim.cfg.dt)

        print(f"✅ Loaded env from {config_path} into slot {eid}")
        print(f"   Active objects: {len(vpt_cfg['objects'])}/{self.num_objs}")
        print(f"   Viewpoints: {len(valid_positions)} valid, {len(collected_positions)} collected")
        print(f"   Agent replay poses: {len(agent_vp_poses)}")

        self._reset_called = True
        return config

    def replay_viewpoints_from_config(self,
                                      config: dict,
                                      env_id: int = 0,
                                      save_images: bool = False,
                                      output_dir: str = None) -> list[torch.Tensor]:
        """
        Teleports the agent through saved viewpoint poses and captures observations.

        Parameters
        ----------
        config : dict
            Config dict returned by load_env_from_json.
        env_id : int
            Environment slot to replay in.
        save_images : bool
            If True, saves RGB frames to output_dir.
        output_dir : str
        ...
        -------
        list[torch.Tensor]
            List of RGB observation tensors per viewpoint.
        """
        device = self.device
        env_tensor = torch.tensor([env_id], dtype=torch.long, device=device)
        zero_vel = torch.zeros((1, 6), device=device)

        env_origin = np.array(config.get("environment", {}).get("origin", [0, 0, 0]))
        uses_local = config.get("version", "1.0") >= "2.0"

        vp_cfg = config.get("viewpoints", {})
        agent_vp_data = vp_cfg.get("agent_poses_at_viewpoints", {})
        poses = agent_vp_data.get("poses_local", agent_vp_data.get("poses", []))

        if not poses:
            # Reconstruct from collected viewpoint positions
            positions = vp_cfg.get("collected_positions_local",
                                    config.get("collected_viewpoints", {}).get("positions", []))
            if not positions:
                print("No viewpoint poses available for replay.")
                return []

            entities = config.get("entities", config)
            cam_cfg = entities.get("camera", config.get("camera_object", {}))
            goal_cfg = entities.get("goal", config.get("goal_ball", {}))

            c_pos = cam_cfg.get("pos_local", cam_cfg.get("position", [0, 0, 0]))[:2]
            g_pos = goal_cfg.get("pos_local", goal_cfg.get("position", [0, 0, 0]))[:2]

            cam_2d = torch.tensor(c_pos, device=device)
            goal_2d = torch.tensor(g_pos, device=device)
            midpoint = (cam_2d + goal_2d) / 2.0

            agent_h = config.get("entities", {}).get("agent", {}).get(
                "default_height", 
                config.get("agent", {}).get("default_height", 0.0)
            )

            poses = []
            for vp in positions:
                pt = torch.tensor(vp[:2], device=device)
                direction = midpoint - pt
                yaw = torch.atan2(direction[1], direction[0])
                half_yaw = yaw / 2
                local_pose = [
                    vp[0], vp[1], agent_h,
                    torch.cos(half_yaw).item(), 0.0, 0.0, torch.sin(half_yaw).item()
                ]
        ...
        frames = []
        for i, pose_local in enumerate(poses):
            # Convert local to world
            if uses_local:
                world_pose = list(pose_local)
                world_pose[0] += float(env_origin[0])
                world_pose[1] += float(env_origin[1])
                world_pose[2] += float(env_origin[2])
            else:
                world_pose = list(pose_local)

            pose = torch.tensor(world_pose, dtype=torch.float32, device=device).unsqueeze(0)

            self._agent.write_root_com_pose_to_sim(pose, env_tensor)
            self._agent.write_root_com_velocity_to_sim(zero_vel, env_tensor)
            ...
            if save_images and output_dir:
                ...
                if rgb_np.shape[-1] == 4:
                    rgb_np = rgb_np[..., :3]
                cv2.imwrite(
                    f"{output_dir}/replay_{i:04d}.png",
                    cv2.cvtColor(rgb_np, cv2.COLOR_RGB2BGR)
                )

        print(f"🎬 Replayed {len(frames)} viewpoints for env {env_id}")
        return frames
    ...
    def _select_viewpoints_for_collection(self, env_id: int) -> bool:
        ...
        # FIX: Use the physical slot (env_id) to query simulator state, 
        # not the incrementing global tracking ID!
        ...
        # Split into the two pools
        pool_a = all_viewpoints[dist_cam < dist_goal]
        pool_b = all_viewpoints[dist_goal < dist_cam]
        ...
        # Greedily pull exactly the required amount from each pool
        selected_a = select_from_pool(pool_a, half_req)
        selected_b = select_from_pool(pool_b, half_req)

        # Validate that we successfully pulled exactly half from both
    ...
    def _randomize_scene_props(self, env_ids: list[int]) -> None:
        ...
        if vpt_paths:
            ...
            # self.randomize_shape_color(prim_path_expr=vpt_paths)
    ...
    def initial_spawn_loop(self,
        ...
        Uses simple randomized rejection sampling (Standard Randomization) instead of 
        ...
        states : list[torch.Tensor]
            List containing mutable state tensors [goal, camera, agent, vpt_objs].
        allow_clipping : bool, optional
            If True, skips overlap checks between objects (fastest).
            If False, uses simple distance checks to prevent overlaps (fast).
        device : torch.device, optional
            The device for tensor operations.
        ...
        # --- 4. VPT Object Scattering Loop (Randomized) ---
        MAX_ATTEMPTS = 50
        ...
        for batch_idx, local_idx in enumerate(retry_indices):
            ...
            # Retrieve dims to check overlaps [num_active, 3]
            active_dims = self.all_vpt_dims[global_env_id, active_indices, :3]

            # Track placed objects: list of (x, y, radius_approx)
            placed_objects = []
            placement_failed = False
            ...
            for k, obj_idx in enumerate(active_indices):
                ...
                # Approximate radius for simple distance check
                obj_radius = max(obj_w, obj_l) / 2.0
                ...
                for _ in range(MAX_ATTEMPTS):
                    ...
                    # --- Constraint 1: Camera Distance ---
                    # Ensure object isn't right on top of the camera
                    dx_cam = rx - (cam_global_x - origin_x)
                    dy_cam = ry - (cam_global_y - origin_y)
                    dist_cam = math.sqrt(dx_cam**2 + dy_cam**2)
                    
                    if dist_cam < 4.5: # Keep clear of camera
                        continue

                    # --- Constraint 2: Goal Distance ---
                    # Ensure object isn't covering the goal
                    dx_goal = rx - (goal_global_x - origin_x)
                    dy_goal = ry - (goal_global_y - origin_y)
                    dist_goal = math.sqrt(dx_goal**2 + dy_goal**2)

                    if dist_goal < (self.goal_radius + 3.0):
                        continue

                    # --- Constraint 3: Object Overlap (Rejection Sampling) ---
                    if not allow_clipping:
                        overlap = False
                        for (px, py, pr) in placed_objects:
                            # Simple Euclidean check: dist < sum_of_radii
                            dist_obj = math.sqrt((rx - px)**2 + (ry - py)**2)
                            if dist_obj < (obj_radius + pr + 0.1): # 0.1 margin
                                overlap = True
                                break
                    ...
                    r_yaw_tensor = torch.tensor(r_yaw, device=device)
                    # Note: We do NOT set Z here; it is handled by the universal grounding loop later
                    quat = quat_from_euler_xyz(torch.tensor(0.0, device=device), 
                                               torch.tensor(0.0, device=device), 
                                               r_yaw_tensor)
                    vpt_obj_default_state[local_idx, obj_idx, 3:7] = quat

                    placed_objects.append((rx, ry, obj_radius))
                    found = True
                    break
    ...
    def _reset_idx_internal(self,
        ...
        # 1b. Stash reference_obj to storage
        ref_storage_pose = torch.tensor([
            self.storage_position[0], self.storage_position[1], self.storage_position[2],
            1.0, 0.0, 0.0, 0.0
        ], dtype=torch.float32, device=device).unsqueeze(0).expand(len(env_ids), -1)
        self._reference_obj.write_root_com_pose_to_sim(ref_storage_pose, env_ids)
        self._reference_obj.write_root_com_velocity_to_sim(
    ...
    def _check_fov_validity(self, points, env_ids, valid_mask, min_req_points):
        ...
        # Override env_status to track dual quotas
        ...
        for step in range(max_steps):
            ...
            for eid, data in env_queues.items():
                # Skip satisfied envs
                if env_status[eid]['done']: continue
            ...
            if not step_ids:
                if self.verbose >= 1 and all(s['done'] for s in env_status.values()):
                    print(
                        f"    ✅ All environments achieved 50/50 split "
                        f"({half_req} Cam, {half_req} Goal). Early stop."
                    )
                break
            ...
            # --- NEW: Classify batched points dynamically ---
            ...
            # Update counts & enforce strict quota dropping
                ...
                if is_vis[i]:
                    ...
                    if is_c:
                        if env_status[eid]['c_count'] < half_req:
                            ...
                            is_vis[i] = False  # Quota full, drop point
                    else:
                    ...
                    if env_status[eid]['c_count'] >= half_req and env_status[eid]['g_count'] >= half_req:
    ...
    def generate_valid_circle_points(
        ...
        """Generate valid viewpoint positions enforcing a strict 50/50 split 
        between Camera and Goal proximity, with a > 0.4m deadzone boundary.
        """
        device = self.device
        num_envs = len(env_ids)

        MIN_REQUIRED_POINTS = self.images_per_env
        HALF_REQUIRED = MIN_REQUIRED_POINTS // 2
        ...
        # Scale radii by random factor [0.75, 1.5]
        ...
        # Add random jitter +/- 1.0 to EACH point individually
        ...
        all_x = self._goal.data.root_pos_w[env_ids, 0].unsqueeze(
            1) + radii * torch.cos(angles_expanded) + jitter_x
        all_y = self._goal.data.root_pos_w[env_ids, 1].unsqueeze(
            1) + radii * torch.sin(angles_expanded) + jitter_y
        ...
        # Step 2: Single-Pool Displacement filtering & Labeling
        displacement_filtered_points = []
        displacement_filtered_labels = [] # NEW: True = Camera, False = Goal
        ...
        for i, env_id in enumerate(env_ids):
            ...
            # --- Apply 0.4m Deadzone ---
            deadzone_mask = torch.abs(dist_cam - dist_goal) > 0.4
            valid_points = valid_points[deadzone_mask]
            
            if len(valid_points) == 0:
                continue
                
            dist_cam = dist_cam[deadzone_mask]
            dist_goal = dist_goal[deadzone_mask]

            # --- Apply "Sticky Note" Labels ---
            # True means Camera-proximal, False means Goal-proximal
            is_c_point = dist_cam < dist_goal
            ...
            sorted_labels = is_c_point[sorted_indices]
            ...
            filtered_labels = sorted_labels[selected_mask]

            # Quick pre-check: Do we even have enough of both types to proceed?
            c_count = filtered_labels.sum().item()
            g_count = (~filtered_labels).sum().item()

            if c_count < HALF_REQUIRED or g_count < HALF_REQUIRED:
                if self.verbose >= 1:
                    print(f"  Env {env_id_item}: ❌ Failed pre-check (C: {c_count}, G: {g_count}, need {HALF_REQUIRED} each).")
                continue

            displacement_filtered_points.append(filtered_candidates)
            displacement_filtered_labels.append(filtered_labels)
        ...
        # Concatenate everything for the FOV batcher
        all_candidates = torch.cat(displacement_filtered_points, dim=0)
        all_labels = torch.cat(displacement_filtered_labels, dim=0)
        ...
        # Step 3: FOV check
        # Force the batcher to evaluate EVERY candidate by passing min_required_points=0
        fov_valid_mask = self._is_point_valid_batch(
            points=all_candidates,
            env_ids=all_candidates_env_ids,
            check_agent_fov=True,
            min_required_points=MIN_REQUIRED_POINTS  # <--- CHANGED FROM 0
        )

        self._agent.write_root_pose_to_sim(torch.cat([original_agent_pos, original_agent_quat], dim=-1), env_ids=env_ids)

        # Step 4: Harvest the exact 50/50 split
        all_valid_points = []
        ...
        for i, env_id in enumerate(env_ids):
            ...
            env_candidates = all_candidates[env_mask]
            env_labels = all_labels[env_mask]
            env_fov_mask = fov_valid_mask[env_mask]

            # Grab only the points that passed the FOV check
            valid_fov_points = env_candidates[env_fov_mask]
            valid_fov_labels = env_labels[env_fov_mask]

            # Separate back into C and G based on the boolean labels
            valid_c_points = valid_fov_points[valid_fov_labels]
            valid_g_points = valid_fov_points[~valid_fov_labels]

            # Enforce final quota
            if len(valid_c_points) >= HALF_REQUIRED and len(valid_g_points) >= HALF_REQUIRED:
                final_points = torch.cat([
                    valid_c_points[:HALF_REQUIRED], 
                    valid_g_points[:HALF_REQUIRED]
                ], dim=0)
                all_valid_points.append(final_points)
                
                if self.verbose >= 2:
                    print(f"  Env {env_id_item}: ✅ Collected {HALF_REQUIRED} C & {HALF_REQUIRED} G points.")
            else:
                all_valid_points.append(torch.zeros((0, 2), device=device))
                if self.verbose >= 1:
                    print(f"  Env {env_id_item}: ❌ Failed final split (C: {len(valid_c_points)}/{HALF_REQUIRED}, G: {len(valid_g_points)}/{HALF_REQUIRED})")
    ...
    def _save_depth_labels(self):
        ...
        # 1. Read existing data if the file is already there
        if os.path.exists(self.depth_labels_json_path):
            with open(self.depth_labels_json_path, 'r') as f:
                try:
                    existing_data = json.load(f)
                except json.JSONDecodeError:
                    pass # If the file is empty or corrupted, start fresh
                    
        # 2. Update the existing data with your new batch
    ...
    def _collect_images_for_slot(self, env_id: torch.Tensor, folder_idx: int) -> None:
        ...
        Tracks if the camera (1) or goal (0) is closer to the agent.
        """
        ...
        # Get static reference points for orientation and distance calculation
        ...
        # Initialize dictionary for this specific environment
        env_key = f"env_{folder_idx}"
        if env_key not in self.depth_labels_data:
            self.depth_labels_data[env_key] = {}
        
        agent_poses_for_slot = []
        ...
        for i in range(self.images_per_env):
            ...
            # --- NEW: Compute Label (1 if cam is closer, 0 if goal is closer) ---
            dist_cam = torch.norm(target_pos_2d - cam_pos_2d).item()
            dist_goal = torch.norm(target_pos_2d - goal_pos_2d).item()
            label = 1 if dist_cam < dist_goal else 0
            
            self.depth_labels_data[env_key][f"image_{i:04d}"] = label

            # Orientation: Look at midpoint between camera and goal
            ...
            agent_poses_for_slot.append(pose.squeeze(0).cpu().tolist())
            ...
            # 5. Save Data
            self._save_single_image(env_slot, folder_idx, rgb, depth, semantic, cam_pov, image_idx=i)

        # Cleanup & Final Save
        self.collected_agent_poses[env_slot] = agent_poses_for_slot
    ...
    def _save_single_image(self,
                           ...
                           semantic_data: torch.Tensor | None = None,
                           camera_pov_data: torch.Tensor | None = None,
                           image_idx: int = 0) -> None:
        """
        Process and save RGB, Depth, Semantic, and optional Camera POV images to disk.
        """
        # --- 1. Path Setup ---
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                try:
                    ...
                    env_idx = int(env_part.split("_")[-1])
                    wall_part = parts[-1]
                    if env_idx not in self.applied_wall_colors:
                        self.applied_wall_colors[env_idx] = {}
                    self.applied_wall_colors[env_idx][wall_part] = [
                        float(rand_color[0]), float(rand_color[1]), float(rand_color[2])
                    ]
    ...
    def randomize_spherical_lights(self, prim_paths, random_light_off=False):
        ...
        for path in prim_paths:
            ...
            if path not in active_paths:
                prim.GetAttribute("inputs:intensity").Set(0.0)
                # --- TRACK INACTIVE LIGHT ---
                if env_idx not in self.applied_light_params:
                    self.applied_light_params[env_idx] = []
                self.applied_light_params[env_idx].append({
                    "prim_path_suffix": path.split(f"env_{env_idx}/")[-1] if f"env_{env_idx}/" in path else path,
                    "intensity": 0.0,
            ...
            # Also populate env_metadata_cache
            if env_idx not in self.env_metadata_cache:
                self.env_metadata_cache[env_idx] = {}
            self.env_metadata_cache[env_idx]["lighting"] = self.applied_light_params[env_idx]
    ...
    def get_material_configs(self,
        ...
        # Deterministic: sort then take first N
        raw_paths = sorted(raw_paths)
        num_to_select = min(len(raw_paths), 100)
        selected_paths = raw_paths[:num_to_select]
        ...
        for path in selected_paths:
            material = sim_utils.MdlFileCfg(
                mdl_path=path, project_uvw=True, texture_scale=tex_scale,
            )
    ...
    def randomize_material(self, prim_paths: list, material_type: str):
        if material_type == "mat":
            ...
            print(f"⚠️ Unknown material type '{material_type}', skipping.")
            return
        ...
        for prim_path in prim_paths:
            ...
            sim_utils.bind_visual_material(prim_path, rand_material)
            ...
            try:
                ...
                if material_type == "vpt":
                    ...
                    obj_idx = int(obs_part.split("_")[-1])
                    self.applied_materials[(env_idx, obj_idx)] = rand_material
                elif material_type == "mat":
                    self.applied_mat_materials[env_idx] = rand_material
            except (StopIteration, ValueError):
                pass

            # Also populate env_metadata_cache for _save_env_config_to_json_new
            try:
                env_match = re.search(r"env_(\d+)", prim_path)
                if env_match:
                    eidx = int(env_match.group(1))
                    if eidx not in self.env_metadata_cache:
                        self.env_metadata_cache[eidx] = {}
                    if "textures" not in self.env_metadata_cache[eidx]:
                        self.env_metadata_cache[eidx]["textures"] = {"vpt_objects": {}, "floor": "unknown"}

                    if material_type == "mat":
                        self.env_metadata_cache[eidx]["textures"]["floor"] = rand_material
                    elif material_type == "vpt":
                        obj_match = re.search(r"obs_(\d+)", prim_path)
                        if obj_match:
                            oidx = int(obj_match.group(1))
                            self.env_metadata_cache[eidx]["textures"]["vpt_objects"][oidx] = rand_material
            except (StopIteration, ValueError):
                pass
