...
from .spawn_boundary import get_boundary_cfg, spawn_random_objects, get_rigid_obj
...
class VPTEnvCfg(DirectRLEnvCfg):
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        antialiasing_mode="FXAA",  # Set your desired mode here
    ...
    obj_assignment = {
        "cross": num_vpt_objs // 6 + (num_vpt_objs % 6 > 0),
        "L": num_vpt_objs // 6 + (num_vpt_objs % 6 > 1),
        "t": num_vpt_objs // 6 + (num_vpt_objs % 6 > 2),
        "cuboid":num_vpt_objs//6 + (num_vpt_objs % 6 > 3),
        "cone":num_vpt_objs//6 + (num_vpt_objs % 6 > 4),
        "cylinder":num_vpt_objs//6,
    }
    ...
    for shape, count in obj_assignment.items():
        for _ in range(count):
            ...
            # rigid_objects.append(get_rigid_obj(prim_path=f"/World/Objects/obs_{idx}", fix_shape=shape))
            idx += 1
