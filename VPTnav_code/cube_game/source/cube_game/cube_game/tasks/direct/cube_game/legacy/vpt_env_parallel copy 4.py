...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.min_viewpoint_distance = 0.15  # Minimum distance between viewpoints (meters)
        self.goal_pixel_threshold = 30  # Minimum pixels for goal visibility
        self.camera_pixel_threshold = 30  # Minimum pixels for camera visibility
    ...
    def _get_observations(self) -> dict:
        ...
        if envs_to_collect:
            ...
            for viewpoint_idx in range(self.images_per_env):
                ...
                # Teleport ALL agents to their respective viewpoint_idx
                for env_id_item in envs_to_collect:
                    target_pos_2d = self.selected_viewpoints_for_collection[env_id_item][viewpoint_idx]
                    
                    # Create full 3D position
                    target_pos_3d = torch.zeros(3, device=device)
                    target_pos_3d[:2] = target_pos_2d[:2]
                    ...
                    self._agent.write_root_com_pose_to_sim(pose, torch.tensor([env_id_item], device=device))
                    self._agent.write_root_com_velocity_to_sim(
                        torch.zeros((1, 6), device=device), 
                        torch.tensor([env_id_item], device=device)
                    )
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            for local_idx, env_idx in enumerate(final_valid_indices):
                ...
                # Validate based on visibility category
                camera_validation_passed = False
                
                if visibility_category == "in_view":
                    # Target MUST be visible (not occluded)
                    ...
                    # Target MUST NOT be visible (occluded)
                    ...
                    # Target MUST NOT be visible (outside FOV)
                    camera_validation_passed = not target_visible_in_camera
                    if not camera_validation_passed:
                        if self.verbose >= 1:
                            print(f"    ❌ Env {env_id_item} (folder {folder_idx}): Camera validation FAILED")
                ...
                # Original occlusion validation (only for in_view and occluded)
                if visibility_category in ["in_view", "occluded"]:
                    ...
                    if self.verbose >= 2:
                        print(f"    ✅ Env {env_id_item} (folder {folder_idx}): Camera pointed away for outside_fov")
    ...
    def _is_point_valid_batch(self, points: torch.Tensor, env_ids: torch.Tensor, 
                              ...
                              min_camera_obstacle_distance: float = 1.0,
        ...
        for batch_start in range(0, len(points_to_check), batch_size):
            print(f"Processing FOV batch: {batch_start} to {min(batch_start + batch_size, len(points_to_check))} / {len(points_to_check)}")
    ...
    def generate_valid_circle_points(self, env_ids: torch.Tensor, angle_step: float = 2.0,
        ...
        MAX_POINTS = self.images_per_env * 2
        ...
        radii = self._camera_obj.data.root_pos_w[env_ids, :2] - self._goal.data.root_pos_w[env_ids, :2]
        radii = torch.norm(radii, dim=1, keepdim=True)
        
        # Generate all points for all envs (num_envs × num_angles)
        ...
        # Step 1: Geometric validation (already parallel)
        ...
        # Step 2: Batch FOV validation for all valid geometric points across all envs
        all_valid_geometric_points = []
        all_valid_geometric_env_ids = []
        all_valid_geometric_indices = []  # Track which env each point belongs to
        
        for i, env_id in enumerate(env_ids):
            valid_mask = geometric_valid_per_env[i]
            if valid_mask.any():
                valid_points = all_points_batch[i * num_angles:(i + 1) * num_angles][valid_mask]
                all_valid_geometric_points.append(valid_points)
                all_valid_geometric_env_ids.extend([env_id.item()] * len(valid_points))
                all_valid_geometric_indices.extend([i] * len(valid_points))
        
        if len(all_valid_geometric_points) == 0:
            return [torch.zeros((0, 2), device=device) for _ in range(num_envs)]
        
        # Concatenate all valid geometric points
        all_valid_geometric_points = torch.cat(all_valid_geometric_points, dim=0)
        all_valid_geometric_env_ids = torch.tensor(all_valid_geometric_env_ids, dtype=torch.long, device=device)
        all_valid_geometric_indices = torch.tensor(all_valid_geometric_indices, dtype=torch.long, device=device)
        ...
        # Batch FOV check - teleport all agents to all their candidate points at once
        fov_valid_mask = self._is_point_valid_batch(
            points=all_valid_geometric_points,
            env_ids=all_valid_geometric_env_ids,
        ...
        # Step 3: Vectorized displacement filtering per environment
        all_valid_points = []
        ...
        for i, env_id in enumerate(env_ids):
            ...
            env_mask = all_valid_geometric_indices == i
            env_fov_valid = fov_valid_mask[env_mask]
            
            if not env_fov_valid.any():
                all_valid_points.append(torch.zeros((0, 2), device=device))
                if self.verbose >= 1:
                    print(f"  Env {env_id_item}: ❌ No FOV-valid points")
                continue
            
            valid_points_tensor = all_valid_geometric_points[env_mask][env_fov_valid]
            
            if self.verbose >= 2:
                print(f"  Env {env_id_item}: {len(valid_points_tensor)} FOV-valid points")
            
            # Sort by angle
            goal_pos = self._goal.data.root_pos_w[env_id, :2]
            relative_pos = valid_points_tensor - goal_pos
            ...
            # Vectorized displacement filtering
            ...
            for point_idx in range(1, len(sorted_points)):
                ...
                # Compute distances to all already filtered points
                distances = torch.norm(
            ...
            if self.verbose >= 2:
                print(f"  Env {env_id_item}: After displacement filter: {len(filtered_points_tensor)}")
            
            if len(filtered_points_tensor) >= MIN_REQUIRED_POINTS:
                all_valid_points.append(filtered_points_tensor)
                if self.verbose >= 2:
                    print(f"  Env {env_id_item}: ✅ {len(filtered_points_tensor)} valid viewpoints")
            else:
                all_valid_points.append(torch.zeros((0, 2), device=device))
                if self.verbose >= 1:
    ...
    def _check_target_in_img(self, file_name: str, cam_pov, return_red_count: bool = False) -> bool:
        ...
        red_mask = ((r >= 242) & (g <= 13) & (b <= 13))  # 0.95*255=242, 0.05*255=13
        red_count = red_mask.sum().item()
        ...
        if return_red_count:
            return target_visible_in_camera and not has_solid_circles, red_count
        return target_visible_in_camera and not has_solid_circles
