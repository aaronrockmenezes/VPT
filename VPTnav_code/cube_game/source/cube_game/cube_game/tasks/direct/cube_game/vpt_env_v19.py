...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: VPTEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        base = os.getenv("BASE_PATH", "/oscar/scratch/arock3/VPT1_DATA/v19")
    ...
    def _save_env_config_to_json(self, env_id: int, folder_idx: int):
        """
        Save complete environment configuration to JSON file (v2.0).
        ...
        env_id_item = env_id if isinstance(env_id, int) else env_id.item()
        ...
        config = {
            ...
            "metadata": {
                ...
                "env_origin": self.scene.env_origins[env_id].cpu().tolist()
            },
            
            # ========== GEOMETRY ==========
            "geometry": {
                "goal_ball": {
                    "position": self._goal.data.root_pos_w[env_id].cpu().tolist(),
                    "orientation": self._goal.data.root_quat_w[env_id].cpu().tolist(),
                    "radius": float(self.cfg.goal_ball.spawn.radius)
                },
                
                "camera_object": {
                    "position": self._camera_obj.data.root_pos_w[env_id].cpu().tolist(),
                    "orientation": self._camera_obj.data.root_quat_w[env_id].cpu().tolist(),
                    "fov_degrees": 30.0,
                    "pitch_degrees": float(self.cfg.agent_camera_pitch),
                    "jitter_metadata": {
                        "base_yaw_radians": None,  # TODO: Track during spawn
                        "jitter_offset_radians": None,
                        "was_jittered": True
                    }
                },
                
                "agent": {
                    "position": self._agent.data.root_pos_w[env_id].cpu().tolist(),
                    "orientation": self._agent.data.root_quat_w[env_id].cpu().tolist(),
                    "size": list(self.cfg.agent.spawn.size)
                },
                
                "vpt_objects": [],  # Populated below
                
                "walls": [
                    {
                        "name": "top_wall",
                        "position": self._boundary_top.data.root_pos_w[env_id].cpu().tolist(),
                        "orientation": self._boundary_top.data.root_quat_w[env_id].cpu().tolist()
                    },
                    {
                        "name": "bottom_wall",
                        "position": self._boundary_bottom.data.root_pos_w[env_id].cpu().tolist(),
                        "orientation": self._boundary_bottom.data.root_quat_w[env_id].cpu().tolist()
                    },
                    {
                        "name": "left_wall",
                        "position": self._boundary_left.data.root_pos_w[env_id].cpu().tolist(),
                        "orientation": self._boundary_left.data.root_quat_w[env_id].cpu().tolist()
                    },
                    {
                        "name": "right_wall",
                        "position": self._boundary_right.data.root_pos_w[env_id].cpu().tolist(),
                        "orientation": self._boundary_right.data.root_quat_w[env_id].cpu().tolist()
                    }
                ],
                
                "floor_mat": {
                    "position": self._mat.data.root_pos_w[env_id].cpu().tolist(),
                    "orientation": self._mat.data.root_quat_w[env_id].cpu().tolist()
                },
                
                "viewpoints": {
                    "valid_count": len(self.valid_viewpoint_poses[env_id_item]) if self.valid_viewpoint_poses[env_id_item] is not None else 0,
                    "valid_positions": self.valid_viewpoint_poses[env_id_item].cpu().tolist() if self.valid_viewpoint_poses[env_id_item] is not None else [],
                    "collected_count": len(self.selected_viewpoints_for_collection[env_id_item]) if self.selected_viewpoints_for_collection[env_id_item] is not None else 0,
                    "collected_positions": self.selected_viewpoints_for_collection[env_id_item].cpu().tolist() if self.selected_viewpoints_for_collection[env_id_item] is not None else []
                }
            },
            ...
            "rendering": {
                "materials": {
                    "floor": {
                        "mdl_path": None,  # TODO: Track during randomization
                        "texture_scale": [1000.0, 1000.0]
                    },
                    "walls": [
                        {"name": "top_wall", "color": None},
                        {"name": "bottom_wall", "color": None},
                        {"name": "left_wall", "color": None},
                        {"name": "right_wall", "color": None}
                    ],
                    "vpt_objects": [],  # Populated below
                    "goal_ball": {
                    ...
                    "agent": {
                        "color": list(self.cfg.agent.spawn.visual_material.diffuse_color)
                    }
                },
                "lighting": {
                    "lights": [
                        {
                            "name": "Light_A",
                            "position": None,  # TODO: Track during randomization
                            "intensity": None,
            ...
            "randomization_control": {
                "flags": {
                    ...
                    "randomize_wall_colors": True,
                    "randomize_object_scales": False,
                    "randomize_object_poses": False
                },
                "seeds": {
                    "material_seed": None,
                    "lighting_seed": None,
        ...
        # ============================================================
        # POPULATE VPT OBJECTS (ALL 32)
        # ============================================================
        active_indices = self.active_vpt_indices[env_id_item]
        active_set = set(active_indices.cpu().tolist())
        
        vpt_keys = list(self.cfg.vpt_objects.rigid_objects.keys())
        
        # Track which object is ball mount (if any)
        ball_mount_idx = None
        if hasattr(self, 'moved_vpt_for_ball') and env_id_item < len(self.moved_vpt_for_ball):
            ball_mount_idx = self.moved_vpt_for_ball[env_id_item]
        
        for obj_idx in range(self.cfg.num_vpt_objs):
            is_active = obj_idx in active_set
            
            # Get spawn config for this object type
            spawn_cfg = self.cfg.vpt_objects.rigid_objects[vpt_keys[obj_idx]].spawn
            
            # Determine shape type and USD filename
            shape_type = "unknown"
            usd_filename = None
            
            if isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                shape_type = "usd_mesh"
                usd_filename = spawn_cfg.usd_path.split("/")[-1].split(".")[0]
            elif isinstance(spawn_cfg, sim_utils.MeshCuboidCfg):
                shape_type = "cuboid"
            elif isinstance(spawn_cfg, sim_utils.MeshCylinderCfg):
                shape_type = "cylinder"
            elif isinstance(spawn_cfg, sim_utils.MeshConeCfg):
                shape_type = "cone"
            
            # Get dimensions
            base_dims = self.vpt_base_dims[obj_idx].cpu().tolist()
            final_dims = self.all_vpt_dims[env_id, obj_idx].cpu().tolist()
            
            # Calculate scale factors (final / base)
            scale_factors = [
                final_dims[i] / base_dims[i] if base_dims[i] > 1e-6 else 1.0
                for i in range(3)
            ]
            ...
            obj_data = {
                "index": obj_idx,
                "is_active": is_active,
                "position": self._vpt_objects.data.object_pos_w[env_id, obj_idx].cpu().tolist(),
                "orientation": self._vpt_objects.data.object_quat_w[env_id, obj_idx].cpu().tolist(),
                "shape_type": shape_type,
                "base_dimensions": base_dims,
                "scale_factors": scale_factors,
                "final_dimensions": final_dims,
                "z_offset_ratio": float(self.vpt_z_offset_ratios[env_id, obj_idx]),
                "shape_id": int(self.vpt_shapes[env_id, obj_idx]),
                "usd_filename": usd_filename,
                "is_ball_mount": (obj_idx == ball_mount_idx)
            }
            
            config["geometry"]["vpt_objects"].append(obj_data)
            
            # Add placeholder material data
            material_data = {
                "index": obj_idx,
                "mdl_path": None,  # TODO: Track during randomization
            ...
            config["rendering"]["materials"]["vpt_objects"].append(material_data)
        
        # ============================================================
        # SAVE TO DISK
        # ============================================================
        config_dir = f"{self.base_path}/frozen_configs"
        ...
        if self.verbose >= 2:
            print(f"  💾 Saved v2.0 config: env_{folder_idx}_config.json")
