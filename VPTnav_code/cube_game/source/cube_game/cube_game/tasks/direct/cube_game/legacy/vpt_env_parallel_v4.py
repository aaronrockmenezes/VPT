...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.base_path = "/media/data_cifs_lrs/projects/prj_robotics/VPTnav_v4"
    ...
    def get_color(self, avoid_color=None):
        """
        ...
        if avoid_color is not None:
            avoid_color = np.array(avoid_color)
            if avoid_color.shape == (3,):
                forbidden_colors.append(np.array(avoid_color))
            elif avoid_color.shape == (len(avoid_color), 3):
                for ac in avoid_color:
                    forbidden_colors.append(np.array(ac))
    ...
    def _customize_objects_for_visibility(self) -> None:
        """Customize object properties based on desired visibility category."""
        num_objs = self.cfg.num_vpt_objs
        current_color = self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[0]].spawn.visual_material.diffuse_color
        # print(f"Current color = {current_color}")
        # print(f"new color = {color.diffuse_color}")
        colors = []
        for obj_idx in range(num_objs):
            color = self.get_color()
            self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[obj_idx]].spawn.visual_material = color
            self._vpt_objects.write_data_to_sim()
            colors.append(color.diffuse_color)
        
        mat_color = self.get_color(avoid_color=colors)
        self._mat.cfg.spawn.visual_material = mat_color
        self._mat.write_data_to_sim()
        
        for _ in range(3):
            self.sim.step()
            self._vpt_objects.update(self.sim.cfg.dt)
        print(f"new color after update = {self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[0]].spawn.visual_material.diffuse_color}")
    
    def _customize_objects_for_size(self) -> None:
        """Customize object sizes randomly within specified ranges."""
        num_objs = self.cfg.num_vpt_objs
        sizes = []
        for idx in range(num_objs):
            sizes.append(self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[idx]].spawn.size)
        
        print(f"Current sizes = {sizes}")
        
        for obj_idx in range(num_objs):
            cuboid_size = (np.random.uniform(0.5, 1.0, (3,)).tolist()) if random.random() < 0.5 else (np.random.uniform(0.2, 0.5, (3,)).tolist())
            self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[obj_idx]].spawn.size = cuboid_size
            self._vpt_objects.write_data_to_sim()
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        ...
        for full_attempt in range(max_full_attempts):
            ...
            # Reset color and size of objects for retrying envs
            self._customize_objects_for_visibility()
            self._customize_objects_for_size()
    ...
    def _get_vpt_heights(self, env_idx):
        """Get the heights of VPT objects for given environment IDs."""
        device = self._agent.device
        heights = torch.zeros((self.num_objs), device=device)

        for obj_idx in range(self.num_objs):
            vpt_size = self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[obj_idx]].spawn.size
            heights[obj_idx] = vpt_size[2]  # Z dimension (height)

        return heights
    
    def _get_vpt_dims(self, env_idx):
        """Get the dims of VPT objects for given environment IDs."""
        device = self._agent.device
        dims = torch.zeros((self.num_objs, 3), device=device)

        for obj_idx in range(self.num_objs):
            vpt_size = self._vpt_objects.cfg.rigid_objects[list(self.cfg.vpt_objects.rigid_objects.keys())[obj_idx]].spawn.size
            dims[obj_idx] = vpt_size    # X, Y, Z dimensions
    ...
    def _check_target_vpt_distance(self, env_idx, target_position, vpt_positions, min_distance=1.0):
        ...
        dists = torch.norm(target_expanded - closest_point_world, dim=1) - goal_radius
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            goal_height_offset = sample_uniform(0.16, 2.0, (batch_size, 1), device)
    ...
    def _load_env_config_from_json(self, config_filepath: str, target_env_id: int = 0):
        ...
        for obj_data in config["vpt_objects"]["objects"]:
            ...
            vpt_colors.append(obj_data["spawn_cfg"]["visual_material"]["diffuse_color"])
        ...
        vpt_colors = torch.tensor(vpt_colors, device=device, dtype=torch.float32)
        ...
        vpt_poses = torch.cat([vpt_positions.unsqueeze(0), vpt_orientations.unsqueeze(0), torch.zeros((1, vpt_count, 6), device=device)], dim=2)
        self._vpt_objects.data.default_object_state[env_ids] = vpt_poses
        self._vpt_objects.write_data_to_sim()
        for color_idx, obj_key in enumerate(self.cfg.vpt_objects.rigid_objects.keys()):
            color = sim_utils.PreviewSurfaceCfg(diffuse_color=tuple(vpt_colors[color_idx].cpu().numpy()))
