...
def get_color():
    ...
    while not valid:
        ...
        base = np.array([random.uniform(0.0, 0.9) for _ in range(3)], dtype=float)
        ...
        if not too_close:
            valid = True
        # Ensure base has only floats
    return sim_utils.PreviewSurfaceCfg(diffuse_color=tuple(base.tolist()))
...
def get_shape(fix_shape=None):
    ...
    if chosen_shape == "capsule":
        ...
        shape = sim_utils.MeshCylinderCfg(
...
def get_shape_randomize(fix_shape=None):
    # shapes_list = ["capsule", "cone", "cuboid", "cylinder", "sphere"]
    shapes_list = ["cone", "cuboid", "cylinder", "cross", "L", "t", "table_A", "table_B", "A", "H", "I", "Z", "Bench"]
    # shapes_list = ["cone", "cuboid", "cylinder", "sphere"]
    # 50% big, 50% small
    ...
    if chosen_shape == "cone":
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/Cross_Norm/Kit1_Cross.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/capital_X.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Capital_letter_L.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            usd_path="/mnt/VPT/VPTnav_code/cube_game/assets/Kit1_Character_T.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
            scale=(1,1,1),
            semantic_tags=[("class", "obstacles")],)
    elif chosen_shape == "table_A":
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Table_A.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
            scale=(1,1,1),
            semantic_tags=[("class", "obstacles")],)
    elif chosen_shape == "table_B":
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Table_B.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Capital_Letter_A.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Capital_Letter_H.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Capital_Letter_I.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
        ...
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Capital_Letter_Z.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
            scale=(1,1,1),
            semantic_tags=[("class", "obstacles")],)
    elif chosen_shape == "Bench":
        shape = sim_utils.UsdFileCfg(
            # usd_path="/home/arock3/Documents/isaaclab_game/isaac_game/cube_game/mass_assets/L_Norm/Kit1_Character_L.usd",
            usd_path="/mnt/VPT/VPTnav_code/cube_game/mass_assets/Bench.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=1.0),
            rigid_props=sim_utils.RigidBodyPropertiesCfg(disable_gravity=True, max_linear_velocity=0.0, max_angular_velocity=0.0),
            # collision_props=sim_utils.CollisionPropertiesCfg(collision_enabled=True),
            visual_material=get_color(),
...
def get_rigid_obj(prim_path, fix_shape=None):
    ...
    if fix_shape in ["None"]:
        ...
        # Random yaw
        random_yaw = np.random.uniform(0, 2 * np.pi)
        rot = euler_to_quaternion(random_yaw, 0, 0)
        # rot = (0.0, 0.0, 0.0, 1.0)
    ...
    rigid_obj = RigidObjectCfg(
        ...
        spawn=shape,
        init_state=RigidObjectCfg.InitialStateCfg(
            # pos=(0.0, 0.0, 0.0) if fix_shape not in ["cross", "L", "t"] else (0.0, 0.0, 0.5),
            pos=(0.0, 0.0, 0.0),
            rot=tuple(rot.tolist()),
            lin_vel=(0.0, 0.0, 0.0),
            ang_vel=(0.0, 0.0, 0.0)
        )
    )
...
def get_mat_material_paths_old() -> list[str]:
    """
...
def get_vpt_material_paths_old() -> list[str]:
    """
    Returns a curated list of standard NVIDIA MDL material paths 
    for the Visual Perspective Taking (VPT) task.
    """
...
def get_obj_assignment(shapes: list[str], num_vpt_objs: int) -> dict[str, int]:
    """
    Distributes num_vpt_objs among the provided shapes as evenly as possible.
    """
    num_shapes = len(shapes)
    base_count = num_vpt_objs // num_shapes
    remainder = num_vpt_objs % num_shapes

    obj_assignment = {}

    for i, shape in enumerate(shapes):
        # Add 1 to the count if the current index is within the remainder
        extra = 1 if i < remainder else 0
        obj_assignment[shape] = base_count + extra
