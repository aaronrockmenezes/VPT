...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.verbose = 0
        self.goal_pixel_threshold_agent = 500  # Minimum pixels for goal visibility
        self.goal_pixel_threshold_cam = 500  # Minimum pixels for goal visibility
        # self.camera_pixel_threshold = 500  # Minimum pixels for camera visibility
        self.camera_pixel_threshold_agent = 800  # Minimum pixels for camera visibility
        self.ref_obj_pixel_threshold_agent = 500
        self.ref_obj_pixel_threshold_cam = 500
        ...
        base = os.getenv("BASE_PATH", "/oscar/scratch/arock3/VPT2_DATA/v4")
    ...
    def _collect_images_for_slot(self, env_id: torch.Tensor, folder_idx: int) -> None:
        ...
        for viewpoint_idx in range(self.images_per_env):
            ...
            SAFETY_MARGIN = math.radians(3.0)
            DESIRED_MAX   = math.radians(20.0)
            WINDOW        = HALF_FOV - SAFETY_MARGIN  # 12°

            def angle_delta(obj_pos_2d):
                """Angular offset from base_yaw to direction toward obj."""
                d = obj_pos_2d - target_pos_3d[:2]
                a = torch.atan2(d[1], d[0])
                delta = a - base_yaw
                # Normalize to [-pi, pi]
                delta = (delta + math.pi) % (2 * math.pi) - math.pi
                return delta.item()

            delta_cam  = angle_delta(cam_pos_2d)
            delta_goal = angle_delta(goal_pos_2d)

            # Get active ref obj position
            active_ref_variant = self.active_ref_idx[env_id_item]
            ref_pos_2d = self._ref_objs[active_ref_variant].data.root_pos_w[env_id_item, :2]
            delta_ref  = angle_delta(ref_pos_2d)

            # Intersect all three ±12° windows + ±20° budget
            jitter_lo = max(delta_cam  - WINDOW, delta_goal  - WINDOW, delta_ref  - WINDOW, -DESIRED_MAX)
            jitter_hi = min(delta_cam  + WINDOW, delta_goal  + WINDOW, delta_ref  + WINDOW,  DESIRED_MAX)

            if jitter_lo < jitter_hi:
                jitter_val = random.uniform(jitter_lo, jitter_hi)
            else:
                # Fallback: aim at centroid of the three objects
                jitter_val = (delta_cam + delta_goal + delta_ref) / 3.0
