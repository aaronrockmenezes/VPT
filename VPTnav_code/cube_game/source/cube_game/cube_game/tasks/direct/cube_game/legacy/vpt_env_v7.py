...
from .vpt_env_cfg_v7 import VPTEnvCfg

def randomize_shape_scale(prim_path_expr: str|list):
    """Randomize the scale of the geometry."""
    # get world instance
    ...
    with Sdf.ChangeBlock():
        for prim_path in prim_paths:
            ...
            if prim.IsValid():
                ...
                # Find existing scale op or create new one
                scale_op = None
                ...
                if scale_op is None:
                    # Add with Double precision (not Float)
                    scale_op = xform.AddScaleOp(UsdGeom.XformOp.PrecisionDouble)
                
                # generate random scale
                random_scale = (random.uniform(0.5, 1.5),random.uniform(0.8, 1.2))
                scale_value = Gf.Vec3d(random_scale[0], random_scale[0], random_scale[1])
                scale_op.Set(scale_value)
                # print(f"Rescaled {prim_path} to {random_scale}")
            else:
                print(f"Warning: Prim not found at {prim_path}")
    
    # restart physics after modifications
    world.play()
    # print("Starting world again")

def randomize_shape_color(prim_path_expr: str|list):
    ...
    with Sdf.ChangeBlock():
        for prim_path in prim_paths:
            ...
            # print(f"Recolored {prim_path} to {color_spec.default}")
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # File paths
        self.base_path = f"/home/arock3/data_v8/data_{self.GPU_ID}"
    ...
    def _cache_vpt_object_dims(self):
        ...
        # Hardcoded scaling factors from your original code
        cross_scaling_factors = (1/0.06, 1/0.06, 1/0.020182)
        l_scaling_factors = (1/0.021433, 1/0.036474, 1/0.018880)
        t_scaling_factors = (1/0.020121, 1/0.034027, 1/0.018880)
        ...
        for i, key in enumerate(vpt_keys):
            ...
            if isinstance(spawn_cfg, sim_utils.CuboidCfg):
                ...
                if first_word_last_letter == "s":   # Cross
                    x_dim /= cross_scaling_factors[0]
                    y_dim /= cross_scaling_factors[1]
                    z_dim /= cross_scaling_factors[2]
                elif first_word_last_letter == "L": # L
                    x_dim /= l_scaling_factors[0]
                    y_dim /= l_scaling_factors[1]
                    z_dim /= l_scaling_factors[2]
                elif first_word_last_letter == "T": # T
                    x_dim /= t_scaling_factors[0]
                    y_dim /= t_scaling_factors[1]
                    z_dim /= t_scaling_factors[2]
    ...
    #     vpt_keys = list(self.cfg.vpt_objects.rigid_objects.keys())
    #     for local_idx, obj_idx in enumerate(active_indices):
    #         if isinstance(self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn, sim_utils.CuboidCfg):
    #             vpt_size = torch.tensor(self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.size, device=device)

    #         elif isinstance(self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn, sim_utils.ConeCfg):
    #             vpt_radius = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.radius
    #             vpt_height = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.height
    #             vpt_size = torch.tensor([2 * vpt_radius, 2 * vpt_radius, vpt_height], device=device)

    #         elif isinstance(self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn, sim_utils.CylinderCfg):
    #             vpt_radius = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.radius
    #             vpt_height = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.height
    #             vpt_size = torch.tensor([2 * vpt_radius, 2 * vpt_radius, vpt_height], device=device)

    #         elif isinstance(self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn, sim_utils.UsdFileCfg):
    #             vpt_size = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.scale
    #             path = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.usd_path
    #             first_word_last_letter = path.split('.')[0][-1]
    #             cross_scaling_factors = (1/0.06, 1/0.06, 1/0.020182)
    #             l_scaling_factors = (1/0.021433, 1/0.036474, 1/0.018880)
    #             t_scaling_factors = (1/0.020121, 1/0.034027, 1/0.018880)
    #             if first_word_last_letter == "s":   # Cross
    #                 x_dim = vpt_size[0] / cross_scaling_factors[0]
    #                 y_dim = vpt_size[1] / cross_scaling_factors[1]
    #                 z_dim = vpt_size[2] / cross_scaling_factors[2]
    #             elif first_word_last_letter == "L": # L
    #                 x_dim = vpt_size[0] / l_scaling_factors[0]
    #                 y_dim = vpt_size[1] / l_scaling_factors[1]
    #                 z_dim = vpt_size[2] / l_scaling_factors[2]
    #             elif first_word_last_letter == "T": # L
    #                 x_dim = vpt_size[0] / t_scaling_factors[0]
    #                 y_dim = vpt_size[1] / t_scaling_factors[1]
    #                 z_dim = vpt_size[2] / t_scaling_factors[2]
    #             vpt_size = torch.tensor([x_dim, y_dim, z_dim], device=device)
    #         dims[local_idx] = vpt_size.clone()
    ...
    def _get_active_vpt_dims(self, env_ids) -> torch.Tensor:
        ...
        Returns:
            If single env: Tensor of shape (active_vpt_objs, 3)
            If batch envs: Tensor of shape (batch_size, active_vpt_objs, 3)
        """
        # Standardize input to Tensor
        ...
        # --- FIX START: Handle List vs Tensor ---
        ...
        # 1. Get Active Indices for these envs
        batch_indices = active_indices_tensor[env_ids]

        # 2. Lookup Dimensions using Advanced Indexing
        # Assumes self.all_vpt_dims exists (created via _cache_vpt_object_dims)
        batch_dims = self.all_vpt_dims[batch_indices]

        # 3. Handle Single Environment Case (Squeeze batch dim to maintain compatibility)
        if batch_dims.shape[0] == 1:
            return batch_dims.squeeze(0)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        ...
        randomize_shape_color(prim_path_expr=[f"/World/envs/env_.*/obs_{idx}" for idx in range(self.cfg.num_vpt_objs)])
        randomize_shape_scale(prim_path_expr=[f"/World/envs/env_.*/obs_{idx}" for idx in range(self.cfg.num_vpt_objs)])
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            if len(move_ball_indices) > 0:
                ...
                print(batch_dims, batch_dims.shape)
...
# interact -t 00:01:00 -q gpu-debug -g 1 -n 10 -f l40s
# cd /files22_lrsresearch/CLPS_Serre_Lab/projects/prj_robotics