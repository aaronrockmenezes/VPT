...
class VPTEnv(DirectRLEnv):
    ...
    def move_agent(self, actions, env_ids: Sequence[int] | None = None):
        ...
        # Track which environments are doing action 6 (for batch processing)
        action_6_mask = torch.zeros(num_envs, dtype=torch.bool, device=device)
        action_6_indices = []
        ...
        for i, _ in enumerate(env_ids):
            ...
            elif action == 6:  # Move to random valid viewpoint - PARALLELIZED
                action_6_mask[i] = True
                action_6_indices.append(i)
                
                # Set velocity to zero (will be teleported)
                desired_vel[i, :3] = 0.0
                desired_vel[i, 3:6] = 0.0
        ...
        # BATCH PROCESS ACTION 6 (Parallel Viewpoint Selection with Visibility Check)
        if action_6_mask.any():
            action_6_env_indices = torch.tensor(action_6_indices, dtype=torch.long, device=device)
            action_6_env_ids = env_ids[action_6_env_indices]
            
            # Process each environment doing action 6
            for local_idx, global_idx in enumerate(action_6_env_indices):
                ...
                if (self.valid_viewpoint_poses is not None
                    ...
                    for attempt in range(max_attempts):
                        ...
                        # Temporarily set agent position and orientation for visibility check
                        temp_pos = current_pos[global_idx].clone()
                        ...
                        # Update camera to check visibility
                        for _ in range(3):
                            self.sim.step()
                        ...
                        if goal_visible and camera_visible:
                            # Mark as used and update final pose
                            self.used_viewpoint_indices[env_id_item].add(random_idx)
                            found_valid = True
                            
                            # Update the actual position and orientation
                            current_pos[global_idx, :3] = target_pos
                            new_quat[global_idx] = temp_quat
            ...
            if self.verbose >= 2:
                print(f"[Action 6] Processed {len(action_6_env_ids)} environments")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        """Reset environments with validation for viewpoints and occlusion status (FULLY PARALLELIZED)."""
        ...
        # Assign visibility categories ONCE at the start (50% in_view, 25% occluded, 25% outside_fov)
        ...
        # Set labels ONCE for all environments
        for i in range(num_envs):
            global_folder_idx = original_folder_indices[i]
        ...
        # Track which environments need retry (use boolean mask for efficiency)
        envs_need_retry = torch.ones(num_envs, dtype=torch.bool, device=self.device)
        
        for reset_attempt in range(max_reset_attempts):
            # Get indices of environments that still need reset
            retry_mask = envs_need_retry
            if not retry_mask.any():
                if self.verbose >= 1:
                    print(f"✅ All {num_envs} environments successfully reset after {reset_attempt} attempts")
                break
            
            num_to_retry = retry_mask.sum().item()
            retry_env_ids = env_ids[retry_mask]
            retry_folder_indices = [original_folder_indices[i] for i in range(num_envs) if retry_mask[i]]
            
            # Use the SAME visibility categories for retries (preserve assignment)
            retry_visibility_categories = [visibility_categories[i] for i in range(num_envs) if retry_mask[i]]
            
            if self.verbose >= 1:
                print(f"\n🔄 Reset attempt {reset_attempt + 1}/{max_reset_attempts}: Processing {num_to_retry} environment(s)")
            
            # Reset ALL failing environments at once (PARALLEL)
            self._reset_idx_internal(
                retry_env_ids, 
                randomize_objects, 
                folder_indices=retry_folder_indices,
                visibility_categories=retry_visibility_categories
            )
            
            # Batch validate all environments that were just reset
            for local_idx, global_idx in enumerate([i for i in range(num_envs) if retry_mask[i]]):
                env_id = env_ids[global_idx]
                env_id_item = env_id.item() if torch.is_tensor(env_id) else env_id
                ...
                if is_valid:
                    ...
                    if visibility_reason in ["in_view", "occluded"]:
                        ...
                        if is_occluded != expected_occluded:
                            is_valid = False
                            reason = f"occlusion mismatch: expected {'occluded' if expected_occluded else 'visible'}, got {'occluded' if is_occluded else 'visible'}"
                
                # Update retry mask
                envs_need_retry[global_idx] = not is_valid
                
                if not is_valid and self.verbose >= 1:
                    print(f"  ❌ Env {env_id_item} (folder {folder_idx}, {visibility_categories[global_idx]}): {reason}")

        # Final comprehensive validation for ALL environments
        if self.verbose >= 1:
            print(f"\n🔍 Final validation: Verifying occlusion status for all {num_envs} environments...")
        ...
        for i, env_id in enumerate(env_ids):
            ...
            visibility_label = self.env_visibility_labels.get(folder_idx, "UNKNOWN")
            visibility_reason = self.env_visibility_reasons.get(folder_idx, "unknown")
            
            # Validate occlusion for in_view and occluded
            if visibility_reason in ["in_view", "occluded"]:
                camera_pos = self._camera_obj.data.root_pos_w[env_id]
                goal_pos = self._goal.data.root_pos_w[env_id]
                is_occluded = self._check_occlusion_raycast(camera_pos, goal_pos, env_id)
                ...
                expected_occluded = (visibility_reason == "occluded")
                status = "✅" if is_occluded == expected_occluded else "❌"
            ...
            elif visibility_reason == "outside_fov":
                # For outside_fov, just check that camera and goal exist
                validation_results.append({
                    "env_id": env_id_item,
                    "folder_idx": folder_idx,
                    "label": visibility_label,
                    "reason": visibility_reason,
                    "expected_occluded": None,
                    "actual_occluded": None,
                    "valid": True,
                    "status": "✅"
                })
        ...
        if self.verbose >= 1:
            ...
            for r in in_view_results:
                print(f"  {r['status']} Env {r['env_id']} (folder {r['folder_idx']}): "
                      f"Label={r['label']}, Occluded={r['actual_occluded']}")
        ...
        if envs_need_retry.any():
            num_failed = envs_need_retry.sum().item()
            print(f"\n⚠️  RESET FAILURE: {num_failed} environment(s) still invalid after {max_reset_attempts} attempts")
            for i in range(num_envs):
                if envs_need_retry[i]:
                    env_id_item = env_ids[i].item() if torch.is_tensor(env_ids[i]) else env_ids[i]
                    print(f"  ❌ Env {env_id_item} (folder {original_folder_indices[i]}, {visibility_categories[i]})")
    ...
    def generate_valid_circle_points(self, env_ids: torch.Tensor, angle_step: float = 2.0,
        ...
        for i, env_id in enumerate(env_ids):
            ...
            if len(candidate_points) > 50:
                ...
                indices = torch.arange(0, len(candidate_points), step, device=device)[:50]
