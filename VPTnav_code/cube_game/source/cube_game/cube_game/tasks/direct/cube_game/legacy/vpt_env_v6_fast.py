...
from .vpt_env_cfg_v6 import VPTEnvCfg
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # Check for CUDA_VISIBLE_DEVICES environment variable to find the global ID
        self.cuda_visible_devices = os.environ.get('CUDA_VISIBLE_DEVICES')

        # File paths
        self.base_path = f"/home/arock3/Documents/data_v6_fast_{self.cuda_visible_devices}"
        print("*"*50)
        ...
        self.total_envs_to_sim = self.num_envs * (self.cfg.num_vpt_objs // self.cfg.objects_per_env)
        ...
        self.buffered_vpt_objects = set()  # Temporary buffer for objects being tested
    ...
    def _select_active_vpt_indices(self, env_ids: torch.Tensor) -> None:
        """Randomly select active VPT object indices for each environment.
        Objects are placed in a buffer and NOT committed to used_vpt_objects yet.
        ...
        for env_id in env_ids:
            ...
            # Get available indices (not yet used OR buffered)
            all_reserved = self.used_vpt_objects | self.buffered_vpt_objects
            available_indices = list(set(range(self.num_objs)) - all_reserved)
            ...
            # Add to buffer (NOT to used yet)
            self.buffered_vpt_objects.update(active_indices.cpu().numpy().tolist())
            
            if self.verbose >= 2:
                leftover_count = self.num_objs - len(all_reserved) - self.active_vpt_objs
                print(f"  🎲 Env {env_id_item}: Selected {self.active_vpt_objs} active VPT indices (buffered, not committed)")
                print(f"     Available objects remaining: {leftover_count}/{self.num_objs} (used: {len(self.used_vpt_objects)}, buffered: {len(self.buffered_vpt_objects)})")


    def _commit_vpt_indices(self, env_id) -> None:
        """Commit buffered VPT indices to permanent used set after env passes validation.
        
        Args:
            env_id: Environment ID (int or tensor) that passed validation
        """
        env_id_item = env_id.item() if torch.is_tensor(env_id) else env_id
        
        if env_id_item not in self.active_vpt_indices or self.active_vpt_indices[env_id_item] is None:
            if self.verbose >= 1:
                print(f"  ⚠️  Env {env_id_item}: No active indices to commit")
            return
        
        # Get this env's active indices
        active_indices = self.active_vpt_indices[env_id_item]
        indices_list = active_indices.cpu().numpy().tolist()
        
        # Commit to permanent used set
        self.used_vpt_objects.update(indices_list)
        
        # Remove from buffer
        self.buffered_vpt_objects.difference_update(indices_list)
        
        if self.verbose >= 2:
            print(f"  ✅ Env {env_id_item}: Committed {len(indices_list)} VPT indices to used set (total used: {len(self.used_vpt_objects)})")


    def _release_buffered_vpt_indices(self, env_id) -> None:
        """Release buffered VPT indices back to available pool when env fails validation.
        
        Args:
            env_id: Environment ID (int or tensor) that failed validation
        """
        env_id_item = env_id.item() if torch.is_tensor(env_id) else env_id
        ...
        # Remove from buffer (making them available again)
        self.buffered_vpt_objects.difference_update(indices_list)
        
        # Clear this env's active indices
        self.active_vpt_indices[env_id_item] = None
        
        if self.verbose >= 2:
            print(f"  🔄 Env {env_id_item}: Released {len(indices_list)} buffered VPT indices back to pool")
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        ...
        while self.next_env_id < self.total_envs_to_sim or len(self.completed_envs) < self.total_envs_to_sim:
            ...
            # Release buffered VPT indices for failed slots
            for slot_idx in failed_slots:
                env_id = self.slot_to_env_id[slot_idx]
                self._release_buffered_vpt_indices(env_id)
                
            # Commit VPT indices for valid slots
            for slot_idx in valid_slots:
                env_id = self.slot_to_env_id[slot_idx]
                self._commit_vpt_indices(env_id)
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["writing_spawn_pose_time"][spawn_attempt] = "failed"
            ...
            for local_idx, env_idx in enumerate(move_ball_indices):
                ...
                # ========== NEW: Filter for cylinders and cuboids only ==========
                vpt_keys = list(self.cfg.vpt_objects.rigid_objects.keys())
                valid_shape_indices = []
                
                for local_idx_obj, obj_idx in enumerate(active_indices):
                    obj_idx_item = obj_idx.item() if torch.is_tensor(obj_idx) else obj_idx
                    spawn_cfg = self.cfg.vpt_objects.rigid_objects[vpt_keys[obj_idx_item]].spawn
                    
                    # Check if it's a cylinder or cuboid
                    if isinstance(spawn_cfg, (sim_utils.CylinderCfg, sim_utils.CuboidCfg)):
                        valid_shape_indices.append(local_idx_obj)
                
                if len(valid_shape_indices) == 0:
                    print(f"  ❌ No cylinders/cuboids found for env {env_ids[env_idx].item()}. Skipping ball move.")
                    continue
                # ================================================================
                
                # Pick a random VPT object that's not too tall
                tries = 0
                max_tries = len(valid_shape_indices) * 2
                while True:
                    # ========== CHANGE #8: Use _get_active_vpt_dims and select from active_indices ==========
                    random_valid_idx = random.choice(valid_shape_indices)
                    random_obj_idx = active_indices[random_valid_idx].item()
                    vpt_dims = self._get_active_vpt_dims(env_idx)
                    vpt_height = vpt_dims[random_valid_idx, 2]
                    # ========================================================================================
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["moving_ball_time"][spawn_attempt] = "failed"
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["vpt_occlusion_time"][spawn_attempt] = "failed"
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["vpt_in_out_fov_time"][spawn_attempt] = "failed"
                        env_dict[self.slot_to_env_id[idx]]["vpt_object_placement_time"][spawn_attempt] = "failed"
            ...
            if self.verbose >= 1:
                print(f"    🕒 Camera writing time: {camera_writing_end_time - camera_writing_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
                current_time = time.time()
                elapsed = current_time - reset_internal_start_time
                spawn_elapsed = current_time - spawn_start_time
                print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["camera_posing_time"][spawn_attempt] = "failed"
            ...
            if self.verbose >= 1:
                print(f"    🕒 Occlusion raycast time: {occlusion_raycast_end_time - occlusion_raycast_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
                current_time = time.time()
                elapsed = current_time - reset_internal_start_time
                spawn_elapsed = current_time - spawn_start_time
                print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["occlusion_raycast_time"][spawn_attempt] = "failed"
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["geometric_check_time"][spawn_attempt] = "failed"
            ...
            if self.verbose >= 1:
                print(f"    🕒 Camera POV validation time: {occlusion_geometric_end_time - occlusion_geometric_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
                current_time = time.time()
                elapsed = current_time - reset_internal_start_time
                print(f"Elapsed time from reset internal start: {elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            # Update dict
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    ...
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["camera_validation_time"][spawn_attempt] = "failed"
        ...
        for idx in range(num_envs):
            if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                ...
                if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["circle_validation_time"][spawn_attempt] = "failed"
...
#module load python/3.11.0s-ixrhc3q cuda/12.4.0-piq32fy  cudnn/8.9.6.50-12-56zgdoa