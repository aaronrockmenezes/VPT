...
class CubeGameEnv(DirectRLEnv):
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_mat_goal: bool = True, objects_close_to_goal: bool = False) -> None:
        if env_ids is None:
            env_ids = self._agent._ALL_INDICES
        ...
        wall_safe_x_range = safe_x_range - 1.5  # Reduced range for wall
        wall_safe_y_range = safe_y_range - 1.5
        ...
        for i in range(num_envs):
            ...
            while not valid_positions and attempts < max_attempts:
                ...
                if objects_close_to_goal:
                    ...
                    wall_distance = sample_uniform(0.6, max_distance_from_mat, (1,), device) + 0.8
...
def compute_rewards(
    ...
    mat_positions = mat.data.root_pos_w.clone()
    ...
    REWARD_CHART = {
        ...
        "goal_displacement": 0.0,  # Max reward for moving goal from default position
        "wall_collision": 0.5
    }
    ...
    goal_displacement_distances = torch.norm(goal_positions[:, :2] - goal_default_positions[:, :2], dim=1)
    ...
    displacement_scale = torch.clamp(goal_displacement_distances / max_displacement, 0.0, 1.0)
    ...
    for i in range(batch_size):
        if collision_mask[i]:
            ...
            if collision_type == "goal":
                ...
                # print(f"Mat penalty -50 for env {i}")
    ...
    # if total_reward.max() > 0:
        ...
        # print(f"Goal displacements: {goal_displacement_distances}, Displacement rewards: {displacement_reward}")
