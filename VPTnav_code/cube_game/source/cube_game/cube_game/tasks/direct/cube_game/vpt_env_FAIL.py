...
class VPTEnv(DirectRLEnv):
    ...
    def move_agent(self, actions, env_ids: Sequence[int] | None = None):
        ...
        for i, _ in enumerate(env_ids):
            ...
            elif action == 5:  # Move to random circle point
                # move to a random self.final_position
                target_pos = self.circle_positions_valid[i][random.randint(
                    0,
                    len(self.circle_positions_valid[i]) - 1)].to(device)

                # Ensure agent quat is looking at midpoint of cam and goal
                centers = (self._camera_obj.data.root_pos_w[env_ids[i], :3] +
                           self._goal.data.root_pos_w[env_ids[i], :3]) / 2.0
                direction = centers - current_pos[i, :3]
                direction[
                    2] = 0.0  # Ignore vertical component for yaw calculation
                direction = direction / torch.norm(direction)

                # Calculate yaw angle
                yaw = math.atan2(direction[1].item(), direction[0].item())

                # Create quaternion from yaw angle
                new_quat[i] = torch.tensor(
                    [math.cos(yaw / 2), 0.0, 0.0,
                     math.sin(yaw / 2)],
                    device=device)
                new_quat[i] = math_utils.quat_mul(
                    new_quat[i].unsqueeze(0),
                    upright_quat[i].unsqueeze(0)).squeeze(0)

                # Store target position for this environment
    ...
    def _reset_idx(self,
        ...
        self.circle_positions_valid = self._generate_valid_circle_points(
            env_ids,
            num_points=5,
    ...
    def _generate_valid_circle_points(
        self,
        env_ids: int = None,
        num_points: int = 10,
        ...
        for env_idx in env_ids:
            camera_pos = self._camera_obj.data.root_pos_w[env_idx, :3]
            target_pos = self._goal.data.root_pos_w[env_idx, :3]
            agent_pos = self._agent.data.root_pos_w[env_idx, :3]
            vpt_obj_pos = self._vpt_objects.data.default_object_state[
                env_idx, :, :3]

            dist_btw_cam_target = torch.norm(camera_pos[:2] - target_pos[:2])

            radius = compute_circle_radius_from_fov(
                camera_to_target_distance=dist_btw_cam_target,
                horizontal_fov_degrees=50.0,
                coverage_fraction=1.0)

            # Midpoint of camera and target
            circle_center = (camera_pos + target_pos) / 2

            # We work with only xy plane
            circle_center = circle_center[:2]

            if self.verbose >= 2:
                print(f"Radius for Env 0: r = {radius}")
            ...
            while (len(acceptable_positions) < num_points) and (max_attempts >
                                                                attempts):
                attempts += 1
                # Randomly select an angle
                selected_angle = random.choice(angles)
                # Remove selected angle from angles
                angles = angles[angles != selected_angle]

                angles_checked.append(selected_angle)

                # First check if point corresponding to angle is within bounds
                points_xy = point_on_circle(radius, selected_angle)

                clear_of_obstacles = False

                if self._is_within_bounds(env_idx, points_xy):
                    # Now check if point is clear of obstacles
                    proposed_pos = torch.zeros((3), device=self.device)
                    proposed_pos[0] = circle_center[0] + points_xy[
                        0] + self.scene.env_origins[env_idx, 0]
                    proposed_pos[1] = circle_center[1] + points_xy[
                        1] + self.scene.env_origins[env_idx, 1]
                    proposed_pos[2] = agent_pos[2]
                    clear_of_obstacles = self._is_clear_of_obstacles(
                        obstacle_poses=vpt_obj_pos,
                        proposed_poses=proposed_pos,
                        min_clearance=obstacle_clearance)
                    if clear_of_obstacles:
                        # Calculate quaternion for agent to look at center of camera and goal
                        centers = (camera_pos + target_pos) / 2.0
                        direction = centers - proposed_pos
                        direction[2] = 0.0  # Ignore vertical component for yaw calculation
                        direction = direction / torch.norm(direction)
                        
                        # Calculate yaw angle
                        yaw = torch.atan2(direction[1].item(), direction[0].item())
                        
                        # Create quaternion from yaw angle
                        proposed_quat = torch.tensor(
                            [math.cos(yaw / 2), 0.0, 0.0, math.sin(yaw / 2)],
                            device=self.device
                        )
                        
                        # Temporarily position agent's tiled camera at proposed position
                        temp_camera_pos = proposed_pos.clone()
                        temp_camera_pos[2] = self.agent_height  # Camera height above agent
                        
                        # Calculate camera orientation with pitch
                        pitch_angle = -math.pi / 6  # -30 degrees
                        half_pitch = pitch_angle / 2
                        pitch_quat = torch.tensor(
                        ...
                        temp_camera_quat = math_utils.quat_mul(
                            proposed_quat.unsqueeze(0), 
                            pitch_quat.unsqueeze(0)
                        ).squeeze(0)
                        
                        # Set tiled camera to temporary position
                        self._tiled_camera.set_world_poses(
                            positions=temp_camera_pos.unsqueeze(0),
                            orientations=temp_camera_quat.unsqueeze(0),
                            env_ids=[env_idx],
                            convention="world"
                        )
                        
                        # Update camera to get new depth readings
                        self._tiled_camera.update(dt=0.0)
                        
                        # Check if both camera object and goal are visible from proposed position
                        camera_visible, goal_visible = self._is_within_agent_fov(
                            agent_pos=proposed_pos,
                            agent_quat=proposed_quat,
                        ...
                        if camera_visible and goal_visible:
                            # Check minimum displacement from current agent position
                            if len(acceptable_positions) == 0:
                                displacement = torch.norm(proposed_pos[:2] -
                                                          agent_pos[:2])
                            else:
                                # displacement is min dist between proposed point and all acceptable poses
                                d = []
                                for pos in acceptable_positions:
                                    d.append(
                                        torch.norm(proposed_pos[:2].cpu() -
                                                   pos[:2]))
                                displacement = min(d)
                            if displacement >= min_displacement:
                                acceptable_positions.append(proposed_pos.cpu())
                                if self.verbose >= 2:
                                    print(
                                        f"Found valid point for Env {env_idx}: {proposed_pos.cpu().numpy()} | Displacement: {displacement:.2f} | Cam visible: {camera_visible}, Goal visible: {goal_visible}"
                                    )
                if self.verbose >= 2:
                    print(
                        f"Checked angle {selected_angle:.2f} rad: Within bounds: {self._is_within_bounds(env_idx, points_xy)}, Clear of obstacles: {clear_of_obstacles}"
                    )
            if len(acceptable_positions) == 0:
                if self.verbose >= 1:
                    print("-" * 50)
                    print(
                        f"Warning: No valid points found for Env {env_idx} after {attempts} attempts"
                    )
                    print("-" * 50)
                    # Fallback: stay in current position
                    for _ in range(num_points):
                        acceptable_positions.append(agent_pos.cpu())

            # Stack the acceptable positions for this environment into a tensor
            print(
                f"For Env idx {env_idx}, found {len(acceptable_positions)} valid points after {attempts} attempts"
            )
            final_positions.append(torch.stack(acceptable_positions))

        if self.verbose >= 2:
            print(f"Final positions = {final_positions}")

        # Stack all environment position tensors
        final_positions = torch.stack(final_positions)

        assert final_positions.shape == (
            len(env_ids), num_points, 3
        ), f"Expected shape ({len(env_ids)}, {num_points}, 3), got {final_positions.shape}"
        return final_positions

    def _is_within_bounds(self, env_idx, points_xy):
        """
        Check if points are within the boundary limits for a given environment.
        
        Args:
            env_idx: Environment index
            points_xy: [x, y] coordinates in local space (relative to circle center)
            
        Returns:
            bool: True if within bounds, False otherwise
        """
        # [[x_min, y_min], [x_max, y_max]]
        x_min, y_min = self.boundary_limits[0]
        x_max, y_max = self.boundary_limits[1]

        # Convert to world space by adding environment origin
        x_world = points_xy[0] + self.scene.env_origins[env_idx, 0]
        y_world = points_xy[1] + self.scene.env_origins[env_idx, 1]

        return (x_min <= x_world <= x_max) and (y_min <= y_world <= y_max)

    def _is_clear_of_obstacles(self,
                               obstacle_poses,
                               proposed_poses,
                               min_clearance: float = 0.75):
        """
        Helper function to check that proposed poses maintain a min_clearance from obstacle_poses
        """
        if len(proposed_poses) == 3:
            proposed_poses = torch.Tensor(proposed_poses).unsqueeze(0)
        for proposed_pos in proposed_poses:
            distances = torch.norm(obstacle_poses[:, :2] - proposed_pos[:2],
                                   dim=1)
            if (distances < min_clearance).any():
                return False
        return True
    
    def _generate_random_points(self, env_idx, num_points):
        """Placeholder for generating random points."""
        pass
        
    def _is_within_agent_fov(
        self,
        ...
        env_id: int,
        horizontal_fov_degrees: float = 50.0,
        vertical_fov_degrees: float = 40.0,
    ) -> Tuple[bool, bool]:
        """
        Check if camera and goal are within agent's field of view AND not occluded.
        Uses actual depth camera data to verify visibility.
        
        Args:
            agent_pos: Agent position [3] tensor
            agent_quat: Agent orientation quaternion [4] tensor (w, x, y, z)
            camera_pos: Camera position [3] tensor
            goal_pos: Goal position [3] tensor
            env_id: Environment ID for camera access
            horizontal_fov_degrees: Horizontal field of view in degrees
            vertical_fov_degrees: Vertical field of view in degrees
            
        Returns:
            Tuple[bool, bool]: (camera_visible, goal_visible) - True if visible AND not occluded
        """
        # Step 1: Check geometric FOV constraints
        agent_forward = math_utils.quat_apply(
            agent_quat.unsqueeze(0),
            torch.tensor([1.0, 0.0, 0.0], device=self.device).unsqueeze(0)
        ).squeeze(0)
        
        # Vectors from agent to camera and goal
        vector_to_camera = camera_pos - agent_pos
        vector_to_goal = goal_pos - agent_pos
        
        # Normalize
        vector_to_camera_norm = vector_to_camera / torch.norm(vector_to_camera)
        vector_to_goal_norm = vector_to_goal / torch.norm(vector_to_goal)
        
        # Horizontal angles
        dot_camera = torch.dot(agent_forward[:2], vector_to_camera_norm[:2])
        dot_goal = torch.dot(agent_forward[:2], vector_to_goal_norm[:2])
        
        angle_to_camera = torch.acos(torch.clamp(dot_camera, -1.0, 1.0))
        angle_to_goal = torch.acos(torch.clamp(dot_goal, -1.0, 1.0))
        
        half_h_fov = torch.deg2rad(torch.tensor(horizontal_fov_degrees / 2.0, device=self.device))
        camera_in_h_fov = angle_to_camera <= half_h_fov
        goal_in_h_fov = angle_to_goal <= half_h_fov
        
        # Vertical angles (pitch)
        distance_camera_xy = torch.norm(vector_to_camera[:2])
        distance_goal_xy = torch.norm(vector_to_goal[:2])
        
        pitch_to_camera = torch.atan2(vector_to_camera[2], distance_camera_xy)
        pitch_to_goal = torch.atan2(vector_to_goal[2], distance_goal_xy)
        
        half_v_fov = torch.deg2rad(torch.tensor(vertical_fov_degrees / 2.0, device=self.device))
        camera_in_v_fov = torch.abs(pitch_to_camera) <= half_v_fov
        goal_in_v_fov = torch.abs(pitch_to_goal) <= half_v_fov
        
        # Geometric FOV checks
        camera_in_geometric_fov = camera_in_h_fov and camera_in_v_fov
        goal_in_geometric_fov = goal_in_h_fov and goal_in_v_fov
        
        # Step 2: Check occlusion using depth camera
        # Only check occlusion if object is geometrically within FOV
        camera_visible = False
        goal_visible = False
        
        if camera_in_geometric_fov:
            # Check if camera object is occluded using tiled camera
            camera_occluded = self._check_occlusion_raycast(agent_pos, camera_pos, env_id, camera=self._tiled_camera)
            camera_visible = not camera_occluded
        
        if goal_in_geometric_fov:
            # Check if goal is occluded using tiled camera
            goal_occluded = self._check_occlusion_raycast(agent_pos, goal_pos, env_id, camera=self._tiled_camera)
            goal_visible = not goal_occluded
        
        if self.verbose >= 2:
            print(f"Env {env_id}:")
            print(f"  Camera: H_angle={torch.rad2deg(angle_to_camera):.2f}°, pitch={torch.rad2deg(pitch_to_camera):.2f}°")
            print(f"    Geometric FOV: {camera_in_geometric_fov}, Visible: {camera_visible}")
            print(f"  Goal: H_angle={torch.rad2deg(angle_to_goal):.2f}°, pitch={torch.rad2deg(pitch_to_goal):.2f}°")
            print(f"    Geometric FOV: {goal_in_geometric_fov}, Visible: {goal_visible}")
        
        return camera_visible, goal_visible
...
def point_on_circle(
    ...
    """
    Gets (x, y) for a point on the circle at the given angle.
    """
...
def compute_circle_radius_from_fov(
    camera_to_target_distance: torch.Tensor,
    horizontal_fov_degrees: float = 34.0,
    coverage_fraction: float = 1.2,
) -> torch.Tensor:
    """
    Calculate circle radius for agent movement around target.
    
    The radius is calculated so that when the agent moves on the circle,
    the target remains within the camera's field of view.
    
    Args:
        camera_to_target_distance: Distance from camera to target [num_envs] or scalar
        horizontal_fov_degrees: Horizontal field of view in degrees (default: 34.0)
        coverage_fraction: Fraction of FOV width to use (0.8 = keep target in center 80%)
    
    Returns:
        Circle radius for each environment [num_envs] or scalar
        
    Example:
        At distance=10m with 34° FOV:
        - Full FOV width at target = 2 * 10 * tan(17°) = 6.1m
        - With 80% coverage, max offset = 2.44m
        - Circle radius ≈ 2.5m
    """
    # Convert FOV to radians
    half_fov_rad = torch.deg2rad(torch.tensor(horizontal_fov_degrees / 2.0))

    # Calculate how far off-center the agent can be while keeping target in view
    # tan(half_fov) = (half_width_at_target) / distance
    max_offset = camera_to_target_distance * torch.tan(
        half_fov_rad) * coverage_fraction

    # Radius is the maximum offset from center
    radius = max_offset
