...
from .vpt_env_cfg_v8 import VPTEnvCfg
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # File paths
        self.base_path = f"/home/arock3/Documents/data_v9/data_{self.GPU_ID}"
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        ...
        while self.next_env_id < self.total_envs_to_sim or len(self.completed_envs) < self.total_envs_to_sim:
            ...
            self.randomize_shape_scale(prim_path_expr=[f"/World/envs/env_.*/obs_{idx}" for idx in range(self.cfg.num_vpt_objs)])
            print(f"Recoloring VPT obstacles for {self.cfg.num_vpt_objs} objects...")
            self.randomize_shape_color(prim_path_expr=[f"/World/envs/env_.*/obs_{idx}" for idx in range(self.cfg.num_vpt_objs)])
    ...
    def randomize_shape_color(self, prim_path_expr: str|list):
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                rand_color = Gf.Vec3f(random.random(), random.random(), random.random())

                # --- STRATEGY 1: FAST PATH (Standard Assets) ---
                # Try the standard Isaac Lab path first. It's faster than searching.
                # We check the root prim spec first.
                prim_spec = Sdf.CreatePrimInLayer(stage.GetRootLayer(), prim_path)
                color_spec = prim_spec.GetAttributeAtPath(prim_path + "/geometry/material/Shader.inputs:diffuseColor")
                
                if color_spec:
                    color_spec.default = rand_color
                    continue 

                # --- STRATEGY 2: DYNAMIC SEARCH (Custom Assets) ---
                # If the fast path failed, we search the stage for the shader.
                
                # 1. Get the composed Prim from the stage (allows us to see children)
                prim = stage.GetPrimAtPath(prim_path)
                
                if not prim.IsValid():
                    continue

                # 2. Walk all descendants to find the FIRST Shader
                ...
                # 3. If we found a shader, modify it via Sdf
                if target_shader_path:
                    # Create the override for the specific shader path found
                    shader_spec = Sdf.CreatePrimInLayer(stage.GetRootLayer(), target_shader_path)
                    
                    # Check common attribute names (UsdPreviewSurface vs OmniPBR)
                    attr_name = "inputs:diffuseColor"
                    
                    # Verify if specific attribute exists, if not, you might need 'inputs:diffuse_color_constant'
                    # But usually creating it as diffuseColor works for standard preview surfaces.
                    color_spec = shader_spec.GetAttributeAtPath(target_shader_path + "." + attr_name)
                    
                    if not color_spec:
                        color_spec = Sdf.AttributeSpec(shader_spec, attr_name, Sdf.ValueTypeNames.Color3f)
    ...
    def randomize_global_light(self, prim_path="/World/Light"):
        ...
        prim.GetAttribute("inputs:angle").Set(0.53)
