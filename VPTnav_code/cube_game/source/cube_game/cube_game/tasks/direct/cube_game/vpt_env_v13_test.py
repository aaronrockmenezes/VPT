...
from .vpt_env_cfg_v13 import VPTEnvCfg
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # File paths
        self.base_path = f"/home/arock3/data_v13_latest_fixed/data_{self.GPU_ID}"
    ...
    def initial_spawn_loop(self,
        ...
        # Subset of origins for the environments we are retrying
        ...
        for _ in range(max_dist_retries):
            ...
            bad_local_indices = torch.where(bad_mask)[0]
            ...
            global_bad_indices = retry_indices[bad_local_indices]
            
            goal_default_state[global_bad_indices, 0] = env_origins[bad_local_indices, 0] + new_goal_offsets[:, 0]
            goal_default_state[global_bad_indices, 1] = env_origins[bad_local_indices, 1] + new_goal_offsets[:, 1]
            
            camera_obj_default_state[global_bad_indices, 0] = env_origins[bad_local_indices, 0] + new_cam_offsets[:, 0]
            camera_obj_default_state[global_bad_indices, 1] = env_origins[bad_local_indices, 1] + new_cam_offsets[:, 1]
        ...
        for batch_idx, env_idx in enumerate(retry_indices):
            ...
            # 1. Get Global Positions
            cam_global_x = camera_obj_default_state[env_idx, 0].item()
            cam_global_y = camera_obj_default_state[env_idx, 1].item()
            goal_global_x = goal_default_state[env_idx, 0].item()
            goal_global_y = goal_default_state[env_idx, 1].item()

            # 2. Convert to Local Positions (Relative to Env Origin)
            # We need this because the polygon is created using local 'rx, ry'
            ...
            active_indices = self.active_vpt_indices[env_id_item]
            active_dims = self.all_vpt_dims[env_id, active_indices, :3]
            ...
            for k, obj_idx in enumerate(active_indices):
                ...
                for _ in range(MAX_ATTEMPTS):
                    ...
                    # This polygon is in LOCAL coordinates
                    ...
                    if not overlap:
                        ...
                        # Calculate Global Final Position
                        cand_global_x = origin_x + rx
                        cand_global_y = origin_y + ry

                        vpt_obj_default_state[env_idx, obj_idx, 0] = cand_global_x
                        vpt_obj_default_state[env_idx, obj_idx, 1] = cand_global_y
                        vpt_obj_default_state[env_idx, obj_idx, 2] = obj_h * z_ratio
                        
                        r_yaw_tensor = torch.tensor(r_yaw, device=device)
                        zero = torch.tensor(0.0, device=device)
                        quat = quat_from_euler_xyz(zero, zero, r_yaw_tensor)
            ...
            if placement_failed:
                fail_ids.append(env_id_item)
                continue
            else:
                envs_need_spawn_retry[env_idx] = False
                success_ids.append(env_id_item)
    ...
    def moving_ball_loop(self,
        ...
        new_goal_z = target_env_origins[:, 2] + selected_heights + self.goal_radius + 0.001
        ...
        print(f"Env ID = {target_env_ids.cpu().numpy()}")
        print(f"Goal Z = {new_goal_z}")
        print(f"VPT Object Z = {new_obj_z} | Heights = {selected_heights} | Ratios = {selected_ratios}")
        print(f"Target env origins Z = {target_env_origins[:, 2]}")
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                if is_random:
                    ...
                    # --- COMPUTE BOUNDS ---
                    bbox_cache.Clear()
                    # Get Local Bound of RAW geometry (unscaled)
                    local_bound = bbox_cache.ComputeLocalBound(root_prim).GetRange()
                    
                    raw_w = local_bound.GetMax()[0] - local_bound.GetMin()[0]
                    raw_l = local_bound.GetMax()[1] - local_bound.GetMin()[1]
                    raw_h = local_bound.GetMax()[2] - local_bound.GetMin()[2]

                    # 1. Update standard dims (Width, Length, Height) SCALED
                    # Multiply raw dims by the scale vector we just applied
                    self.all_vpt_dims[env_idx, obj_idx, 0] = raw_w * final_scale_vec[0]
                    self.all_vpt_dims[env_idx, obj_idx, 1] = raw_l * final_scale_vec[1]
                    self.all_vpt_dims[env_idx, obj_idx, 2] = raw_h * final_scale_vec[2]
                    print(f"Obj {obj_idx} in Env {env_idx} - Dims (W,L,H): {self.all_vpt_dims[env_idx, obj_idx].cpu().numpy()} | Raw (W,L,H): {[raw_w, raw_l, raw_h]} | Scale: {final_scale_vec}")

                    # 2. Update Bounding Box [min_x, min_y, max_x, max_y] for Placement
                    # Bounding Box Min/Max is relative to the pivot (0,0)
                    # We usually want the raw bound scaled, assuming pivot is consistent
