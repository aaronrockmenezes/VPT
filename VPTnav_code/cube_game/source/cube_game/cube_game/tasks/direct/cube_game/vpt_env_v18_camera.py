...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: VPTEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.images_per_env = len(self.camera_sweep_angles_deg)  # 13
        ...
        base = os.getenv("BASE_PATH", "/oscar/scratch/arock3/VPT1_DATA/camera/v18_camera")
    ...
    def _preallocate_visibility_labels(self) -> None:
        ...
        if self.verbose >= 1:
            ...
            print(f"   - images per env: {self.images_per_env}")
    ...
    def _validate_env_state(self, env_id: torch.Tensor, folder_idx: int, min_viewpoints: int) -> tuple[bool, str]:
        ...
        # TODO: enforce global 50/50 in_view/occluded balance across dataset at collection time.
        # Currently collecting all safe frames regardless of per-env label distribution.
    ...
    def _build_fixed_sweep_trajectory_batch(self, eids: list[int]) -> dict[int, bool]:
        ...
        for angle_deg in self.camera_sweep_angles_deg:
            ...
            # Per-slot: read visibility + cache image tensors
            for j in range(len(place_idx)):
                if not bool(av_valid[j].item()):
                    continue
                ...
                cached_frames[eid].append({
                    **frame,
                    ...
                    "_depth":       self._rgb_tiled_camera.data.output["distance_to_camera"][eid].clone(),
                    "_semantic":    self._rgb_tiled_camera.data.output["semantic_segmentation"][eid].clone(),
                    "_cam_pov_rgb": (self._occlusion_camera.data.output["rgb"][eid].clone()
        ...
        for eid in eids:
            ...
            if traj:
                ...
                self._cached_frames[eid] = cached_frames[eid]
                success[eid] = True
            else:
                self.camera_move_trajectories[eid] = None
                success[eid] = False
    ...
    def _save_slot_from_cache(self, env_slot: int, folder_idx: int) -> None:
        ...
        for frame in frames:
            self._save_camera_move_frame(
                ...
                cam_pov_rgb=frame.get("_cam_pov_rgb"),
    ...
    def _save_camera_move_frame(
        ...
        self._save_single_image(
            ...
            camera_pov_rgb_data=cam_pov_rgb,
            angle_deg=angle_deg,
    ...
    def _save_single_image(self,
        ...
        """
        Save RGB, Depth, Semantic (agent POV) and cam POV (occlusion camera) for one sweep frame.
        ...
        Agent POV  -> RGB/ Depth/ Semantic/
        Camera POV -> cam_Semantic/ cam_RGB/ cam_RGB_norm/   (own dirs, not one flat cam/)
        """
        # --- 1. Path Setup ---
        ...
        if self.save_camera_pov:
            ...
            os.makedirs(base_dir.format("cam_RGB"), exist_ok=True)
            os.makedirs(base_dir.format("cam_RGB_norm"), exist_ok=True)
        ...
        cv2.imwrite(f"{depth_dir}/{fname}", depth_norm)

        # --- 4. Save Semantic (agent POV) ---
        ...
        # --- 5. Save Camera POV (occlusion camera) at every sweep angle ---
        # Each cam image type gets its own dir, mirroring the agent-POV layout.
        if self.save_camera_pov and camera_pov_data is not None:
            ...
            cam_rgb_norm_dir = base_dir.format("cam_RGB_norm")
            ...
            # RGB from occlusion camera
            if camera_pov_rgb_data is not None:
                cam_pov_rgb_np = camera_pov_rgb_data.cpu().numpy()
                if cam_pov_rgb_np.dtype != np.uint8:
                    cam_pov_rgb_np = (cam_pov_rgb_np * 255.0).astype(np.uint8) if cam_pov_rgb_np.max() <= 1.0 else cam_pov_rgb_np.astype(np.uint8)
                if cam_pov_rgb_np.shape[-1] == 4:
                    cam_pov_rgb_np = cam_pov_rgb_np[..., :3]
                cv2.imwrite(f"{cam_rgb_dir}/{fname}", cv2.cvtColor(cam_pov_rgb_np, cv2.COLOR_RGB2BGR))

                cam_pov_rgb_float = cam_pov_rgb_np.astype(np.float32)
                mn, mx = cam_pov_rgb_float.min(), cam_pov_rgb_float.max()
                if mx > mn:
                    cam_pov_rgb_norm = ((cam_pov_rgb_float - mn) / (mx - mn) * 255).astype(np.uint8)
                else:
                    cam_pov_rgb_norm = np.zeros_like(cam_pov_rgb_np)
                cv2.imwrite(f"{cam_rgb_norm_dir}/{fname}", cv2.cvtColor(cam_pov_rgb_norm, cv2.COLOR_RGB2BGR))
