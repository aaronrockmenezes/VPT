...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.verbose = 2
        # Both to 50 in v2 and v1 | 100, 500 in v3
        self.goal_pixel_threshold = 100  # Minimum pixels for goal visibility
        self.goal_pixel_threshold_occlusion = 500 # Higher threshold for occlusion check to reduce false positives
        ...
        # File paths
        self.base_path = f"/home/arock3/data_v15_17_high_v3/data_{self.GPU_ID}"
    ...
    def _setup_scene(self):
        ...
        self.mat_material_configs = self.mat_material_configs[:len(
            self.mat_material_configs) // 2]
        random.shuffle(self.vpt_material_configs)
        self.vpt_material_configs = self.vpt_material_configs[:len(
            self.vpt_material_configs) // 2]
    ...
    def _check_occlusion_raycast(self,
        ...
        GOAL_THRESHOLD = self.goal_pixel_threshold_occlusion
    ...
    def _check_target_in_img(self,
        ...
        target_visible_in_camera = red_count >= self.goal_pixel_threshold_occlusion
    ...
    def _reset_idx(self,
        ...
        for env_idx in range(self.num_envs):
            for obj_idx in range(self.cfg.num_vpt_objs):
                vpt_mat_prim_paths.append(
                    f"/World/envs/env_{env_idx}/obs_{obj_idx}")
        ...
        while self.next_env_id < self.total_envs_to_sim or len(
            ...
            self.randomize_mat_material(prim_paths=[
                f"/World/envs/env_{i}/mat" for i in range(self.num_envs)
            ])
        ...
        for batch_idx, local_idx in enumerate(retry_indices):
            ...
            for k, obj_idx in enumerate(active_indices):
                ...
                for _ in range(MAX_ATTEMPTS):
                    ...
                    if (minx < -safe_x_range_obstacles
                            or miny < -safe_x_range_obstacles
                            or maxx > safe_x_range_obstacles
                            or maxy > safe_x_range_obstacles):
                        continue
    ...
    def initial_spawn_loop_mbc(self,
        ...
        agent_offsets = sample_uniform(-safe_x_range, safe_x_range,
                                       (batch_size, 2), device)
        goal_perturb_offsets = sample_uniform(-0.4, 0.4, (batch_size, 2),
                                              device)
            ...
            for k, obj_idx in enumerate(active_indices):
                ...
                for _ in range(MAX_ATTEMPTS):
                    ...
                    for _ in range(NUM_CANDIDATES):
                        raw_rx = (random.random() * 2 * safe_x_range_obstacles
                                  ) - safe_x_range_obstacles
                        raw_ry = (random.random() * 2 * safe_x_range_obstacles
                                  ) - safe_x_range_obstacles
    ...
    def vpt_in_out_fov_movement(self,
                                env_ids,
                                valid_indices,
                                visibility_categories,
                                in_view_displaced,
                                outside_fov_displaced,
                                moved_vpt_for_ball,
                                states,
                                device=None):
            ...
            if visibility_category == "in_view":
                ...
                if not camera_validation_passed:
                    if self.verbose >= 1:
                        ...
                        print(
                            f"       Expected: in_view (target visible), Got: target NOT visible (red pixels: {red_count}/{self.goal_pixel_threshold_occlusion})"
                        )
                ...
                if not camera_validation_passed:
                    if self.verbose >= 1:
                        ...
                        print(
                            f"       Expected: occluded (target NOT visible), Got: target visible (red pixels: {red_count}/{self.goal_pixel_threshold_occlusion})"
                        )
                        print(f"       Debug image saved: {debug_filename}")
                else:
                    if self.verbose >= 2:
                ...
                if not camera_validation_passed:
                    if self.verbose >= 1:
                        ...
                        print(
                            f"       Expected: outside_fov (target NOT visible), Got: target visible (red pixels: {red_count}/{self.goal_pixel_threshold_occlusion})"
                        )
                        print(f"       Debug image saved: {debug_filename}")
                else:
                    if self.verbose >= 2:
    ...
    def _reset_idx_internal(self,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            for local_idx, env_idx in enumerate(valid_indices):
                ...
                cam_z_ok = (camera_new_pos[local_idx, 2]
                            >= 0.0) and (camera_new_pos[local_idx, 2] <= 1.0)
                agent_z_ok = (agent_new_pos[local_idx, 2]
                              >= 0.0) and (agent_new_pos[local_idx, 2] <= 1.0)

                # VPT check: Get Z slice for this env
                # Allow -0.001 to 0.101
                current_vpt_z = vpt_new_pos[local_idx, :, 2]
                vpt_z_ok = torch.all((current_vpt_z >= -tol)
                                     & (current_vpt_z <= (0.1 + tol)))
                ...
                if not z_valid:
                    ...
                    if not goal_z_ok:
                        print(
                            f"   ❌ Goal Z out of bounds (Target 0.0-1.0 +/- {tol}): {goal_val.item():.6f}"
                        )

                    if not cam_z_ok:
                        print(
                            f"   ❌ Camera Z out of bounds: {camera_new_pos[local_idx, 2].item():.4f}"
                        )

                    if not agent_z_ok:
                        print(
                            f"   ❌ Agent Z out of bounds: {agent_new_pos[local_idx, 2].item():.4f}"
                        )

                    if not vpt_z_ok:
                        # Find exactly which objects are failing the tolerance check
                        failed_mask = (current_vpt_z < -tol) | (current_vpt_z
                                                                > (0.1 + tol))
                        failed_indices = torch.where(failed_mask)[0]
                        failed_values = current_vpt_z[failed_mask]

                        print(
                            f"   ❌ VPT Object Zs out of bounds (Target 0.0-0.1 +/- {tol}):"
                        )
                        for i, idx in enumerate(failed_indices):
                            print(
                                f"      - Obj Local Index {idx.item()}: Z={failed_values[i].item():.6f}"
                            )
        ...
        for i, env_id in enumerate(env_ids):
            ...
            safe_z_values = self.vpt_obj_default_state[env_id, active_indices,
                                                       2]
    ...
    def _cache_z_offsets(self):
        ...
        for key, obj_cfg in self.cfg.vpt_objects.rigid_objects.items():
            ...
            # Case 2: USD Files (Check filename for X, L, T)
                ...
                if filename.endswith(('X', 'L', 'T', 'A', 'H', 'I', 'Z',
                                      'Table_A', 'Table_B', 'Bench')):
                    offsets.append(0.0)
                else:
                    # print("0.5 used")
    ...
    def randomize_shape_scale(self,
        ...
        if not hasattr(
                self, "vpt_z_offset_ratios"
        ) or self.vpt_z_offset_ratios.shape[0] != self.cfg.num_vpt_objs:
            self.vpt_z_offset_ratios = torch.zeros((self.cfg.num_vpt_objs),
                                                   device=self.device)
                    ...
                    if isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                        ...
                        if filename.endswith(
                            ...
                            if filename.endswith(("X")):
                                ...
                                s_xz = random.uniform(0.8, 3.0)
                                s_y = random.uniform(1, 5.0)
                                final_scale_vec = Gf.Vec3d(
                                    base_dim[0] * s_xz, base_dim[1] * s_y,
                                    base_dim[2] * s_xz)
                        ...
                        z_offset_multiplier = 0.5
                        if isinstance(
                            spawn_cfg, sim_utils.MeshConeCfg):
                            z_offset_multiplier = 0.0

                        s_r = random.uniform(0.75, 1.5)
    ...
    def randomize_global_lights(
            self,
            prim_paths=["/World/Light_A", "/World/Light_B"],
            random_light_off=False):
        """
        ...
        for path in prim_paths:
            ...
            for _ in range(max_retries):
                candidate_azimuth = random.uniform(
                    0, 360)  # Covers East (Morning) and West (Evening)
                collision = False
    ...
    def _get_dists_to_active_vpt_surface(self,
                                         env_ids,
                                         reference_points,
                                         device=None):
        """
        ...
        vpt_poses = self._get_active_vpt_positions(env_ids,
                                                   base_pivoted=False,
    ...
    def get_all_mdls_from_nucleus_path(self, path: str) -> list[str]:
        ...
        for entry in entries:
            ...
            if entry.flags & omni.client.ItemFlags.CAN_HAVE_CHILDREN:
                ...
                mdl_files.extend(
                    self.get_all_mdls_from_nucleus_path(full_path))
            else:
    ...
    def get_mat_material_configs(self) -> list[sim_utils.MdlFileCfg]:
        ...
        mat_materials_paths = random.sample(mat_materials_paths,
                                            num_mat_materials)

        print(
            f"[INFO] Final count: {len(mat_materials_paths)} materials selected."
        )
    ...
    def get_vpt_material_configs(self) -> list[sim_utils.MdlFileCfg]:
        ...
        vpt_materials_paths = random.sample(vpt_materials_paths,
                                            num_vpt_materials)

        print(
            f"[INFO] Final count: {len(vpt_materials_paths)} materials selected."
        )
