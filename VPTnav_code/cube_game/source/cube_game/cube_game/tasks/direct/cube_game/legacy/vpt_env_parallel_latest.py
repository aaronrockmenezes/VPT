...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.camera_pixel_threshold = 1400  # Minimum pixels for camera visibility
        ...
        self.base_path = "/media/data_cifs_lrs/projects/prj_robotics/VPTnav_v2b_Yes"
        self.visibility_labels_json_path = f"{self.base_path}/visibility_labels.json"
        self._reset_called = False
        self.max_rgb_images = 8_000
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            for batch_idx, env_idx in enumerate(retry_indices):
                ...
                if camera_goal_distance > 10.0 or camera_goal_distance < 1.0:
                    continue
            ...
            for local_idx, env_idx in enumerate(valid_indices):
                ...
                if distance_cam_to_goal > 1e-6:
                    ...
                    vpt_pos = vpt_obj_default_state[env_idx, tallest_obj_idx, :3]
                    # print(f"Occlusion: Old pos = {vpt_pos[:2].cpu().numpy()}, New pos = {new_pos.cpu().numpy()}")
                    
                    vpt_obj_default_state[env_idx, tallest_obj_idx, 0] = new_pos[0]
                    vpt_obj_default_state[env_idx, tallest_obj_idx, 1] = new_pos[1]
                    vpt_obj_default_state[env_idx, tallest_obj_idx, 2] = env_origin[2]
            ...
            for local_idx, env_idx in enumerate(valid_indices):
                ...
                # vpt_heights = vpt_obj_default_state[env_idx, :, 2]
                # bottom_3_indices = torch.topk(vpt_heights, k=4, largest=False).indices
                ...
                if distance_cam_to_goal > 1e-6:
                    ...
                    t = random.uniform(0.2, 0.5)  # Interpolation factor
                    new_pos = camera_pos[:2] + direction_cam_to_goal * (distance_cam_to_goal * t)
    ...
    def _is_point_valid_batch(self, points: torch.Tensor, env_ids: torch.Tensor, 
                              ...
                              min_camera_obstacle_distance: float = 0.1,
        ...
        # Group points by environment and randomly sample up to 50 per env
        ...
        for point_idx in range(max_points_per_env):
            ...
            # Collect environments and points for this iteration
            batch_env_ids = []
            batch_points = []
    ...
    def generate_valid_circle_points(self, env_ids: torch.Tensor, angle_step: float = 2.0,
        ...
        # Use a fixed FOV of 36 degrees (in radians)
        fov_deg = 30.0
        fov_rad = math.radians(fov_deg)
        camera_pos = self._camera_obj.data.root_pos_w[env_ids, :2]
        goal_pos = self._goal.data.root_pos_w[env_ids, :2]
        d = torch.norm(camera_pos - goal_pos, dim=1)  # shape: (num_envs,)

        half_fov = torch.tensor(fov_rad / 2, device=self.device)
        radii = (d / 2) / torch.tan(half_fov)
        radii = radii * 1.2  # add 20% margin
        radii = radii.unsqueeze(1)  # shape: (num_envs, 1)
