...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # File paths
        self.base_path = f"/home/arock3/data_v10c_test/data_{self.GPU_ID}"
    ...
    def _setup_scene(self):
        ...
        for idx in range(0, 32):
            self.check_usd_pivot_alignment(prim_path_expr=f"/World/envs/env_0/obs_{idx}")
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            # ========== CHANGE #4: Loop over active_indices with Collision Checks ==========
            self._cache_z_offsets()
            
            # We iterate over the environments that need a retry
            for batch_idx, env_idx in enumerate(retry_indices):
                ...
                # Get dimensions for these active objects: [N, 3] -> (width, length, height)
                # We only care about x (width) and y (length) for overlap
                active_dims = self.all_vpt_dims[env_idx, active_indices, :2] 
                
                placed_pos = []     # List to store [x, y] of successfully placed objects
                placed_dims = []    # List to store [w, l] of successfully placed objects
                placement_failed = False
                
                margin = 0.05 # Minimum gap between objects
                ...
                for i, obj_idx in enumerate(active_indices):
                    ...
                    # Try to place this object up to 20 times
                    for _ in range(20):
                        # 1. Sample a random candidate position (Local Offset)
                        ...
                        # 2. Check collision against all previously placed objects
                        collision = False
                        for p_idx, (px, py) in enumerate(placed_pos):
                            pw, pl = placed_dims[p_idx]
                            
                            # AABB Collision Logic:
                            # Distance X < (Width1 + Width2)/2 + margin
                            # Distance Y < (Len1 + Len2)/2 + margin
                            dist_x = abs(rx - px)
                            dist_y = abs(ry - py)
                            
                            min_dist_x = (curr_w + pw) / 2.0 + margin
                            min_dist_y = (curr_l + pl) / 2.0 + margin
                    ...
                    if not found_valid_pos:
                        ...
                        break # Stop trying to place objects for this env
                
                # If we couldn't place all objects without overlap, 
                # we skip the rest of the checks for this env.
                # envs_need_spawn_retry[env_idx] is ALREADY True, so we just continue.
                # The outer loop (spawn_attempt) will try again from scratch next time.
                if placement_failed:
                    # Optional: Print debug info
                    # print(f"  ⚠️ Env {env_id_item}: Failed to place objects without overlap. Retrying...")
                    continue 
            ...
            # print(vpt_obj_default_state[valid_indices, :, :3])
            ...
            if isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                ...
                if filename.endswith(('X', 'L', 'T')):
                    ...
                    if filename.endswith(("X")):
                        dims = torch.tensor([1.0 * scale[0], 0.25 * scale[1], 1.0 * scale[2]])
    ...
    def _cache_z_offsets(self):
        ...
        for key, obj_cfg in self.cfg.vpt_objects.rigid_objects.items():
            ...
            if isinstance(spawn_cfg, (sim_utils.CuboidCfg, sim_utils.ConeCfg, sim_utils.CylinderCfg)):
                offsets.append(0.5)
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                for op in xform.GetOrderedXformOps():
                    if op.GetOpType() == UsdGeom.XformOp.TypeScale: scale_op = op
                    elif op.GetOpType() == UsdGeom.XformOp.TypeTranslate: translate_op = op
                
                if scale_op is None: scale_op = xform.AddScaleOp(UsdGeom.XformOp.PrecisionDouble)
                if translate_op is None: translate_op = xform.AddTranslateOp(UsdGeom.XformOp.PrecisionDouble)
                ...
                if is_random:
                    ...
                    if isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                        ...
                        if filename.endswith(('X', 'L', 'T')):
                            ...
                            final_z_pos = final_scale_vec[2] * z_offset_multiplier
                    ...
                    # Update Bounds Tensor
                    # We still run this to get the accurate 'visual' size for your observation/reward tensors
                    bbox_cache.Clear()
                    new_bound = bbox_cache.ComputeLocalBound(root_prim).GetRange()
                    new_w = new_bound.GetMax()[0] - new_bound.GetMin()[0]
                    new_l = new_bound.GetMax()[1] - new_bound.GetMin()[1]
                    new_h = new_bound.GetMax()[2] - new_bound.GetMin()[2]
    ...
    def randomize_sphere_lights(self, light_names: list = ["Light_A"], z_heights: list = [None]):
        """
        ...
        with Sdf.ChangeBlock(): # Batch updates for performance
            for i in range(self.num_envs):
                ...
                active_light_names = set(random.sample(light_names, num_active))
                ...
                for light_name, z_req in zip(light_names, z_heights):
                    ...
                    if light_name in active_light_names:
                        ...
                        translate_op.Set(Gf.Vec3d(float(rand_x), float(rand_y), final_z))
                        ...
                        prim.GetAttribute("inputs:color").Set(Gf.Vec3d(r, g, b))
    ...
    def check_usd_pivot_alignment(self, prim_path_expr: str):
        ...
        if abs(z_min) < 0.05 * height:
            ...
            print(f"   Placement Logic: Z = Floor_Z + 0.0 (Do NOT add half-height)")
