...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # File paths
        self.base_path = f"/home/arock3/data_v12_new/data_{self.GPU_ID}"
    ...
    def initial_spawn_loop(self,
        ...
        for batch_idx, env_idx in enumerate(retry_indices):
            ...
            placed_pos = [
            ]  # List to store [x, y] of successfully placed objects
            placed_dims = [
            ]  # List to store [w, l] of successfully placed objects
            ...
            for i, obj_idx in enumerate(active_indices):
                ...
                for _ in range(20):
                    ...
                    if not collision:
                        ...
                        # Assign to Tensor
                        vpt_obj_default_state[env_idx, obj_idx,
                                              0] = env_origins[batch_idx,
                                                               0] + rx
                        vpt_obj_default_state[env_idx, obj_idx,
                                              1] = env_origins[batch_idx,
                                                               1] + ry
                        vpt_obj_default_state[env_idx, obj_idx,
                                              2] = self.vpt_obj_default_state[
                                                  env_idx, obj_idx, 2]
                        break
        ...
        for batch_idx, env_idx in enumerate(retry_indices):
            ...
            camera_distances_from_vpt = torch.norm(camera_pos.unsqueeze(0) -
                                                   vpt_positions,
                                                   dim=1)
            
            # camera_distances_from_vpt = self._get_dists_to_active_vpt_surface(
            #     env_ids=env_idx,
            #     reference_points=[camera_obj_default_state[env_idx, :3]],
            #     )
            # print(camera_distances_from_vpt)
            if not torch.all(camera_distances_from_vpt >= 3.0):
                continue

            envs_need_spawn_retry[env_idx] = False
            print(f"  ✅ Env {env_ids[env_idx].item()} passed")
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                if is_random:
                    ...
                    bbox_cache.Clear()
                    new_bound = bbox_cache.ComputeLocalBound(
                        root_prim).GetRange()
