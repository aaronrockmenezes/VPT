...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.mode = "testing" if (self.config_file and os.path.exists(
            self.config_file)) else "data_collection"
        ...
        print("*" * 50)
        print(
            f"🚀 Initializing VPTEnv on Node {self.NODE_ID} GPU {self.GPU_ID}..."
        )
        print("*" * 50)

        self.visibility_labels_json_path = os.path.join(
            self.base_path, "visibility_labels.json")

        # --- Precomputed Math ---
        # 7.5 deg per turn action (matches v17_alekh.py).
        # heading_bin_rad and num_heading_bins below auto-derive.
        ...
        self.camera_yaw_correction_rad = math.radians(
            self.camera_yaw_correction_deg)
        self.heading_bin_rad = self.theta  # pi/24 == 7.5 degrees
        self.num_heading_bins = int(
            round((2.0 * math.pi) / self.heading_bin_rad))
        # Single source of truth for the forward step.
        # Sim physics: agent_speed (1.5 m/s) * dt (0.1 s) = 0.15 m/action.
        # Both move_agent and the A* planner read from this constant so
        # they stay locked together.
        self.agent_speed = 1.5
        self.forward_step_m = self.agent_speed * self.cfg.sim.dt  # = 0.15 m
        self.grid_step_m = self.forward_step_m
        # Finer yaw bins (48 with 7.5 deg steps) double the per-cell
        # branching factor versus 15 deg bins, so expansion budget bumped.
        # State space: 134x134 grid * 48 headings = 862k nodes.
        # 250k = 29% coverage -- enough for most configs but not all.
        # The inflation-fallback retries use a higher budget (500k)
        # because they only fire when the default attempt already failed.
        self.astar_max_expansions = 250000
        self.astar_max_expansions_fallback = 500000
        # Inflation must be >= agent half-extent (0.1m) to prevent the
        # planner from routing the agent center into positions where its
        # collision body overlaps an object (causing sim collision
        # reverts). 0.08m was too small -- it allowed center-to-AABB
        # distances of 0.08-0.1m where the 0.2x0.2m body still collides.
        # 0.12m = 0.1m (half-extent) + 0.02m margin. Avoids collision
        # reverts while still leaving narrow passages open (0.15m was
        # over-conservative).
        self.planner_inflation_m = 0.12
    ...
    def _preallocate_visibility_labels(self) -> None:
        ...
        if hasattr(self,
                   'visibility_label_pool') and self.visibility_label_pool:
            return
    ...
    def _setup_scene(self):
        ...
        # --- Lighting ---
        light_cfg = sim_utils.SphereLightCfg(intensity=1000.0,
    ...
    def move_agent(self,
        ...
        if moving_mask.any():
            ...
            world_vel = math_utils.quat_apply(current_quat[moving_mask],
                                              local_move)

            # Apply displacement (Speed * dt * 2.0 multiplier)
            tentative_pos[moving_mask] += (world_vel * self.agent_speed * dt)

        # Enforce fixed height constraint
        tentative_pos[:, 2] = self._agent.data.default_root_state[phys_ids, 2]

        # --- 6. Collision Checking ---
        # Check collisions if we proposed ANY change (pos or quat)
        changed_mask = moving_mask | mask_left | mask_right
        if changed_mask.any():
            ...
            if is_collision.any():
                # Only revert those that were actually trying to move or turn
                revert_mask = is_collision & changed_mask
                if revert_mask.any():
                    revert_indices = torch.where(revert_mask)[0]
        ...
        zero_vel = torch.zeros(len(phys_ids), 6, device=self.device)
        self._agent.write_root_com_velocity_to_sim(zero_vel, phys_ids)
        self._agent.reset()
    ...
    def _yaw_to_heading_bin(self, yaw: float) -> int:
        idx = int(round(self._normalize_angle(yaw) /
                        self.heading_bin_rad)) % self.num_heading_bins
        return idx

    def _yaw_to_quat_wxyz(self, yaw: float) -> torch.Tensor:
        half = yaw / 2.0
        return torch.tensor([math.cos(half), 0.0, 0.0,
                             math.sin(half)],
                            device=self.device,
                            dtype=torch.float32)

    def _world_to_grid(self, xy: torch.Tensor, origin_xy: torch.Tensor,
                       min_xy: torch.Tensor) -> tuple[int, int]:
        rel = (xy[:2] - origin_xy[:2]) - min_xy
        gx = int(round(float(rel[0] / self.grid_step_m)))
        gy = int(round(float(rel[1] / self.grid_step_m)))
        return gx, gy

    def _grid_to_world(self, gx: int, gy: int, origin_xy: torch.Tensor,
                       min_xy: torch.Tensor) -> torch.Tensor:
    ...
    def _forward_grid_transition(
        ...
        yaw = self._heading_bin_to_yaw(heading_bin)
        world_x = origin_x + min_x + gx * self.grid_step_m
        world_y = origin_y + min_y + gy * self.grid_step_m
        world_x += math.cos(yaw) * self.forward_step_m
        world_y += math.sin(yaw) * self.forward_step_m
        nx = int(round((world_x - origin_x - min_x) / self.grid_step_m))
        ny = int(round((world_y - origin_y - min_y) / self.grid_step_m))
    ...
    def build_nav_grid(self,
                       env_id: int,
                       inflation_radius: float | None = None) -> dict:
        ...
        if hasattr(self, "obb_corners_cache"):
            env_ids = torch.tensor([env_id],
                                   device=self.device,
                                   dtype=torch.long)
            # Slice to just this env so the (env_ids, :, :, :7) shape matches
            # the (env_ids,) target in update_obb_cache.
            vpt_pose = self._vpt_objects.data.object_state_w[env_ids, :, :7]
            self.update_obb_cache(env_ids, vpt_pose)

        origin_xy = self.scene.env_origins[env_id, :2]
        bound = float(self.center_to_boundary)
        min_xy = torch.tensor([-bound, -bound],
                              device=self.device,
                              dtype=torch.float32)
        max_xy = torch.tensor([bound, bound],
                              device=self.device,
                              dtype=torch.float32)
        ...
        if hasattr(self, "obb_corners_cache"):
            ...
            # --- OBB-aware inflation (replaces AABB bounding-box approach) ---
            # The old AABB approach took min/max of 8 OBB corners then added
            # inflation_radius, which over-inflates rotated objects by up to 41%
            # on the diagonal.  The new approach expands each OBB edge outward
            # by inflation_radius along its outward normal, then rasterizes the
            # resulting convex quadrilateral.  This preserves the true OBB
            # orientation and only inflates where the agent body could actually
            # collide.
            #
            # OBB corner layout (from get_obb_hitbox signs):
            # bottom: 0=(-1,-1,-1) 1=(+1,-1,-1) 2=(+1,+1,-1) 3=(-1,+1,-1)
            # top:    4=(-1,-1,+1) 5=(+1,-1,+1) 6=(+1,+1,+1) 7=(-1,+1,+1)
            # 2D footprint (bottom face, CCW when viewed from above):
            #   c0 -> c1 -> c2 -> c3
            inflate = inflation_radius
            for i in range(corners_xy.shape[0]):
                # 4 bottom corners in world xy
                c = [corners_xy[i, j].cpu().numpy() for j in range(4)]
                # Expand each edge outward by inflate along its outward normal.
                # For a CCW quad (c0->c1->c2->c3), the RIGHT perpendicular
                # of the edge direction points OUTWARD:
                #   edge dir = (ex, ey), outward normal = (ey, -ex) / length
                # (The old code used LEFT perpendicular (-ey, ex)/length which
                # pointed INWARD for CCW, causing under-inflation.)
                #
                # Instead of just shifting each vertex along one edge normal
                # (which undershoots at corners), we compute the intersection
                # of consecutive inflated edges for a proper Minkowski sum.

                # Step 1: compute outward normals for all 4 edges
                edge_normals = []
                degenerate = False
                for k in range(4):
                    p0 = c[k]
                    ...
                    if length < 1e-9:
                        degenerate = True
                        break
                    # RIGHT perpendicular = outward for CCW winding
                    nx = ey / length
                    ny = -ex / length
                ...
                # Step 2: compute inflated vertices as intersections of
                # consecutive shifted edges
                inflated = []
                for k in range(4):
                    k_prev = (k - 1) % 4
                    n_prev = edge_normals[k_prev]
                    n_cur = edge_normals[k]

                    # Shifted origin points for edge k_prev and edge k
                    p0_prev = c[k_prev]
                    p1_prev = c[(k_prev + 1) % 4]
                    sp0_prev_x = p0_prev[0] + n_prev[0] * inflate
                    sp0_prev_y = p0_prev[1] + n_prev[1] * inflate
                    d_prev_x = p1_prev[0] - p0_prev[0]
                    d_prev_y = p1_prev[1] - p0_prev[1]

                    p0_cur = c[k]
                    sp0_cur_x = p0_cur[0] + n_cur[0] * inflate
                    sp0_cur_y = p0_cur[1] + n_cur[1] * inflate
                    p1_cur = c[(k + 1) % 4]
                    d_cur_x = p1_cur[0] - p0_cur[0]
                    d_cur_y = p1_cur[1] - p0_cur[1]

                    # Solve: sp0_prev + t * d_prev = sp0_cur + s * d_cur
                    det = d_prev_x * d_cur_y - d_prev_y * d_cur_x
                    if abs(det) < 1e-12:
                        inflated.append((sp0_cur_x, sp0_cur_y))
                    else:
                ...
                g_pts = []
                for wx, wy in inflated:
                    gx, gy = self._world_to_grid(
                        torch.tensor([wx, wy],
                                     device=self.device,
                                     dtype=torch.float32), origin_xy, min_xy)
                    g_pts.append((gx, gy))

                # Bounding box of the quad in grid coords
                row_lo = max(0, min(p[0] for p in g_pts))
                row_hi = min(size - 1, max(p[0] for p in g_pts))
                col_lo = max(0, min(p[1] for p in g_pts))
                col_hi = min(size - 1, max(p[1] for p in g_pts))

                # For each grid row in the bounding box, find the
                # intersection of the row with the quad edges to
                # determine which columns are inside.
                n = len(g_pts)
                for row in range(row_lo, row_hi + 1):
                    # Collect x-intercepts of quad edges at this row
                    # (using half-integer centres for better accuracy)
                    x_ints = []
                    for e in range(n):
                        r0, c0 = g_pts[e]
                        r1, c1 = g_pts[(e + 1) % n]
                        ...
                        # Does the row cross this edge?
                        if (r0 <= row <= r1) or (r1 <= row <= r0):
                            if r0 == r1:
                                continue
                            t = (row - r0) / (r1 - r0)
                            if 0.0 <= t <= 1.0:
                                x_ints.append(c0 + t * (c1 - c0))
                    if len(x_ints) >= 2:
                        x_ints.sort()
                        c0_fill = max(col_lo, int(math.floor(x_ints[0])))
                        c1_fill = min(col_hi, int(math.ceil(x_ints[-1])))
                        occ[row, c0_fill:c1_fill + 1] = True

        wall_cells = max(1,
                         int(math.ceil(inflation_radius / self.grid_step_m)))
        occ[:wall_cells, :] = True
        occ[-wall_cells:, :] = True
        occ[:, :wall_cells] = True
        occ[:, -wall_cells:] = True
    ...
    def _is_goal_reached_3act(
        ...
        yaw_err = abs(
            math.degrees(self._normalize_angle(target_yaw - agent_yaw)))
    ...
    def plan_to_camera_actions_3act(
        ...
        Two-phase approach:
          Phase 1 — Pure 2D A* on the occupancy grid finds a waypoint
          path from start to goal.  State = (gx, gy) grid cell only;
          no heading dimension.  8-connected moves keep the path short.
          Phase 2 — Convert the waypoint sequence into a turn+forward
          action sequence: for each consecutive waypoint pair, compute
          the heading the agent must face, emit in-place turn actions
          (2=left, 3=right) to align, then one forward action (0).
          Finally, append in-place turns to face the camera yaw.
        """
        _ = yaw_tol_deg  # Kept for API compatibility; completion is position-only.

        true_forward_step_m = self.forward_step_m

        nav = self.build_nav_grid(env_id, inflation_radius=inflation_radius)
        occ = nav["occupancy"]
        size = nav["grid_size"]
        origin_xy = nav["origin_xy"]
        min_xy = nav["min_xy"]
        origin_x = float(origin_xy[0].item())
        origin_y = float(origin_xy[1].item())
        ...
        pre_plan_quat_err = abs(
            math.degrees(self._normalize_angle(target_yaw - start_yaw)))

        # --- Grid coordinate helpers ---
        def world_to_grid(wx: float, wy: float):
            gx = int(round((wx - origin_x - min_x) / self.grid_step_m))
            gy = int(round((wy - origin_y - min_y) / self.grid_step_m))
            return max(0, min(size - 1, gx)), max(0, min(size - 1, gy))

        def grid_to_world(gx: int, gy: int):
            x = origin_x + min_x + gx * self.grid_step_m
            y = origin_y + min_y + gy * self.grid_step_m
            return x, y

        start_gx, start_gy = world_to_grid(float(start_xy[0].item()),
                                           float(start_xy[1].item()))
        goal_gx, goal_gy = world_to_grid(float(goal_xy[0].item()),
                                         float(goal_xy[1].item()))

        # Force-clear start and goal cells so the planner can always
        # enter/exit them even if an OBB footprint overlaps.
        if occ[start_gx, start_gy]:
            occ[start_gx, start_gy] = False
        if occ[goal_gx, goal_gy]:
            occ[goal_gx, goal_gy] = False

        # Also clear cells within pos_tol_m of the goal so the agent
        # can approach from any direction.
        goal_tol_cells = max(1, int(math.ceil(pos_tol_m / self.grid_step_m)))
        for dx in range(-goal_tol_cells, goal_tol_cells + 1):
            for dy in range(-goal_tol_cells, goal_tol_cells + 1):
                cx, cy = goal_gx + dx, goal_gy + dy
                if (0 <= cx < size and 0 <= cy < size and
                        math.hypot(dx, dy) * self.grid_step_m <= pos_tol_m):
                    if occ[cx, cy]:
                        occ[cx, cy] = False

        # ============================================================
        # Phase 1: 2D A* on (gx, gy) grid — no heading dimension.
        # State space: size x size (e.g. 134x134 = ~18k cells).
        # 8-connected neighbours with diagonal cost = sqrt(2).
        # ============================================================
        max_exp = self.astar_max_expansions

        def h2d(gx: int, gy: int) -> float:
            """Octile distance heuristic (admissible for 8-connected)."""
            dx = abs(gx - goal_gx)
            dy = abs(gy - goal_gy)
            # Chebyshev distance with diagonal correction
            return max(dx, dy) + (math.sqrt(2) - 1) * min(dx, dy)

        open_heap = []
        # (f, tiebreak, gx, gy, g_cost)
        heapq.heappush(open_heap,
                       (h2d(start_gx, start_gy), 0, start_gx, start_gy, 0.0))
        came_from: dict = {}  # (gx,gy) -> (parent_gx, parent_gy)
        g_cost: dict = {}  # (gx,gy) -> best g
        visited: set = set()
        push_idx = 1
        expansions = 0

        # 8-connected neighbours: (dgx, dgy, cost)
        # Cardinal moves cost 1 cell; diagonals cost sqrt(2).
        # But each sim forward step moves grid_step_m = 0.15m = 1 cell.
        # Diagonal moves require sqrt(2) forward steps in practice
        # (turn + forward + turn), so use sqrt(2) cost.
        NEIGHBOURS = [
            (1, 0, 1.0),
            (-1, 0, 1.0),
            (0, 1, 1.0),
        ...
        while open_heap and expansions < max_exp:
            _, _, gx, gy, g = heapq.heappop(open_heap)
            if (gx, gy) in visited:
                continue
            visited.add((gx, gy))
            ...
            for dgx, dgy, move_cost in NEIGHBOURS:
                ...
                if occ[nx, ny]:
                    continue
                # For diagonal moves, also check that both cardinal
                # neighbours are free (prevents cutting corners).
                if abs(dgx) + abs(dgy) == 2:
                    if occ[gx + dgx, gy] or occ[gx, gy + dgy]:
                        continue
                ng = g + move_cost
                if (nx, ny) in g_cost and ng >= g_cost[(nx, ny)]:
                    continue
                g_cost[(nx, ny)] = ng
                f = ng + h2d(nx, ny)
                heapq.heappush(open_heap, (f, push_idx, nx, ny, ng))
        ...
        if found_goal is None:
            return {
                ...
                "metrics": {
                    ...
                    expansions,
                    "goal_radius_cells":
                    pos_tol_m / true_forward_step_m,
                    "pre_plan_dist_err_m":
                    pre_plan_dist_err,
                    "pre_plan_quat_err_deg":
                    pre_plan_quat_err,
                    "post_plan_dist_err_m":
                    None,
                    "post_plan_quat_err_deg":
                    None,
                    "grid_step_m":
                    self.grid_step_m,
                    "true_forward_step_m":
                    true_forward_step_m,
                    "heading_bin_deg":
                    math.degrees(self.heading_bin_rad),
                    "camera_yaw_correction_deg":
                    self.camera_yaw_correction_deg,
                    "inflation_radius_m":
                    inflation_radius if inflation_radius is not None else
                    self.planner_inflation_m,
                },
            }

        # Reconstruct 2D grid-cell path.
        grid_path = []
        cur = found_goal
        ...
        # This avoids turn+forward for every single grid cell on a
        # straight line — instead we walk straight until we need to
        # turn.
        if len(grid_path) >= 3:
            ...
            for i in range(1, len(grid_path) - 1):
                ...
                curr = grid_path[i]
                nxt = grid_path[i + 1]
                d_prev = (curr[0] - prev[0], curr[1] - prev[1])
                d_next = (nxt[0] - curr[0], nxt[1] - curr[1])
                if d_prev != d_next:
        ...
        # ============================================================
        # Phase 2: Convert waypoint path to turn + forward actions.
        # For each consecutive waypoint pair, compute the heading
        # the agent must face to walk from A to B, emit turn actions
        # to align, then forward actions to cover the distance.
        # ============================================================
        actions = []
        curr_yaw = float(start_yaw)

        for i in range(len(grid_path) - 1):
            ax, ay = grid_to_world(*grid_path[i])
            bx, by = grid_to_world(*grid_path[i + 1])
            dx = bx - ax
            dy = by - ay
            dist = math.hypot(dx, dy)
            if dist < 1e-9:
                continue
            # Desired heading to go from A toward B.
            desired_yaw = math.atan2(dy, dx)

            # Turn to face desired heading.
            delta = self._normalize_angle(desired_yaw - curr_yaw)
            n_turn = int(abs(delta) / self.heading_bin_rad + 0.5)
            if n_turn > 0:
                turn_action = 2 if delta > 0.0 else 3
                actions.extend([turn_action] * n_turn)
                if delta > 0.0:
        ...
        # Advance position (for final pose estimate).
        # curr_yaw doesn't change during forward steps.

        # NO final yaw alignment in the plan. The plan only gets the
        # agent to the camera's XY position. Heading alignment is done
        # at runtime by `execute_action_sequence` which greedily spams
        # left or right until yaw error is minimized.

        # Hard cap.
        actions = actions[:max_steps]

        # Replay to produce waypoints and final pose.
        waypoints = []
        curr_x, curr_y = float(start_xy[0]), float(start_xy[1])
        curr_yaw = start_yaw
        for a in actions:
            if a == 2:
                curr_yaw = self._normalize_angle(curr_yaw +
                                                 self.heading_bin_rad)
            elif a == 3:
                curr_yaw = self._normalize_angle(curr_yaw -
                                                 self.heading_bin_rad)
            elif a == 0:
                curr_x += math.cos(curr_yaw) * true_forward_step_m
                curr_y += math.sin(curr_yaw) * true_forward_step_m
                waypoints.append([curr_x, curr_y])

        final_xy = torch.tensor([curr_x, curr_y], device=self.device)
        post_plan_dist_err = float(torch.norm(final_xy - goal_xy).item())
        post_plan_quat_err = abs(
            math.degrees(self._normalize_angle(target_yaw - curr_yaw)))
        ...
        return {
            ...
            "metrics": {
                ...
                "goal_radius_cells": pos_tol_m / true_forward_step_m,
                "pre_plan_dist_err_m": pre_plan_dist_err,
                "pre_plan_quat_err_deg": pre_plan_quat_err,
                "post_plan_dist_err_m": post_plan_dist_err,
                "post_plan_quat_err_deg": post_plan_quat_err,
                "grid_step_m": self.grid_step_m,
                "true_forward_step_m": true_forward_step_m,
                "heading_bin_deg": math.degrees(self.heading_bin_rad),
    ...
    def execute_action_sequence(
        ...
        # NOTE: default fill is a NO-OP action (4), not 5. Action 5 is SOFT RESET
        # in move_agent — using it as a default would silently reset every env
        # whose sequence has finished or that is already marked done, scrambling
        # the scene mid-execution.
        NOOP_ACTION = 4
        max_len = max((len(seq) for seq in action_sequences), default=0)
        for t in range(max_len):
            actions = torch.full((len(env_ids), ),
                                 NOOP_ACTION,
                                 device=self.device,
                                 dtype=torch.long)
            for i, env_id in enumerate(env_ids.tolist()):
                ...
                if t < len(seq):
                    ...
                    steps_used[i] += 1
                    reached, _, _ = self._is_goal_reached_3act(
                        env_id, pos_tol_m, yaw_tol_deg)
                    if reached:
                        actions[i] = NOOP_ACTION  # prevent overshoot
        ...
        for env_id in env_ids.tolist():
            reached, pos_err, yaw_err = self._is_goal_reached_3act(
                env_id, pos_tol_m, yaw_tol_deg)
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        # Debug toggle: when set on the unwrapped env, disables auto-reset so
        # interactive A* sweeps don't get the scene re-randomized the moment
        # the agent reaches the camera. Set via:
        #   env.unwrapped._disable_auto_reset = True
        if getattr(self, "_disable_auto_reset", False):
            zeros = torch.zeros(self.num_envs,
                                dtype=torch.bool,
                                device=self.device)
            return zeros, zeros

        # Success requires BOTH pos and yaw to be in tolerance. Matches the
        # stated env termination criteria: dist <= 0.2 m AND yaw <= 0.2 rad
        # (~11.46 deg) off the camera-corrected target yaw.
        distance = (self._camera_obj.data.root_pos_w[:, :2] -
                    self._agent.data.root_pos_w[:, :2])**2
        distance = torch.sqrt(distance.sum(dim=1))
        pos_ok = distance <= 0.2

        agent_quat = self._agent.data.root_quat_w
        cam_quat = self._camera_obj.data.root_quat_w

        # Vectorized yaw extraction (siny_cosp / cosy_cosp).
        def _yaw(q):
        ...
        agent_yaw = _yaw(agent_quat)
        target_yaw = _yaw(cam_quat) + (math.pi / 2.0)  # camera_yaw_correction
        d_yaw = (target_yaw - agent_yaw + math.pi) % (2.0 * math.pi) - math.pi
        yaw_ok = d_yaw.abs() <= 0.2  # rad

        goal_reached = pos_ok & yaw_ok
    ...
    def _validate_env_state(self, env_id: torch.Tensor, folder_idx: int,
                            min_viewpoints: int) -> tuple[bool, str]:
        """
        ...
        # --- 1. Check Viewpoint Count ---
        # Safely retrieve poses (handle uninitialized/None states)
        poses = self.valid_viewpoint_poses[
            env_idx] if self.valid_viewpoint_poses else None
    ...
    def render(self):
        ...
        # Draw the previous action on env_0, with safe fallback if unavailable.
        if hasattr(self, "old_actions"):
            action = int(self.old_actions[0].item())
        elif hasattr(self, "actions"):
            action = int(self.actions[0].item())
        else:
            action = -1
    ...
    def _check_occlusion(self,
        ...
        if torch.norm(target_pos - sensor_pos).item() > 0.01:
            ...
            new_orient = math_utils.quat_mul(
                body_quat.unsqueeze(0), rot_correction.unsqueeze(0)).squeeze(0)
            ...
            camera.set_world_poses(positions=target_pos.unsqueeze(0),
    ...
    def _save_env_config_to_json(self, env_id: int, folder_idx: int):
        ...
        config = {
            ...
            "environment_settings": {
                ...
                "planner": {
                    "camera_yaw_correction_deg":
                    float(self.camera_yaw_correction_deg),
                    "grid_step_m":
                    float(self.grid_step_m),
                    "heading_bin_deg":
                    float(math.degrees(self.heading_bin_rad)),
                },
            },
            "goal_ball": {
                "position": goal_pos,
    ...
    def _get_batch_active_indices(
        self, env_ids: int | list | torch.Tensor
        ...
        if isinstance(self.active_vpt_indices, list):
            ...
            if self.active_vpt_indices and isinstance(
                    self.active_vpt_indices[0], torch.Tensor):
                full_indices = torch.stack(self.active_vpt_indices).to(
                    self.device)
            else:
                full_indices = torch.tensor(self.active_vpt_indices,
                                            device=self.device,
                                            dtype=torch.long)
        else:
    ...
    def _select_active_vpt_indices(self, env_ids: torch.Tensor) -> None:
        ...
        """
        env_ids = env_ids.view(-1) if torch.is_tensor(
            env_ids) else torch.tensor(env_ids)

        for env_id in env_ids:
            # Random permutation to select unique objects
            active = torch.randperm(self.num_objs,
                                    device=self.device)[:self.active_vpt_objs]
    ...
    def _store_inactive_vpt_objects(
        ...
        inactive_mask = torch.ones((num_batch, self.num_objs),
                                   dtype=torch.bool,
                                   device=self.device)
        ...
        local_row_indices = torch.arange(num_batch, device=self.device).view(
            -1, 1).expand(-1, self.num_objs)

        col_indices = torch.arange(self.num_objs,
                                   device=self.device).expand(num_batch, -1)
        ...
        if len(target_rows) > 0:
            vpt_obj_default_state[target_rows, target_cols,
    ...
    def _get_active_vpt_dims(self,
                             env_ids: int | torch.Tensor) -> torch.Tensor:
        """
    ...
    def _get_active_vpt_positions(
        ...
        active_pos = self._vpt_objects.data.object_pos_w[
            env_ids_expanded, batch_indices].clone()
        ...
        # 3. Return Logic
        if return_full_pose:
            active_quat = self._vpt_objects.data.object_quat_w[
                env_ids_expanded, batch_indices].clone()
    ...
    def _select_viewpoints_for_collection(self, env_id: int) -> bool:
        ...
        if (self.valid_viewpoint_poses is None
                ...
                or self.valid_viewpoint_poses[env_id] is None or
                len(self.valid_viewpoint_poses[env_id]) < self.images_per_env):
            return False
    ...
    def _reset_idx(self,
        ...
        subset_idx = torch.randperm(len(env_ids),
                                    device=self.device)[:num_targets]
        self.envs_to_move_ball = env_ids[subset_idx]

        # --- 3. Physics Reset ---
        # Map slots to their current folder/visibility state
        reset_folder_indices = [
            self.slot_folder_indices[i] for i in active_slots_list
        ]
        reset_visibility_cats = [
            self.slot_visibility_categories[i] for i in active_slots_list
        ]

        if self.verbose >= 1:
            print(
                f"🔄 Resetting {len(active_slots_list)} envs (RL: {rl_reset})")

        self._reset_idx_internal(env_ids,
                                 rl_reset=rl_reset,
        ...
        for slot_idx in valid_slots:
            ...
            # Select viewpoints and collect images
    ...
    def _ensure_slot_initialization(self) -> None:
        ...
        self.slot_folder_indices = [
            self.next_env_folder_idx + i for i in range(self.num_envs)
        ]
        self.slot_attempt_counts = [0] * self.num_envs
        ...
        for idx in self.slot_folder_indices:
            self.slot_visibility_categories.append(
                self._assign_next_visibility_label(idx))

        self._save_visibility_labels()

    def _validate_slots(
            self, active_slots: list[int]) -> tuple[list[int], list[int]]:
        """
        ...
        for slot_idx in active_slots:
            ...
            if is_valid:
                ...
                if self.slot_attempt_counts[
                    ...
                    if self.verbose >= 1:
                        print(
                            f"  ⚠️ Slot {slot_idx} | Env {env_id} EXCEEDED attempts."
                        )
                elif self.verbose >= 2:
    ...
    def _replenish_slots(self, slots_to_replace: list[int]) -> None:
        ...
        for slot_idx in slots_to_replace:
            ...
            self.slot_folder_indices[
                slot_idx] = self.next_env_folder_idx + new_env
            self.slot_attempt_counts[slot_idx] = 0

            # Assign new visibility label
            self.slot_visibility_categories[
                slot_idx] = self._assign_next_visibility_label(
    ...
    def _randomize_scene_props(self, env_ids: list[int]) -> None:
        ...
        vpt_paths = [
            f"/World/envs/env_{eid}/obs_{oid}" for eid in env_ids
            for oid in range(self.cfg.num_vpt_objs)
        ]

        if vpt_paths:
            self.randomize_shape_scale(prim_path_expr=vpt_paths,
                                       is_random=True)
    ...
    def initial_spawn_loop_old(self,
        ...
        # Half side length of the square (centered on the camera) inside which
        # the agent may spawn. Total square side = 2 * AGENT_CAMERA_HALF_SQUARE.
        AGENT_CAMERA_HALF_SQUARE = 2.5

        # --- 1. Initial Sampling (Goal, Camera) ---
        # Agent is spawned later, relative to the finalized camera position.
        ...
        # --- 3b. Spawn agent inside a (2 * AGENT_CAMERA_HALF_SQUARE)-side
        # square centered on the (now-finalized) camera position. Clamp to the
        # safe range so the agent never spawns outside the arena bounds.
        cam_pos_xy = camera_obj_default_state[retry_indices, :2]
        agent_local_offsets = sample_uniform(-AGENT_CAMERA_HALF_SQUARE,
                                             AGENT_CAMERA_HALF_SQUARE,
                                             (batch_size, 2), device)
        agent_xy_world = cam_pos_xy + agent_local_offsets
        agent_xy_local = agent_xy_world - env_origins[:, :2]
        _safe_xr = float(safe_x_range)
        agent_xy_local = torch.clamp(agent_xy_local, -_safe_xr, _safe_xr)
        agent_default_state[retry_indices,
                            0] = env_origins[:, 0] + agent_xy_local[:, 0]
        agent_default_state[retry_indices,
                            1] = env_origins[:, 1] + agent_xy_local[:, 1]
            ...
            for k, obj_idx in enumerate(active_indices):
                ...
                for _ in range(MAX_ATTEMPTS):
                    ...
                    if allow_clipping:
                        ...
                        if dist_cam < 5.5 or dist_goal < (self.goal_radius +
                                                          2.0 + 0.05):
                            continue
                        ...
                        quat = quat_from_euler_xyz(
                            ...
                            torch.tensor(0.0, device=device), r_yaw_tensor)
    ...
    def initial_spawn_loop(self,
        ...
        rxs = (torch.rand((batch_size, num_vpt_objs), device=device) * 2 *
               safe_x_range_obstacles) - safe_x_range_obstacles
        rys = (torch.rand((batch_size, num_vpt_objs), device=device) * 2 *
               safe_x_range_obstacles) - safe_x_range_obstacles
        r_yaws = torch.rand(
            (batch_size, num_vpt_objs), device=device) * 2 * math.pi

        vpt_obj_default_state[retry_indices, :,
                              0] = env_origins[:, 0].unsqueeze(1) + rxs
        vpt_obj_default_state[retry_indices, :,
                              1] = env_origins[:, 1].unsqueeze(1) + rys
        ...
        for _ in range(max_dist_retries):
            ...
            camera_obj_default_state[
                bad_local_indices] = self.place_object_safely(
        ...
        # ==========================================================
        # 6. PLACE AGENT
        #    (a) inside a square centered on the camera,
        #    (b) yaw set so agent faces the camera,
        #    (c) collision-rejected against VPT obstacles.
        # NOTE: this MUST be the final agent placement. The post-loop
        # `place_object_safely(object_type='agent', ...)` shotgun is
        # disabled below — it would otherwise randomize agent xy across
        # the whole arena and randomize yaw, undoing all of this.
        # ==========================================================
        # Spawn rules:
        #   * L_inf  : |dx| <= AGENT_CAMERA_HALF_SQUARE AND |dy| <= AGENT_CAMERA_HALF_SQUARE
        #             (10 x 10 m box centered on the camera; per-axis max 5 m).
        #   * L_2    : Euclidean distance from camera >= AGENT_CAMERA_MIN_DIST.
        #     This is a *circular* deadzone, not a square one.
        AGENT_CAMERA_HALF_SQUARE = 5.0
        AGENT_CAMERA_MIN_DIST = 1.0
        AGENT_CAND_PER_ENV = 256
        _safe_xr = float(safe_x_range)
        cam_pos_xy = camera_obj_default_state[retry_indices, :2]  # (B, 2)

        # Sample K candidates per env inside the camera-centered outer square,
        # clamped into the safe arena range. Deadzone rejection is applied
        # below alongside the collision filter.
        cand_offsets = sample_uniform(
            -AGENT_CAMERA_HALF_SQUARE,
            AGENT_CAMERA_HALF_SQUARE,
            (batch_size, AGENT_CAND_PER_ENV, 2),
            device,
        )
        cand_world = cam_pos_xy.unsqueeze(1) + cand_offsets  # (B, K, 2)
        cand_local = cand_world - env_origins[:, :2].unsqueeze(1)
        cand_local = torch.clamp(cand_local, -_safe_xr, _safe_xr)
        cand_world = env_origins[:, :2].unsqueeze(1) + cand_local  # (B, K, 2)

        # Yaw so agent's +x (forward) points AT the camera.
        cam_xy_b = cam_pos_xy.unsqueeze(1)  # (B, 1, 2)
        dxy = cam_xy_b - cand_world  # (B, K, 2)
        cand_yaw = torch.atan2(dxy[..., 1], dxy[..., 0])  # (B, K)

        # Build full pose tensors for the vectorized collision check.
        agent_z = agent_default_state[retry_indices, 2].unsqueeze(1).expand(
            -1, AGENT_CAND_PER_ENV)
        cand_pos = torch.stack(
            [cand_world[..., 0], cand_world[..., 1], agent_z],
            dim=-1,
        )  # (B, K, 3)
        zeros_yaw = torch.zeros_like(cand_yaw)
        cand_quat = quat_from_euler_xyz(zeros_yaw, zeros_yaw,
                                        cand_yaw)  # (B, K, 4)

        flat_env_ids = global_retry_env_ids.repeat_interleave(
            AGENT_CAND_PER_ENV)
        cand_collisions = self._check_collisions_vectorized(
            flat_env_ids,
            cand_pos.reshape(-1, 3),
            cand_quat.reshape(-1, 4),
            object_type='agent',
        ).reshape(batch_size, AGENT_CAND_PER_ENV)  # (B, K) True = collision

        # Reject candidates whose Euclidean distance to the camera is below
        # AGENT_CAMERA_MIN_DIST. Sampling is in a square so the deadzone
        # check uses the L2 norm directly: ||cand - cam||_2 >= MIN_DIST.
        cand_dist_sq = (cand_offsets**2).sum(dim=-1)  # (B, K)
        in_deadzone = cand_dist_sq < (AGENT_CAMERA_MIN_DIST**2)  # (B, K)

        # Two-tier selection:
        # Tier 1 (preferred): outside deadzone AND collision-free.
        # Tier 2 (fallback):  outside deadzone, allow collision.
        # The deadzone is enforced HARD — we'd rather pick a colliding pose
        # outside the deadzone than a clean one inside it. Collisions get
        # resolved later (sim has its own collision response); a too-close
        # spawn corrupts the data point semantically.
        tier1 = (~cand_collisions) & (~in_deadzone)
        tier2 = (~in_deadzone)

        chosen_idx_t1 = torch.argmax(tier1.int(), dim=1)  # (B,)
        chosen_idx_t2 = torch.argmax(tier2.int(), dim=1)  # (B,)
        has_t1 = tier1.any(dim=1)
        has_t2 = tier2.any(dim=1)
        chosen_idx = torch.where(has_t1, chosen_idx_t1, chosen_idx_t2)  # (B,)

        n_collide_fallback = int(((~has_t1) & has_t2).sum().item())
        n_no_outside = int((~has_t2).sum().item())
        if n_collide_fallback:
            print(
                f"[WARN] place_agent: {n_collide_fallback} env(s) had only "
                f"colliding poses outside the deadzone (kept anyway — sim will resolve)."
            )
        if n_no_outside:
            print(
                f"[ERROR] place_agent: {n_no_outside} env(s) had ZERO candidates "
                f"outside the {AGENT_CAMERA_MIN_DIST} m radial deadzone in "
                f"{AGENT_CAND_PER_ENV} samples (extremely unlikely — check rng)."
            )

        b_idx = torch.arange(batch_size, device=device)
        chosen_xy = cand_world[b_idx, chosen_idx]  # (B, 2)
        chosen_quat = cand_quat[b_idx, chosen_idx]  # (B, 4)

        agent_default_state[retry_indices, 0] = chosen_xy[:, 0]
        agent_default_state[retry_indices, 1] = chosen_xy[:, 1]
        agent_default_state[retry_indices, 3:7] = chosen_quat
        ...
        zero = torch.zeros_like(yaw)
        camera_obj_default_state[retry_indices,
                                 3:7] = quat_from_euler_xyz(roll, zero, yaw)
    ...
    def move_vpt_objects(
        ...
        for env_idx in valid_indices:
            ...
            # --- 1. Filter Logic ---
            should_move = (category == "occluded") or (env_idx_int
                                                       in indices_to_move)
            if not should_move:
                continue
            ...
            if dist > 1e-6:
                ...
                t_min, t_max = (0.3, 0.7) if category == "in_view" else (0.2,
                                                                         0.8)
    ...
    def check_z_bounds(self,
        ...
        for local_idx, env_idx in enumerate(valid_indices):
            ...
            valid_obj_mask = (adjusted_z >= -tolerance) & (adjusted_z <=
                                                           0.1 + tolerance)
            ...
            if not torch.all(valid_obj_mask):
                ...
                for idx in failed_indices:
                    ...
                    failure_reasons.append(
                        f"VPT Obj {global_id} Z: {bad_z:.6f}")
    ...
    def occlusion_validation_check(self, final_valid_env_ids, valid_indices,
        ...
        for local_idx, env_idx in enumerate(valid_indices):
            ...
            if visibility_category in ["in_view", "occluded", "outside_fov"]:
                ...
                is_occluded = self._check_occlusion(camera_pos, goal_pos,
                                                    env_id)
    ...
    def geometric_occlusion_check(
            self, env_ids: torch.Tensor, valid_indices: torch.Tensor,
        ...
        for i, global_env_id in enumerate(passed_env_ids):
            ...
            local_idx = (env_ids == global_env_id).nonzero(
    ...
    def camera_pov_validation(self, env_ids: torch.Tensor,
                              valid_indices: torch.Tensor,
        ...
        for local_idx, env_idx in enumerate(valid_indices):
            ...
            if is_valid:
                if self.verbose >= 2:
                    print(
                        f"    ✅ Env {env_id_val}: Camera valid ({category}) | Red: {red_count}"
                    )
            else:
                # Mark failure
                envs_need_spawn_retry[env_idx] = True
                ...
                if self.verbose >= 1:
                    ...
                    print(
                        f"       Expected: {category}, Got: {status} (Red: {red_count})"
                    )
    ...
    def _reset_idx_internal(self,
        ...
        # NOTE: agent placement is owned by `initial_spawn_loop` — it picks
        # a collision-free pose inside the camera-centered square AND sets
        # yaw to face the camera. Calling `place_object_safely` here would
        # re-randomize agent xy across the whole arena and randomize yaw,
        # undoing both of those constraints. We still need the OBB cache
        # refresh that `place_object_safely` did as a side effect, so do
        # that explicitly.
    ...
    def _teleport_and_step(self, env_ids, points):
        ...
        yaw_jitter = (torch.rand(len(env_ids), device=points.device) -
                      0.5) * 2 * math.radians(15)
    ...
    def generate_valid_circle_points(
        ...
        radial_jitter = (
            torch.rand(num_envs, num_angles, device=device) * 0.4 - 0.2)
        jitter_scale = 1.0 + radial_jitter  # [0.8, 1.2] multiplier

        all_x = self._goal.data.root_pos_w[env_ids, 0].unsqueeze(
            1) + (radii * jitter_scale) * torch.cos(angles_expanded)
        all_y = self._goal.data.root_pos_w[env_ids, 1].unsqueeze(
            1) + (radii * jitter_scale) * torch.sin(angles_expanded)
    ...
    def _collect_images_for_slot(self, env_id: torch.Tensor,
        ...
        For each of the N selected viewpoints:
        1. Teleports agent to the viewpoint.
        2. Orients agent to face the midpoint between the camera and goal.
        3. Steps simulation to update sensors.
        4. Saves RGB and Depth data.
        ...
        if viewpoints is None:
            raise RuntimeError(
                f"No viewpoints selected for Slot {env_slot} (Env {global_env_id})"
            )

        if self.verbose >= 1:
            print(
                f"    📸 Collecting {self.images_per_env} images | Slot {env_slot} -> Folder {folder_idx}"
            )

        # --- Setup Constants ---
        single_env_tensor = torch.tensor([env_slot],
                                         dtype=torch.long,
                                         device=self.device)
        zero_velocity = torch.zeros((1, 6), device=self.device)

        # Get static reference points for orientation calculation
        ...
        for i in range(self.images_per_env):
            ...
            SAFETY_MARGIN = math.radians(
                5.0)  # keep tip this far inside the edge
            ...
            # Construct Quaternion (wxyz for Isaac Sim: Rotation around Z)
            half_yaw = yaw / 2
            quat = torch.tensor(
                [torch.cos(half_yaw), 0.0, 0.0,
                 torch.sin(half_yaw)],
                device=self.device)

            # Construct Full Pose [x, y, z, w, x, y, z]
            pose = torch.tensor([
                target_pos_2d[0], target_pos_2d[1], agent_z, quat[0], quat[1],
                quat[2], quat[3]
            ],
            ...
            self._agent.write_root_com_velocity_to_sim(zero_velocity,
                                                       single_env_tensor)

            # 3. Update Sensors (Step Sim)
            for _ in range(30):
                self.sim.step()
                self._rgb_tiled_camera.update(self.sim.cfg.dt)
            ...
            depth = self._rgb_tiled_camera.data.output["distance_to_camera"][
                env_slot]
            semantic = self._rgb_tiled_camera.data.output[
                "semantic_segmentation"][env_slot]
            cam_pov_rgb = self._occlusion_camera.data.output["rgb"][env_slot]
            cam_pov = self._occlusion_camera.data.output[
                "semantic_segmentation"][
                    env_slot] if self.save_camera_pov else None

            # 5. Save Data
            self._save_single_image(env_slot,
                                    folder_idx,
                                    rgb,
                                    depth,
                                    semantic,
    ...
    def _save_single_image(self,
                           ...
                           camera_pov_rgb_data: torch.Tensor | None = None,
                           image_idx: int = 0) -> None:
        """
        Process and save RGB, Depth, and optional Camera POV images to disk.
        """
        # --- 1. Path Setup ---
        label = self.env_visibility_labels.get(folder_idx)
        if label not in ["Yes", "No"]:
            raise RuntimeError(
                f"Invalid visibility label '{label}' for folder {folder_idx}")
        ...
        # Optimization: Only create directories on the first image of the batch
        if image_idx == 0:
        ...
        # --- 2. Save RGB ---
        # Handle Float vs Byte conversion
        ...
        # Strip Alpha channel if present (4 channels -> 3 channels)
        if rgb_np.shape[-1] == 4:
            rgb_np = rgb_np[..., :3]

        cv2.imwrite(f"{rgb_dir}/image_{image_idx:04d}.png",
                    cv2.cvtColor(rgb_np, cv2.COLOR_RGB2BGR))

        # --- 3. Save Depth ---
        depth_np = depth_data.cpu().numpy()

        # Replace Inf with max valid value or 0
        ...
        if depth_np.max() > depth_np.min():
            depth_norm = cv2.normalize(depth_np,
                                       None,
                                       0,
                                       255,
                                       cv2.NORM_MINMAX,
        ...
        if semantic_data is not None:
            ...
            cv2.imwrite(f"{semantic_dir}/image_{image_idx:04d}.png",
                        cv2.cvtColor(sem_np, cv2.COLOR_RGB2BGR))

        # --- 5. Save Camera POV (Once per env) ---
        if self.save_camera_pov and camera_pov_data is not None and image_idx == 0:
            ...
            cv2.imwrite(f"{cam_dir}/cam_pov.png",
                        cv2.cvtColor(cam_pov_np, cv2.COLOR_RGB2BGR))

            torch.save(camera_pov_rgb_data, f"{cam_dir}/cam_rgb_save.pt")

            # --- 6. Save Camera POV RGB ---
            cam_pov_rgb_np = camera_pov_rgb_data.cpu().numpy()
            if cam_pov_rgb_np.dtype != np.uint8:
                if cam_pov_rgb_np.max() <= 1.0:
                    cam_pov_rgb_np = (cam_pov_rgb_np * 255.0).astype(np.uint8)
                else:
                    cam_pov_rgb_np = cam_pov_rgb_np.astype(np.uint8)
            if cam_pov_rgb_np.shape[-1] == 4:
                cam_pov_rgb_np = cam_pov_rgb_np[..., :3]

            # Regular
            cv2.imwrite(f"{cam_dir}/cam_pov_rgb.png",
                        cv2.cvtColor(cam_pov_rgb_np, cv2.COLOR_RGB2BGR))

            # Normalized
            cam_pov_rgb_float = cam_pov_rgb_np.astype(np.float32)
            mn, mx = cam_pov_rgb_float.min(), cam_pov_rgb_float.max()
            if mx > mn:
                cam_pov_rgb_norm = ((cam_pov_rgb_float - mn) / (mx - mn) *
                                    255).astype(np.uint8)
            else:
                cam_pov_rgb_norm = np.zeros_like(cam_pov_rgb_np)
            cv2.imwrite(f"{cam_dir}/cam_pov_rgb_normalized.png",
    ...
    @staticmethod
    def _to_uint8_rgb(arr: torch.Tensor) -> np.ndarray:
        """Tensor → uint8 RGB numpy (drop alpha)."""
        a = arr.cpu().numpy()
        if a.dtype != np.uint8:
            if a.max() <= 1.0:
    ...
    def save_rollout_step(self,
        ...
        cv2.imwrite(f"{dirs['rgb']}/{fname}",
                    cv2.cvtColor(rgb_np, cv2.COLOR_RGB2BGR))
        cv2.imwrite(f"{dirs['semantic']}/{fname}",
                    cv2.cvtColor(sem_np, cv2.COLOR_RGB2BGR))

    def save_rollout_final(self,
                           env_slot: int,
                           folder_idx: int,
                           pos_err: float,
                           yaw_err: float) -> None:
        ...
        cv2.imwrite(f"{dirs['rgb']}/{tag}.png",
                    cv2.cvtColor(rgb_np, cv2.COLOR_RGB2BGR))
        cv2.imwrite(f"{dirs['semantic']}/{tag}.png",
                    cv2.cvtColor(sem_np, cv2.COLOR_RGB2BGR))
        cv2.imwrite(f"{dirs['cam']}/final_cam_semantic.png",
                    cv2.cvtColor(cam_np, cv2.COLOR_RGB2BGR))
        ...
        for key, obj_cfg in self.cfg.vpt_objects.rigid_objects.items():
            ...
            # Updated Primitive Names: MeshCuboid, MeshCone, MeshCylinder
                ...
                dims = torch.tensor(spawn_cfg.size, device=self.device)
                    ...
                    spawn_cfg,
                (sim_utils.MeshConeCfg, sim_utils.MeshCylinderCfg)):
    ...
    def _get_object_corners(self, pos, quat, object_type='agent'):
        if object_type == 'agent':
            ...
            half_extents = torch.tensor([0.55, 0.575, 0.45],
                                        device=self.device)
    ...
    def _check_collisions_vectorized(self,
        ...
        # Shape: (N, 1, 8, 3)
        agent_corners = self._get_object_corners(proposed_pos,
                                                 proposed_quat,
    ...
    def place_object_safely(self,
        ...
        flat_cols = self._check_collisions_vectorized(
            ...
            object_type=object_type).reshape(num_envs, BATCH_SIZE)
        ...
        if unsafe_mask.any():
            print(
                f"⚠️ Shotgun Fail: {unsafe_mask.sum().item()} envs for {object_type}."
            )
