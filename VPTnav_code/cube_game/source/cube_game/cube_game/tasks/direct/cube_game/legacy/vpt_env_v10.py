...
from .vpt_env_cfg_v10 import VPTEnvCfg
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        print("*"*50)
        print(f"🚀 Initializing VPTEnv on GPU {self.GPU_ID} with {self.num_envs} environments...")
        print("*"*50)
        
        # File paths
        self.base_path = f"/home/arock3/data_v10c/data_{self.GPU_ID}"
    ...
    # def _cache_vpt_object_dims(self):
    #     """Pre-computes dimensions for all VPT objects to enable fast batch lookups."""
    #     num_total_objs = self.num_objs # Or len(self.cfg.vpt_objects.rigid_objects)
    #     self.all_vpt_dims = torch.zeros((num_total_objs, 3), device=self.device)
    ...
    #         if isinstance(spawn_cfg, sim_utils.CuboidCfg):
    #             # [x, y, z]
    #             dims = torch.tensor(spawn_cfg.size, device=self.device)
                
    #         elif isinstance(spawn_cfg, (sim_utils.ConeCfg, sim_utils.CylinderCfg)):
    #             # [2r, 2r, h]
    #             dims = torch.tensor([2 * spawn_cfg.radius, 2 * spawn_cfg.radius, spawn_cfg.height], device=self.device)
                
    #         elif isinstance(spawn_cfg, sim_utils.UsdFileCfg):
    #             scale = spawn_cfg.scale
    #             path = spawn_cfg.usd_path
    #             # Robust file name parsing
    #             filename = path.split('/')[-1] # Get filename from full path
    #             first_word_last_letter = filename.split('.')[0][-1]

    #             x_dim, y_dim, z_dim = scale[0], scale[1], scale[2]

    #             if first_word_last_letter == "s":   # Cross
    #                 x_dim /= self.cross_scaling_factors[0]
    #                 y_dim /= self.cross_scaling_factors[1]
    #                 z_dim /= self.cross_scaling_factors[2]
    #             elif first_word_last_letter == "L": # L
    #                 x_dim /= self.l_scaling_factors[0]
    #                 y_dim /= self.l_scaling_factors[1]
    #                 z_dim /= self.l_scaling_factors[2]
    #             elif first_word_last_letter == "T": # T
    #                 x_dim /= self.t_scaling_factors[0]
    #                 y_dim /= self.t_scaling_factors[1]
    #                 z_dim /= self.t_scaling_factors[2]
                
    #             dims = torch.tensor([x_dim, y_dim, z_dim], device=self.device)
    #         else:
    #             # Fallback / Default
    #             dims = torch.tensor([0.1, 0.1, 0.1], device=self.device)

    #         self.all_vpt_dims[i] = dims
    ...
    def _setup_scene(self):
        ...
        # self.cfg.light_cfg_a.func(
        #     prim_path,
        #     self.cfg.light_cfg_a,
    ...
    def _select_active_vpt_indices(self, env_ids: torch.Tensor) -> None:
        ...
        # # Then in _select_active_vpt_indices:
        # for env_id in env_ids:
        #     env_id_item = env_id.item() if torch.is_tensor(env_id) else env_id
            
        #     # Get available indices (not yet used)
        #     available_indices = list(set(range(self.num_objs)) - self.used_vpt_objects)
            
        #     if len(available_indices) < self.active_vpt_objs:
        #         raise RuntimeError(f"Not enough available VPT objects: {len(available_indices)} < {self.active_vpt_objs}")
            
        #     # Randomly select from available
        #     selected = torch.randperm(len(available_indices), device=device)[:self.active_vpt_objs]
        #     active_indices = torch.tensor([available_indices[i] for i in selected], device=device)
            
        #     # Store for this environment
        #     self.active_vpt_indices[env_id_item] = active_indices
            
        #     # Mark as used (add to global set)
        #     self.used_vpt_objects.update(active_indices.cpu().numpy().tolist())
            
        #     if self.verbose >= 2:
        #         print(f"  🎲 Env {env_id_item}: Selected {self.active_vpt_objs} active VPT indices from {len(available_indices)} available (total used: {len(self.used_vpt_objects)})")
    ...
    def _get_active_vpt_dims(self, env_ids) -> torch.Tensor:
        ...
        batch_indices = active_indices_tensor[env_ids] # Shape: [batch_size, active_objs]
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        ...
        vpt_prim_paths = [f"/World/envs/env_.*/obs_{idx}" for idx in range(self.cfg.num_vpt_objs)]
        ...
        while self.next_env_id < self.total_envs_to_sim or len(self.completed_envs) < self.total_envs_to_sim:
            ...
            print(f"Rescaling VPT obstacles for {self.cfg.num_vpt_objs} objects...")
            self.randomize_shape_scale(prim_path_expr=vpt_prim_paths)
            print(f"Recoloring VPT obstacles for {self.cfg.num_vpt_objs} objects...")
            # self.randomize_shape_color(prim_path_expr=vpt_prim_paths, random_roughness=True, random_metallic=True)
            self.randomize_shape_color(prim_path_expr=vpt_prim_paths)
            print(f"Randomizing environment floor color...")
            self.randomize_shape_color(prim_path_expr=["/World/envs/env_.*/mat"])
            self.randomize_shape_color(prim_path_expr=["/World/envs/env_.*/bottom_wall", "/World/envs/env_.*/right_wall", "/World/envs/env_.*/left_wall", "/World/envs/env_.*/top_wall"])
            ...
            print(f"Randomization time for size and color = {np.round(randomize_end_time - randomize_start_time, 3) / self.cfg.num_vpt_objs}")
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            ...
            for idx in range(self.num_envs):
                if envs_need_spawn_retry[idx].item() is True:
                    ...
                    env_dict[self.slot_to_env_id[idx]]["completion_status"] = "incomplete"
                elif envs_need_spawn_retry[idx].item() is False:
                    if env_dict[self.slot_to_env_id[idx]]["completion_status"] != "complete": 
                        env_dict[self.slot_to_env_id[idx]]["completion_status"] = "complete"
                ...
                for key in ["writing_spawn_pose_time", "moving_ball_time", "camera_posing_time", 
                            "vpt_occlusion_time", "vpt_in_out_fov_time",
                            "vpt_object_placement_time",
                            "occlusion_raycast_time", "geometric_check_time", 
                            "camera_validation_time", "circle_validation_time"]:
                    if key in env_dict[self.slot_to_env_id[idx]] and isinstance(env_dict[self.slot_to_env_id[idx]][key], dict):
                        for sub_key in env_dict[self.slot_to_env_id[idx]][key]:
                            if isinstance(env_dict[self.slot_to_env_id[idx]][key][sub_key], (float, int)):
                                env_dict[self.slot_to_env_id[idx]][key][sub_key] = np.round(env_dict[self.slot_to_env_id[idx]][key][sub_key], 3)
            ...
            for batch_idx, env_idx in enumerate(retry_indices):
                ...
                for local_idx, obj_idx in enumerate(active_indices):
                    ...
                    # 2. Get Actual Randomized Height
                    current_height = self.all_vpt_dims[env_id, obj_idx, 2]
                    
                    # 3. Get Z-Offset Ratio (0.5 for Primitives, 0.0 for USDs)
                    z_ratio = self.vpt_z_offset_ratios[obj_idx]
                    
                    # 4. Calculate Z
                    # Z = Floor + (Height * Ratio)
                    vpt_obj_default_state[env_idx, obj_idx, 2] = env_origins[batch_idx, 2] + (current_height * z_ratio)
            ...
            for batch_idx, env_idx in enumerate(retry_indices):
                ...
                if not torch.all(camera_distances_from_vpt >= 2.5):
                    continue
                
                envs_need_spawn_retry[env_idx] = False
                print(f"  ✅ Env {env_ids[env_idx].item()} passed spawn attempt {spawn_attempt}")
            ...
            # if self.verbose >= 1:
            #     print(f"    🕒 Writing spawn poses to sim time: {writing_spawn_end_time - writing_spawn_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     current_time = time.time()
            #     elapsed = current_time - reset_internal_start_time
            #     spawn_elapsed = current_time - spawn_start_time
            #     print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     print(f"Elapsed time from reset internal start: {elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            # Update dicts for all incomplete envs
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["writing_spawn_pose_time"][spawn_attempt] = writing_spawn_end_time - writing_spawn_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["writing_spawn_pose_time"][spawn_attempt] = "F"
            ...
            if len(move_ball_indices) > 0:
                ...
                # --- FIX START ---
                # Convert list of tensors to a single tensor if necessary
                if isinstance(self.active_vpt_indices, list):
                    if len(self.active_vpt_indices) > 0 and isinstance(self.active_vpt_indices[0], torch.Tensor):
                ...
                # Use the new tensor variable here
                ...
                if not has_candidates_mask.all():
                    ...
                    # (Prints skipped envs effectively)
                    skipped_indices = move_ball_indices[~has_candidates_mask]
                    if len(skipped_indices) > 0:
                        print(f"  ❌ Skipping ball move for {len(skipped_indices)} envs (no valid objects found).")
                ...
                if batch_size > 0:
                    ...
                    # Get current goal positions
                    target_goal_pos = goal_default_state[move_ball_indices, :3]
                    target_env_origins = self.scene.env_origins[target_env_ids]
                    
                    # Calculate new Z for Object: Origin + Height/2
                    new_obj_z = target_env_origins[:, 2] + (selected_heights / 2.0)
                    
                    # Calculate new Z for Goal: Object Top + Radius + Buffer
                    new_goal_z = new_obj_z + (selected_heights / 2.0) + (self.goal_radius + 0.01)
                    ...
                    # 6. Write to Sim (Batch)
                    # We only write the envs we modified
                    self._goal.write_root_pose_to_sim(goal_default_state[move_ball_indices, :7], target_env_ids)
                    self._goal.write_root_velocity_to_sim(torch.zeros((batch_size, 6), device=device), target_env_ids)
                    
                    self._vpt_objects.write_object_pose_to_sim(vpt_obj_default_state[move_ball_indices, :, :7], target_env_ids)
                    self._vpt_objects.write_object_velocity_to_sim(torch.zeros((batch_size, self.num_objs, 6), device=device), target_env_ids)
            ...
            moving_ball_end_time = time.time()
            # if self.verbose >= 1:
            #     print(f"    🕒 Moving ball VPT object time: {moving_ball_end_time - ball_movement_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     current_time = time.time()
            #     elapsed = current_time - reset_internal_start_time
            #     spawn_elapsed = current_time - spawn_start_time
            #     print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["moving_ball_time"][spawn_attempt] = moving_ball_end_time - ball_movement_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["moving_ball_time"][spawn_attempt] = "F"
            ...
            # if self.verbose >= 1:
            #     print(f"    🕒 VPT occlusion placement time: {vpt_occlusion_end_time - vpt_occlusion_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     current_time = time.time()
            #     elapsed = current_time - reset_internal_start_time
            #     spawn_elapsed = current_time - spawn_start_time
            #     print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["vpt_occlusion_time"][spawn_attempt] = vpt_occlusion_end_time - vpt_occlusion_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["vpt_occlusion_time"][spawn_attempt] = "F"
            ...
            # if self.verbose >= 1:
            #     print(f"    🕒 VPT in/out FOV placement time: {vpt_in_out_fov_end_time - vpt_in_out_fov_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     current_time = time.time()
            #     elapsed = current_time - reset_internal_start_time
            #     spawn_elapsed = current_time - spawn_start_time
            #     print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["vpt_in_out_fov_time"][spawn_attempt] = vpt_in_out_fov_end_time - vpt_in_out_fov_start_time
                        env_dict[self.slot_to_env_id[idx]]["vpt_object_placement_time"][spawn_attempt] = vpt_in_out_fov_end_time - vpt_occlusion_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["vpt_in_out_fov_time"][spawn_attempt] = "F"
                        env_dict[self.slot_to_env_id[idx]]["vpt_object_placement_time"][spawn_attempt] = "F"
            ...
            # 1. Vectorize "outside_fov" Mask Creation
            # Assuming visibility_categories is a list of strings matching indices
            # We slice the list first, then compare. 
            # Optimization: Create a full mask first if possible, but this slice is okay for now.
            # Better: Use numpy if available, but list comprehension is fast enough for <512 items
            # We can avoid the loop by converting to array if this list is static per reset.
            # Current approach is OK, but let's ensure we don't move data unnecessarily.
            
            # Identify indices relative to the batch (final_valid_indices)
            # We map global indices to local validity
            current_categories = [visibility_categories[i] for i in final_valid_indices.cpu().tolist()]
            ...
            if outside_fov_mask.any():
                ...
                outside_fov_global_idxs = final_valid_indices[outside_fov_mask]
                ...
                yaw_away = yaw + sample_uniform(
                    math.pi - math.radians(30), math.pi + math.radians(30), (len(outside_fov_global_idxs),), device=device
                )
                ...
                # Update the Master State Tensor (No Write to Sim yet)
                camera_obj_default_state[outside_fov_global_idxs, 3:7] = quaternion_away

            # 3. Single Batch Write to Sim
            # We updated the default state tensor for outside_fov. 
            # Now we write ALL valid cameras (in_view + occluded + outside_fov) in one go.
            # Note: We only write to Sim if we need physics to know about it. 
            self._camera_obj.write_root_pose_to_sim(
                camera_obj_default_state[final_valid_indices, :7],
                final_valid_env_ids
            )

            # 4. Update Occlusion Camera (Sensor)
            # Get latest poses from the tensor we just updated
            ...
            rotated_orientations = math_utils.quat_mul(
                camera_orientations, 
                left_90_quat.unsqueeze(0).expand(len(final_valid_env_ids), -1)
            )
            ...
            # 5. Step Simulation
            # Only step ONCE to apply changes and capture data
            # Note: If you can use self.scene.update(dt=0.0) instead, it's faster.
            for _ in range(1):
                self.sim.step()
                self._occlusion_camera.update(self.sim.cfg.dt)
                # self.sim.render()
                # self._occlusion_camera.update(dt=0.0)
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["camera_posing_time"][spawn_attempt] = camera_writing_end_time - camera_writing_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["camera_posing_time"][spawn_attempt] = "F"
                ...
                if visibility_category in ["in_view", "occluded"]:
                    ...
                    if not occlusion_valid:
                        ...
                        if env_id_item not in env_dict:
                            env_dict[env_id_item] = np.round(time.time() - reset_internal_start_time, 3)
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["occlusion_raycast_time"][spawn_attempt] = occlusion_raycast_end_time - occlusion_raycast_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["occlusion_raycast_time"][spawn_attempt] = "F"
            ...
            # if self.verbose >= 1:
            #     print(f"    🕒 Geometric viewpoint check time: {geometric_check_end_time - geometric_check_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
            #     current_time = time.time()
            #     elapsed = current_time - reset_internal_start_time
            #     spawn_elapsed = current_time - spawn_start_time
            #     print(f"Elapsed time from spawn start: {spawn_elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["geometric_check_time"][spawn_attempt] = geometric_check_end_time - geometric_check_start_time
                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["geometric_check_time"][spawn_attempt] = "F"
            ...
            for idx in range(num_envs):
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                    if envs_need_spawn_retry[idx].item() is False:
                        env_dict[self.slot_to_env_id[idx]]["camera_validation_time"][spawn_attempt] = occlusion_geometric_end_time - occlusion_geometric_start_time
                        env_dict[self.slot_to_env_id[idx]]["final_setup_time"] = np.round(occlusion_geometric_end_time - reset_internal_start_time, 3)

                    if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["camera_validation_time"][spawn_attempt] = "F"
                
                    # Update time before circle validation
                    current_time = time.time()
                    env_dict[self.slot_to_env_id[idx]]["total_time_wo_FOV"] = np.round(current_time - reset_internal_start_time, 3)
        ...
        # if self.verbose >= 1:
        #     print(f"    🕒 Circle viewpoint validation time: {circle_validation_end_time - circle_validation_start_time:.3f} seconds for spawn_attempt = {spawn_attempt}")
        #     current_time = time.time()
        #     elapsed = current_time - reset_internal_start_time
        #     print(f"Elapsed time from reset internal start: {elapsed:.3f} seconds for spawn_attempt = {spawn_attempt}")
        ...
        for idx in range(num_envs):
            if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "incomplete":
                if envs_need_spawn_retry[idx].item() is False:
                    env_dict[self.slot_to_env_id[idx]]["circle_validation_time"][spawn_attempt] = circle_validation_end_time - circle_validation_start_time
                if envs_need_spawn_retry[idx].item() is True:
                        env_dict[self.slot_to_env_id[idx]]["circle_validation_time"][spawn_attempt] = "F"
            elif env_dict[self.slot_to_env_id[idx]]["completion_status"] == "complete":
                env_dict[self.slot_to_env_id[idx]]["circle_validation_time"][spawn_attempt] = circle_validation_end_time - circle_validation_start_time
        ...
        for idx in range(self.num_envs):
            if envs_need_spawn_retry[idx].item() is True:
                ...
                if env_dict[self.slot_to_env_id[idx]]["completion_status"] != "complete": 
                    env_dict[self.slot_to_env_id[idx]]["completion_status"] = "complete"
                    if env_dict[self.slot_to_env_id[idx]]["total_time"] == 0:
                        env_dict[self.slot_to_env_id[idx]]["total_time"] = np.round(reset_internal_end_time - reset_internal_start_time, 3)
                # Round off times to 4 decimal places for readability
            for key in ["writing_spawn_pose_time", "moving_ball_time", "camera_posing_time", 
                            "vpt_occlusion_time", "vpt_in_out_fov_time",
                            "vpt_object_placement_time",
            ...
            if env_dict[self.slot_to_env_id[idx]]["completion_status"] == "complete":
                env_dict[self.slot_to_env_id[idx]]["last_attempt_setup_time"] = (
                    list(env_dict[self.slot_to_env_id[idx]]["writing_spawn_pose_time"].values())[-1] +
                    list(env_dict[self.slot_to_env_id[idx]]["moving_ball_time"].values())[-1] +
                    list(env_dict[self.slot_to_env_id[idx]]["camera_posing_time"].values())[-1] +
                    list(env_dict[self.slot_to_env_id[idx]]["vpt_object_placement_time"].values())[-1] +
                    list(env_dict[self.slot_to_env_id[idx]]["occlusion_raycast_time"].values())[-1] +
                    list(env_dict[self.slot_to_env_id[idx]]["camera_validation_time"].values())[-1]
                )
                
                # Round up
                env_dict[self.slot_to_env_id[idx]]["final_setup_time"] = np.round(env_dict[self.slot_to_env_id[idx]]["final_setup_time"], 3)
                env_dict[self.slot_to_env_id[idx]]["last_attempt_setup_time"] = np.round(env_dict[self.slot_to_env_id[idx]]["last_attempt_setup_time"], 3)
        ...
        for key, value in env_dict.items():
            ...
            if isinstance(value, dict):
                for k, v in value.items():
                    print(f"  {k}: {v}")
        print(f"-"*50)
        print(f"Total reset time for spawn attempts = {spawn_attempt}: {reset_internal_end_time - reset_internal_start_time:.3f} seconds")
        print("\n")
    ...
    def _is_point_valid_batch(self, points: torch.Tensor, env_ids: torch.Tensor, 
        ...
        if isinstance(self.active_vpt_indices, list):
            if len(self.active_vpt_indices) > 0 and isinstance(self.active_vpt_indices[0], torch.Tensor):
                ...
                active_indices_tensor = torch.stack(self.active_vpt_indices).to(dtype=torch.long, device=device)
            else:
        ...
        batch_active_indices_expanded = batch_active_indices.unsqueeze(-1).expand(-1, -1, 2)
        active_vpt_positions = torch.gather(all_vpt_pos, 1, batch_active_indices_expanded)

        # Calculate Distances: |point - vpt_obj|
        # points: [num_points, 2] -> [num_points, 1, 2]
        # active_vpt_positions: [num_points, active_objs, 2]
        distances_to_vpt = torch.norm(points.unsqueeze(1) - active_vpt_positions, dim=2)
        ...
        camera_vpt_distances = torch.norm(camera_positions.unsqueeze(1) - active_vpt_positions, dim=2)
        ...
        for point_idx in range(max_points_per_env):
            ...
            for _ in range(1):
                ...
                self._vpt_objects.update(self.sim.cfg.dt)
            # self.sim.render()
            # self._tiled_camera.update(dt=0.0)
            # self._agent.update(dt=0.0)
    ...
    # def randomize_shape_scale(self, prim_path_expr: str | list):
    #     """Randomize the scale of the geometry, accounting for unit-scale multipliers."""
    #     # get world instance
    ...
    #     # (X, Y, Z)
    #     scaling_map = {
    #         "s": self.cross_scaling_factors, # Ends in 's' -> Cross
    #         "L": self.l_scaling_factors,     # Ends in 'L' -> L-Shape
    #         "T": self.t_scaling_factors      # Ends in 'T' -> T-Shape
    #     }
        
    #     # Default fallback (1.0, 1.0, 1.0)
    ...
    #             if prim.IsValid():
    #                 # --- 1. Detect Shape Type from Reference ---
    #                 multiplier = default_scale
                    
    #                 # Iterate through the PrimStack to find the reference asset path
    #                 for spec in prim.GetPrimStack():
    #                     if spec.hasReferences:
    #                         refs = spec.referenceList.GetAddedOrExplicitItems()
    #                         if refs:
    #                             # Get the asset path (e.g., ".../Cross.usd")
    #                             asset_path = refs[0].assetPath
    #                             filename = asset_path.split('/')[-1]
    #                             # Logic: Get last letter of the filename (excluding extension)
    #                             # e.g. "Cross.usd" -> "Cross" -> "s"
    #                             try:
    #                                 first_word_last_letter = filename.split('.')[0][-1]
    #                                 if first_word_last_letter in scaling_map:
    #                                     multiplier = scaling_map[first_word_last_letter]
    #                             except Exception:
    #                                 pass # Keep default
    #                             break 

    #                 # --- 2. Apply Randomization + Multiplier ---
    ...
    #                 # Uniform random float for XY (0.5 to 1.5)
    #                 rand_xy = random.uniform(0.5, 1.5)
    #                 # Uniform random float for Z (0.8 to 1.2)
    #                 rand_z = random.uniform(0.8, 1.2)

    #                 # Calculate final scale: Random_Variation * Unit_Multiplier
    #                 # Note: We use rand_xy for both X and Y to maintain the aspect ratio 
    #                 # of the random noise, but apply the specific shape multipliers.
    #                 final_x = rand_xy * multiplier[0]
    #                 final_y = rand_xy * multiplier[1]
    #                 final_z = rand_z * multiplier[2]

    #                 scale_value = Gf.Vec3d(final_x, final_y, final_z)
    ...
    def randomize_shape_scale(self, prim_path_expr: str | list):
        """
        Randomize geometry scale and update 'self.all_vpt_dims' with the ACTUAL dimensions,
        using the provided lookup table logic to determine base asset sizes.
        """
        # --- 1. Setup World & Stage ---
        world = World.instance()
        if world.is_playing():
        ...
        # --- 2. Cache Base Dimensions (UPDATED LOGIC) ---
        if not hasattr(self, "vpt_base_dims"):
            ...
            for key in vpt_keys:
                ...
                # --- CASE A: Primitives (Box, Cylinder, etc) ---
                if isinstance(spawn_cfg, sim_utils.CuboidCfg):
                    dims = torch.tensor(spawn_cfg.size, device=self.device)
                ...
                # --- CASE B: USD Files (UPDATED) ---
                elif isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                    # ---------------------------------------------------------
                    # CHANGE 1: Always grab the config scale first.
                    # This ensures we respect the size defined in the config, 
                    # even if it isn't (1,1,1).
                    # ---------------------------------------------------------
                    scale = spawn_cfg.scale if hasattr(spawn_cfg, "scale") else (1.0, 1.0, 1.0)
                    
                    # Convert to mutable float variables
                    x_dim, y_dim, z_dim = scale[0], scale[1], scale[2]
                    
                    # Attempt to apply shape-specific logic
                    path = spawn_cfg.usd_path
                    ...
                    try:
                        ...
                        # If parsing fails, we keep x_dim/y_dim/z_dim as the config scale
                        pass
                    
                    self.vpt_base_dims.append(torch.tensor([x_dim, y_dim, z_dim], device=self.device))
                    
                else:
                    # Fallback for unknown types (still default to 1s if no scale attr exists)
                    self.vpt_base_dims.append(torch.tensor([1.0, 1.0, 1.0], device=self.device))
                    
            self.vpt_base_dims = torch.stack(self.vpt_base_dims) # Shape: [num_objs, 3]

        # --- 3. Initialize/Reset Tensor Storage ---
        if not hasattr(self, "all_vpt_dims") or self.all_vpt_dims.shape[0] != self.num_envs:
            self.all_vpt_dims = torch.zeros(
                (self.num_envs, self.cfg.num_vpt_objs, 3), 
                device=self.device
            )

        # --- 4. Resolve Prim Paths ---
        if isinstance(prim_path_expr, str):
            prim_paths = sim_utils.find_matching_prim_paths(prim_path_expr)
        elif isinstance(prim_path_expr, list):
            prim_paths = []
        ...
        # --- 5. Define Scaling Maps ---
        scaling_map = {
            "s": self.cross_scaling_factors, "L": self.l_scaling_factors, "T": self.t_scaling_factors
        }
        default_scale = (1.0, 1.0, 1.0)
            ...
            for prim_path in prim_paths:
                ...
                for spec in prim.GetPrimStack():
                    if spec.hasReferences:
                        refs = spec.referenceList.GetAddedOrExplicitItems()
                        if refs:
                            filename = refs[0].assetPath.split('/')[-1]
                            try:
                                f_let = filename.split('.')[0][-1]
                                if f_let in scaling_map:
                                    multiplier = scaling_map[f_let]
                            except: pass
                            break 

                # C. Generate Random Scale
                rand_xy = random.uniform(0.5, 1.5)
                rand_z = random.uniform(0.8, 1.2)
                ...
                final_scale_vec = Gf.Vec3d(final_x, final_y, final_z)
                ...
                # E. Calculate & Store ACTUAL Dimensions
                if env_idx is not None and obj_idx is not None:
                    base_dims = self.vpt_base_dims[obj_idx]
                    
                    # Actual Dims = Base_Dims (from config/lookup) * Applied_Scale (random)
                    # ---------------------------------------------------------
                    # Note: Because base_dims already divides by 'multiplier' (in Step 2)
                    # and final_x multiplies by 'multiplier' (in Step C),
                    # the result is essentially: Config_Scale * Random_Noise
                    # ---------------------------------------------------------
                    self.all_vpt_dims[env_idx, obj_idx, 0] = base_dims[0] * final_x
                    self.all_vpt_dims[env_idx, obj_idx, 1] = base_dims[1] * final_y
                    self.all_vpt_dims[env_idx, obj_idx, 2] = base_dims[2] * final_z

        # --- 7. Restart Physics ---
        world.play()

    def randomize_shape_color(self, prim_path_expr: str | list, random_roughness: bool = False, random_metallic: bool = False):
        """
        ...
        with Sdf.ChangeBlock():
            for prim_path in prim_paths:
                ...
                def _set_shader_attr(shader_spec, attr_name, value, type_name):
                    ...
                    attr_spec = shader_spec.GetAttributeAtPath(shader_spec.path.AppendProperty(attr_name))
                    if not attr_spec:
                ...
                color_check = check_prim_spec.GetAttributeAtPath(standard_shader_path + ".inputs:diffuseColor")
                ...
                if target_shader_path:
                    ...
                    _set_shader_attr(shader_spec, color_attr_name, rand_color, Sdf.ValueTypeNames.Color3f)
    ...
    def randomize_sphere_lights(self, light_names: list = ["Light_A", "Light_B"], z_heights: list =[8.5, 6.5]):
        """
        Randomizes the (x, y) position of local sphere lights within the environment boundaries.
        Z height is kept fixed.
        
        Args:
            z_height (float): The fixed height for the light (e.g., 3.5m).
        """
        # Get the stage
        stage = get_current_stage()
        
        # We use a safety margin (e.g., 80% of the full width) so the light 
        # doesn't spawn exactly inside the wall.
        limit = self.center_to_boundary * 0.8 

        # Loop through every environment to move its specific light
        for i in range(0, self.num_envs):
            for light_name, z_height in zip(light_names, z_heights):
                # 1. Construct the path dynamically
                # Regex like "env_.*" doesn't work for direct stage access, 
                # so we build the specific path: /World/envs/env_0/Light
                prim_path = f"/World/envs/env_{i}/{light_name}"
                
                prim = stage.GetPrimAtPath(prim_path)
                if not prim.IsValid():
                    # If you are debugging, you might print here, but usually skip
                    continue

                # 2. Generate Random Local Positions
                # Since 'Light' is a child of 'env_{i}', (0,0) is the center of the room.
                rand_x = random.uniform(-limit, limit)
                rand_y = random.uniform(-limit, limit)
                
                # 3. Apply Translation (Move the light)
                xform = UsdGeom.Xformable(prim)
                
                # Check if a translation operation already exists to update it,
                ...
                # Ensure rand_x and rand_y are python floats, not tensors
                x_val = rand_x.item() if hasattr(rand_x, 'item') else float(rand_x)
                y_val = rand_y.item() if hasattr(rand_y, 'item') else float(rand_y)

                # Update the position
                # Gf.Vec3d requires (x, y, z) doubles
                translate_op.Set(Gf.Vec3d(x_val, y_val, z_height))

    def _cache_z_offsets(self):
        """
        Creates a tensor of Z-offset ratios based on object type.
        Primitives (Center Origin) = 0.5
        USD Assets (Base Origin)   = 0.0 (Change to 0.5 if your USDs are center-pivoted)
        """
        offsets = []
        vpt_keys = list(self.cfg.vpt_objects.rigid_objects.keys())
        ...
        for key in vpt_keys:
            ...
            if isinstance(spawn_cfg, (sim_utils.CuboidCfg, sim_utils.CylinderCfg, sim_utils.ConeCfg)):
                # Primitives are centered -> Add half height
                offsets.append(0.5)
            elif isinstance(spawn_cfg, sim_utils.UsdFileCfg):
                # USDs usually have origin at bottom -> Add 0 height (just place at floor)
                # If your specific USDs are sinking, change this to 0.5!
                offsets.append(0.0) 
            else:
                offsets.append(0.5)
