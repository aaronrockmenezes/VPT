...
class VPTEnv(DirectRLEnv):
    ...
    def _reset_idx(self,
        ...
        for i in range(num_envs):
            ...
            target_occluded = True
            target_outside_fov = False
            is_goal_occluded = False
            is_goal_outside_fov = False
            
            while not valid_positions and attempts < max_attempts and (is_goal_occluded != target_occluded or is_goal_outside_fov != target_outside_fov):
        ...
        for i, env_id in enumerate(env_ids):
            ...
            if valid_points_2d.shape[0] > 0:
                ...
                if self.verbose >= 1:
                    print(f"  Env {env_id_item}: Found {valid_points_2d.shape[0]} valid viewpoint poses")
            else:
                self.valid_viewpoint_poses[env_id_item] = torch.zeros((0, 3), device=device)
                if self.verbose >= 1:
                    print(f"  Env {env_id_item}: WARNING - No valid viewpoint poses found!")
    ...
    def _check_occlusion_raycast(self,
        ...
        # Not enough red pixels means occluded
        if red_count < GOAL_THRESHOLD:
            return True
        # Enough red pixels means visible
        return False
