...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: VPTEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.base_path = f"/oscar/scratch/arock3/VPTnav_data/v17_depth_NEW/data_node{self.NODE_ID}_gpu{self.GPU_ID}"
    ...
    def _teleport_and_step(self, env_ids, points):
        ...
        for _ in range(1):
            ...
            self._rgb_tiled_camera.update(self.sim.cfg.dt, force_recompute=True)
    ...
    def generate_valid_circle_points(
        ...
        # ==== ADDED: Dynamic Center Calculation (±20% of Midpoint) ====
        direction_vectors = goal_pos - camera_pos
        # t is a random scalar between 0.3 and 0.7
        t = (torch.rand(num_envs, 1, device=device) * 0.4) + 0.3
        centers = camera_pos + t * direction_vectors
        # ==============================================================

        # ==== MODIFIED: Anchor the circle to `centers` instead of `goal_pos` ====
        ...
        for i, env_id in enumerate(env_ids):
            ...
            # ==== ADDED: Extract the specific randomized center for sorting ====
            center_2d = centers[i]
            # ===================================================================
            ...
            # ==== MODIFIED: Swapped to a radial deadzone to avoid sliding distortions ====
            ...
            # --- APPLY GREEDY FILTER ---
            # ==== MODIFIED: Sorting the pools relative to the new dynamic center ====
