...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: VPTEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.base_path = f"/oscar/scratch/arock3/VPTnav_data/data_{self.GPU_ID}"
    ...
    def _save_env_config_to_json_new(self, env_id: int, folder_idx: int):
        ...
        # Default to empty dicts if something bypassed the cache
        ...
        for local_idx, obj_idx in enumerate(active_indices):
            ...
            vpt_objects_data.append(vpt_obj)
        ...
        config = {
            ...
            "environment_aesthetics": {
                "floor_material_path": floor_texture
            },
    ...
    def initial_spawn_loop(self,
        ...
        safe_x_range = safe_range - 2.0
        safe_x_range_obstacles = float(safe_range - 1.5)
        env_origins = self.scene.env_origins[global_retry_env_ids]
    ...
    def generate_valid_circle_points(
        ...
        enforcing a 50/50 split between camera-proximal and goal-proximal points
        with a 0.2m deadzone.
        """
        device = self.device
        num_envs = len(env_ids)
        ...
        # Step 2: Vectorized displacement filtering & Proximity Pooling
        ...
        # Helper function for greedy spatial filtering
        ...
        for i, env_id in enumerate(env_ids):
            ...
            # Enforce 0.2m deadzone
            valid_dist_mask = torch.abs(dist_cam - dist_goal) >= 0.2
            ...
            # --- SEGREGATE POOLS ---
            pool_a_mask = dist_cam < dist_goal
            pool_b_mask = dist_goal < dist_cam

            pool_a_points = valid_points[pool_a_mask]
            pool_b_points = valid_points[pool_b_mask]

            # --- APPLY GREEDY FILTER ---
            filtered_a = filter_pool(pool_a_points, goal_pos_2d)
            filtered_b = filter_pool(pool_b_points, goal_pos_2d)

            num_a = len(filtered_a)
            num_b = len(filtered_b)

            # Check if we have enough in BOTH pools to hit our FOV minimum targets
            if num_a < min_half_candidates or num_b < min_half_candidates:
                if self.verbose >= 1:
                    print(
                        f"  Env {env_id_item}: ❌ Failed 50/50 split criteria (Cam-proximal: {num_a}, "
                        f"Goal-proximal: {num_b}, need {min_half_candidates} each). Skipping FOV check."
                    )
                continue

            # Recombine the filtered candidates
            filtered_candidates = torch.cat([filtered_a, filtered_b], dim=0)

            if self.verbose >= 2:
                print(
                    f"  Env {env_id_item}: Split filter passed. A:{num_a}, B:{num_b} "
        ...
        # Re-enable the standard min_required_points since early stop is now dual-quota
        ...
        for i, env_id in enumerate(env_ids):
            ...
            # Because _check_fov_validity caps exactly at MIN_REQUIRED_POINTS (10 C / 10 G)
            # any environment that succeeds will have exactly this amount.
    ...
    def randomize_material(self, prim_paths: list, material_type: str):
        ...
        #         if "textures" not in self.env_metadata_cache[env_idx]:
        #             self.env_metadata_cache[env_idx]["textures"] = {"vpt_objects": {}, "floor": "unknown"}

        #         # Save path based on type
        #         if material_type == "mat":
        #             self.env_metadata_cache[env_idx]["textures"]["floor"] = rand_material.mdl_path
        #         elif material_type == "vpt":
        #             obj_match = re.search(r"obs_(\d+)", prim_path)
        #             if obj_match:
        #                 obj_idx = int(obj_match.group(1))
        #                 self.env_metadata_cache[env_idx]["textures"]["vpt_objects"][obj_idx] = rand_material.mdl_path
