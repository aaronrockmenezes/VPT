...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # Rolling window configuration - TODO: Implement rolling window logic. Modify init properly later.
        self.total_target_envs = 100  # Total environments to collect
        self.active_pool_size = self.num_envs  # Use all available sim envs (16)
        self.max_attempts_per_env = 50  # Max spawn attempts before giving up
        
        # Pre-allocate all environment configurations
        print(f"\n{'='*80}")
        print(f"🎲 INITIALIZING ROLLING WINDOW DATA COLLECTION")
        print(f"   Active Pool: {self.active_pool_size} environments")
        print(f"   Target Total: {self.total_target_envs} environments")
        print(f"{'='*80}\n")
        
        self.env_queue = self._preallocate_env_configs(self.total_target_envs)
        
        # Slot tracking
        self.active_slot_to_global_env = torch.full(
            (self.active_pool_size,), -1, dtype=torch.long, device=self.device
        )  # -1 = unassigned
        
        self.slot_attempt_counters = torch.zeros(
            self.active_pool_size, dtype=torch.long, device=self.device
        )
        
        self.slot_spawn_status = torch.zeros(
            self.active_pool_size, dtype=torch.bool, device=self.device
        )  # True = spawned successfully, ready for validation
        
        # Global tracking
        self.global_env_completed = torch.zeros(
            self.total_target_envs, dtype=torch.bool, device=self.device
        )
        self.next_env_to_assign = 0
        self.num_completed_envs = 0
        self.num_failed_envs = 0
        
        # Performance tracking
        self.start_time = time.time()
        self.last_progress_print = time.time()
        self.progress_print_interval = 60.0  # Print every 60 seconds
        
        # Initialize first batch (only in data collection mode)
        if self.mode == "data_collection":
            self._initialize_rolling_window()
        
        self.envs_to_move_ball = torch.tensor([], dtype=torch.long, device=self.device)
    ...
    def _get_active_vpt_dims(self, env_id) -> torch.Tensor:
        ...
        for local_idx, obj_idx in enumerate(active_indices):
            try:
                ...
                vpt_size = self._vpt_objects.cfg.rigid_objects[vpt_keys[obj_idx]].spawn.scale / 10.0
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None, randomize_objects: bool = True) -> None:
        """Rolling window reset: continuously process envs, replace completed ones.
        
        Args:
            env_ids: Environment IDs (typically all envs, but can be subset)
            randomize_objects: Whether to randomize object spawning (always True for data collection)
        """
        
        # Testing mode uses config file loading (unchanged)
        ...
        # Get active slots (slots with assigned envs, not -1)
        active_mask = self.active_slot_to_global_env >= 0
        active_slots = torch.where(active_mask)[0]
        
        # Check if all slots are done
        if not active_slots.numel():
            print("\n🎉 ALL SLOTS COMPLETED - COLLECTION FINISHED!\n")
            self._print_final_summary()
            self.close()
            exit(0)
        
        # Separate slots by spawn status
        needs_spawn_mask = ~self.slot_spawn_status[active_slots]
        needs_spawn_slots = active_slots[needs_spawn_mask]
        needs_validation_slots = active_slots[~needs_spawn_mask]
        
        # Phase 1: Attempt spawn for slots that need it
        if needs_spawn_slots.numel() > 0:
            self._attempt_spawn_batch(needs_spawn_slots)
        
        # Phase 2: Validate and collect for successfully spawned slots
        if needs_validation_slots.numel() > 0:
            completed_slots = self._validate_and_collect_batch(needs_validation_slots)
            
            # Phase 3: Immediately replace completed slots
            for slot_idx in completed_slots:
                self._finalize_and_reassign_slot(slot_idx)
        
        # Phase 4: Handle slots that exceeded max attempts
        self._handle_failed_slots()
        
        # Phase 5: Print progress periodically
        if time.time() - self.last_progress_print > self.progress_print_interval:
            self._print_progress_dashboard()
            self.last_progress_print = time.time()
...
# New methods
    def _preallocate_env_configs(self, total_envs: int) -> List[Dict]:
        """Pre-allocate visibility labels for all environments upfront.
        
        Args:
            total_envs: Total number of environments to pre-allocate
            
        Returns:
            List of config dicts with pre-assigned visibility categories
        """
        print(f"\n{'='*80}")
        print(f"🎲 PRE-ALLOCATING {total_envs} ENVIRONMENT CONFIGURATIONS")
        print(f"{'='*80}\n")
        
        # Determine split
        num_in_view = total_envs // 2
        num_occluded = total_envs // 4
        num_outside_fov = total_envs - num_in_view - num_occluded
        ...
        # Pre-allocate configs
        env_queue = []
        for global_env_idx in range(total_envs):
            category = visibility_categories[global_env_idx]
            
            config = {
                "global_env_id": global_env_idx,
                "folder_idx": global_env_idx,
                "visibility_category": category,
                "visibility_label": "Yes" if category == "in_view" else "No",
                "visibility_reason": category,
                "attempts": 0,
            ...
            env_queue.append(config)
            
            # Pre-save visibility labels
            self.env_visibility_labels[global_env_idx] = config["visibility_label"]
            self.env_visibility_reasons[global_env_idx] = config["visibility_reason"]
        
        self._save_visibility_labels()
        
        print(f"✅ Pre-allocated {total_envs} configs:")
        print(f"   • {num_in_view} in_view ({num_in_view/total_envs*100:.1f}%)")
        print(f"   • {num_occluded} occluded ({num_occluded/total_envs*100:.1f}%)")
        print(f"   • {num_outside_fov} outside_fov ({num_outside_fov/total_envs*100:.1f}%)\n")
        
        return env_queue
    
    def _initialize_rolling_window(self) -> None:
        """Fill all active slots with initial environment configs.
        
        Should be called after _preallocate_env_configs in __init__.
        """
        
        print(f"🚀 Initializing {self.active_pool_size} active slots...\n")
        
        for slot_idx in range(self.active_pool_size):
            success = self._assign_env_to_slot(slot_idx)
            if not success:
                print(f"  ⚠️  Warning: Could not assign env to slot {slot_idx}")
                break
        
        assigned_count = (self.active_slot_to_global_env >= 0).sum().item()
        print(f"✅ Initialized {assigned_count}/{self.active_pool_size} slots\n")
    

    def _assign_env_to_slot(self, slot_idx: int) -> bool:
        """Assign next pending env config from queue to given slot.
        
        Args:
            slot_idx: Slot index to assign env to
            
        Returns:
            True if assignment successful, False if no more envs in queue
        """
        
        if self.next_env_to_assign >= self.total_target_envs:
            if self.verbose >= 1:
                print(f"  ⚠️  Slot {slot_idx}: No more envs in queue")
            self.active_slot_to_global_env[slot_idx] = -1  # Mark as inactive
            return False
        
        global_env_id = self.next_env_to_assign
        config = self.env_queue[global_env_id]
        
        # Update mappings
        self.active_slot_to_global_env[slot_idx] = global_env_id
        config["status"] = "in_progress"
        config["start_time"] = time.time()
        self.slot_attempt_counters[slot_idx] = 0
        self.slot_spawn_status[slot_idx] = False
        
        # Increment queue pointer
        self.next_env_to_assign += 1
        
        if self.verbose >= 2:
            print(f"  📋 Slot {slot_idx} ← Global Env {global_env_id} ({config['visibility_category']})")
        
        return True
    
    def _attempt_spawn_batch(self, slot_indices: torch.Tensor) -> None:
        """Attempt to spawn objects for given slots in parallel.
        
        Uses existing _reset_idx_internal for actual spawning logic.
        
        Args:
            slot_indices: Tensor of slot indices to attempt spawn for
        """
        
        if not slot_indices.numel():
            return
        
        # Map slots to global env IDs
        global_env_ids = self.active_slot_to_global_env[slot_indices]
        
        # Filter out inactive slots (-1)
        active_mask = global_env_ids >= 0
        if not active_mask.any():
            return
        
        slot_indices = slot_indices[active_mask]
        global_env_ids = global_env_ids[active_mask]
        
        # Get configs for these envs
        folder_indices = [self.env_queue[gid.item()]["folder_idx"] for gid in global_env_ids]
        visibility_categories = [self.env_queue[gid.item()]["visibility_category"] for gid in global_env_ids]
        
        # ========== ADD THIS SECTION ==========
        # Randomly select 25% of these slots to have goal on top of object
        num_to_select = max(1, int(0.25 * len(slot_indices)))
        selected_indices = torch.randperm(len(slot_indices), device=self.device)[:num_to_select]
        self.envs_to_move_ball = slot_indices[selected_indices]
        # ======================================
        
        if self.verbose >= 2:
            print(f"\n🔨 Attempting spawn for {len(slot_indices)} slots...")
        
        # Call existing spawn logic
        try:
            ...
            # Mark as successfully spawned (ready for validation)
            self.slot_spawn_status[slot_indices] = True
            
            if self.verbose >= 2:
                print(f"✅ Spawn successful for {len(slot_indices)} slots\n")
                
        except Exception as e:
            print(f"❌ Spawn failed for batch: {e}")
            # Mark these slots for retry
            self.slot_spawn_status[slot_indices] = False
        
        # Increment attempt counters
        self.slot_attempt_counters[slot_indices] += 1
        
        # Update attempts in queue
        for slot_idx in slot_indices:
            global_env_id = self.active_slot_to_global_env[slot_idx.item()].item()
            if global_env_id >= 0:
                self.env_queue[global_env_id]["attempts"] = self.slot_attempt_counters[slot_idx.item()].item()
    
    
    def _validate_and_collect_batch(self, slot_indices: torch.Tensor) -> List[int]:
        """Validate slots and collect data for those that pass validation.
        
        Args:
            slot_indices: Tensor of slot indices to validate
            
        Returns:
            List of slot indices that completed successfully
        """
        
        completed_slots = []
        
        for slot_idx in slot_indices:
            slot_idx_item = slot_idx.item()
            global_env_id = self.active_slot_to_global_env[slot_idx_item].item()
            
            if global_env_id < 0:
                continue
            
            folder_idx = self.env_queue[global_env_id]["folder_idx"]
            
            # ========== FIX: Pass slot_idx as tensor, not global_env_id ==========
            # Create a proper env_id tensor for validation
            slot_idx_tensor = torch.tensor([slot_idx_item], dtype=torch.long, device=self.device)
            
            # Validate this environment
            is_valid, reason = self._validate_env_state(
                slot_idx_tensor[0],  # Pass the slot index (which is the env_id in the simulation)
                folder_idx,
                self.images_per_env
            )
            # =====================================================================
            ...
            if is_valid:
                # ✅ PASSED! Collect data immediately
                if self.verbose >= 1:
                    elapsed = time.time() - self.env_queue[global_env_id]["start_time"]
                    print(f"\n✅ Slot {slot_idx_item} (Global {global_env_id}) PASSED in {elapsed:.1f}s")
                
                # Collect images for this slot
                self._collect_images_for_slot(slot_idx)
                
                # Save config
                self._save_env_config_to_json(slot_idx, folder_idx)
                
                # Mark as completed
                self.env_queue[global_env_id]["status"] = "completed"
                self.env_queue[global_env_id]["end_time"] = time.time()
                self.global_env_completed[global_env_id] = True
                
                completed_slots.append(slot_idx_item)
            # In _validate_and_collect_batch, after the validation check:
            elif not is_valid:
                if self.verbose >= 1:
                    print(f"  ⚠️  Slot {slot_idx_item} (Global {global_env_id}) validation failed: {reason}")
                    num_viewpoints = 0
                    if (self.valid_viewpoint_poses is not None and 
                        slot_idx_item < len(self.valid_viewpoint_poses) and 
                        self.valid_viewpoint_poses[slot_idx_item] is not None):
                        num_viewpoints = len(self.valid_viewpoint_poses[slot_idx_item])
                    print(f"       Has {num_viewpoints} viewpoints (need {self.images_per_env})")
                self.slot_spawn_status[slot_idx_item] = False
        
        return completed_slots

    def _collect_images_for_slot(self, slot_idx: torch.Tensor) -> None:
        """Collect all 20 images for a single slot by teleporting through viewpoints.
        
        Args:
            slot_idx: Slot index to collect images for (can be int or tensor)
        """
        
        # Convert to int if tensor
        if torch.is_tensor(slot_idx):
            slot_idx_item = slot_idx.item()
        else:
            slot_idx_item = slot_idx
        
        global_env_id = self.active_slot_to_global_env[slot_idx_item].item()
        
        # Check if we have valid viewpoints
        if (self.valid_viewpoint_poses[slot_idx_item] is None or 
            len(self.valid_viewpoint_poses[slot_idx_item]) < self.images_per_env):
            print(f"  ⚠️  Slot {slot_idx_item}: Insufficient viewpoints for collection")
            return
        
        # Select diverse viewpoints using greedy distance selection
        all_viewpoints = self.valid_viewpoint_poses[slot_idx_item]
        selected_points = [all_viewpoints[0]]
        ...
        if len(selected_points) < self.images_per_env:
            print(f"  ⚠️  Slot {slot_idx_item}: Only {len(selected_points)} diverse viewpoints available")
            return
        
        viewpoints = torch.stack(selected_points)
        
        if self.verbose >= 2:
            print(f"  📸 Collecting {self.images_per_env} images for Slot {slot_idx_item}...")
        
        # Collect images at each viewpoint
        device = self.device
        slot_tensor = torch.tensor([slot_idx_item], dtype=torch.long, device=device)
        
        for vp_idx in range(self.images_per_env):
            target_pos = viewpoints[vp_idx]
            
            # Build 3D position for agent
            target_pos_3d = torch.zeros(3, device=device)
            target_pos_3d[:2] = target_pos[:2]
            target_pos_3d[2] = self._agent.data.default_root_state[slot_idx_item, 2]
            
            # Calculate orientation (look at midpoint between camera and goal)
            camera_pos_3d = self._camera_obj.data.root_pos_w[slot_idx_item]
            goal_pos_3d = self._goal.data.root_pos_w[slot_idx_item]
            midpoint = (camera_pos_3d + goal_pos_3d) / 2.0
            direction = midpoint[:2] - target_pos[:2]
            yaw = torch.atan2(direction[1], direction[0]) if torch.norm(direction) > 1e-6 else torch.tensor(0.0, device=device)
            ...
            # Teleport agent to viewpoint
            pose = torch.cat([target_pos_3d, quat])
            self._agent.write_root_com_pose_to_sim(pose.unsqueeze(0), slot_tensor)
            
            # Update simulation and cameras
            for _ in range(3):
                self.sim.step()
                self._rgb_tiled_camera.update(self.sim.cfg.dt)
                self._distance_tiled_camera.update(self.sim.cfg.dt)
            ...
            # Save this image
            self._save_single_image(slot_idx_item, global_env_id, vp_idx, rgb_data, depth_data, camera_pov_data)
        
        if self.verbose >= 2:
            print(f"  ✅ Slot {slot_idx_item}: Collected {self.images_per_env} images")

    def _save_single_image(self, slot_idx: int, global_env_id: int, image_idx: int,
                       rgb_data: torch.Tensor, depth_data: torch.Tensor, 
                       camera_pov_data: torch.Tensor = None) -> None:
        """Save a single RGB/depth/camera image for given slot and viewpoint index.
        
        Args:
            slot_idx: Slot index (for indexing into data tensors)
            global_env_id: Global environment ID (for folder naming)
            image_idx: Image index within this environment (0-19)
            rgb_data: RGB tensor from camera
            depth_data: Depth tensor from camera
            camera_pov_data: Optional camera POV semantic segmentation data
        """
        
        folder_idx = self.env_queue[global_env_id]["folder_idx"]
        visibility_label = self.env_queue[global_env_id]["visibility_label"]
        ...
        # Save Camera POV (only once - at image_idx 0)
        if self.save_camera_pov and camera_pov_data is not None and image_idx == 0:
            cam_pov_filename = f"{cam_env_folder}/cam_pov.png"
            cam_pov_img = camera_pov_data[slot_idx, :, :, :3]
            if cam_pov_img.max() <= 1.0:
                cam_pov_np = (cam_pov_img.cpu().numpy() * 255.0).astype(np.uint8)
            else:
                cam_pov_np = cam_pov_img.cpu().numpy().astype(np.uint8)
    ...
    def _finalize_and_reassign_slot(self, slot_idx: int) -> None:
        """Finalize completed slot and assign next env from queue.
        
        Args:
            slot_idx: Slot index that just completed
        """
        
        global_env_id = self.active_slot_to_global_env[slot_idx].item()
        
        # Increment global completion counter
        self.num_completed_envs += 1
        
        # Calculate stats
        duration = self.env_queue[global_env_id]["end_time"] - self.env_queue[global_env_id]["start_time"]
        attempts = self.env_queue[global_env_id]["attempts"]
        
        print(f"\n{'='*80}")
        print(f"✅ COMPLETED: Global Env {global_env_id} (Slot {slot_idx})")
        print(f"   Duration: {duration:.1f}s | Attempts: {attempts}")
        print(f"   Progress: {self.num_completed_envs}/{self.total_target_envs} ({self.num_completed_envs/self.total_target_envs*100:.1f}%)")
        print(f"{'='*80}\n")
        
        # Assign next env from queue to this slot
        assigned = self._assign_env_to_slot(slot_idx)
        
        if not assigned:
            print(f"  ℹ️  Slot {slot_idx} retired (no more envs in queue)")
        
        # Check if we've reached the target
        if self.num_completed_envs >= self.total_target_envs:
            self._print_final_summary()
            self.close()
            exit(0)
    

    def _handle_failed_slots(self) -> None:
        """Handle slots that exceeded max attempts - mark as failed and reassign.
        
        Checks all active slots and reassigns any that have exceeded max_attempts_per_env.
        """
        
        # Find slots that exceeded max attempts and are still active
        failed_mask = (self.slot_attempt_counters >= self.max_attempts_per_env) & \
                    (self.active_slot_to_global_env >= 0)
        failed_slots = torch.where(failed_mask)[0]
        
        if not failed_slots.numel():
            return
        
        for slot_idx in failed_slots:
            slot_idx_item = slot_idx.item()
            global_env_id = self.active_slot_to_global_env[slot_idx_item].item()
            ...
            duration = time.time() - self.env_queue[global_env_id]["start_time"]
            attempts = self.slot_attempt_counters[slot_idx_item].item()
            
            print(f"\n{'='*80}")
            print(f"❌ FAILED: Global Env {global_env_id} (Slot {slot_idx_item})")
            print(f"   Duration: {duration:.1f}s | Attempts: {attempts}/{self.max_attempts_per_env}")
            print(f"   Reason: Exceeded maximum spawn attempts")
            print(f"{'='*80}\n")
            
            # Mark as failed in queue
            self.env_queue[global_env_id]["status"] = "failed"
            self.env_queue[global_env_id]["end_time"] = time.time()
            self.num_failed_envs += 1
            
            # Assign next env to this slot
            assigned = self._assign_env_to_slot(slot_idx_item)
            
            if not assigned:
                print(f"  ℹ️  Slot {slot_idx_item} retired (no more envs in queue)")


    def _print_progress_dashboard(self) -> None:
        """Print real-time progress statistics and per-slot status."""
        
        elapsed_total = time.time() - self.start_time
        rate = self.num_completed_envs / (elapsed_total / 3600) if elapsed_total > 0 else 0
        eta_hours = (self.total_target_envs - self.num_completed_envs) / rate if rate > 0 else 0
        
        print(f"\n{'='*80}")
        print(f"📊 ROLLING WINDOW STATUS (Elapsed: {elapsed_total/3600:.2f}h)")
        print(f"{'='*80}")
        print(f"  ✅ Completed:   {self.num_completed_envs:4d} / {self.total_target_envs} ({self.num_completed_envs/self.total_target_envs*100:.1f}%)")
        print(f"  ❌ Failed:      {self.num_failed_envs:4d}")
        print(f"  🔄 In Progress: {self.next_env_to_assign - self.num_completed_envs - self.num_failed_envs:4d}")
        print(f"  ⏳ Pending:     {self.total_target_envs - self.next_env_to_assign:4d}")
        print(f"")
        print(f"  ⚡ Rate:        {rate:.1f} envs/hour")
        ...
        for slot_idx in range(self.active_pool_size):
            ...
            if global_env_id >= 0:
                active_count += 1
                config = self.env_queue[global_env_id]
                attempts = self.slot_attempt_counters[slot_idx].item()
                elapsed = time.time() - config["start_time"] if config["start_time"] else 0
                status_icon = "🔨" if not self.slot_spawn_status[slot_idx] else "✓"
                print(f"    [{status_icon}] Slot {slot_idx:2d}: Global {global_env_id:4d} | {config['visibility_category']:12s} | {elapsed:5.0f}s | Attempts: {attempts:2d}")
        
        print(f"\n  Active Slots: {active_count}/{self.active_pool_size}")
        print(f"{'='*80}\n")
    
    
    def _print_final_summary(self) -> None:
        """Print final collection statistics when all envs are complete."""
        
        total_time = time.time() - self.start_time
        avg_rate = self.num_completed_envs / (total_time / 3600) if total_time > 0 else 0
        
        # Calculate per-category stats
        category_stats = {
            "in_view": {"completed": 0, "failed": 0, "total_time": 0.0, "total_attempts": 0},
            "occluded": {"completed": 0, "failed": 0, "total_time": 0.0, "total_attempts": 0},
            "outside_fov": {"completed": 0, "failed": 0, "total_time": 0.0, "total_attempts": 0}
        }
        
        for config in self.env_queue:
            category = config["visibility_category"]
            if config["status"] == "completed":
                category_stats[category]["completed"] += 1
                duration = config["end_time"] - config["start_time"]
                category_stats[category]["total_time"] += duration
                category_stats[category]["total_attempts"] += config["attempts"]
        ...
        print(f"  Completed:       {self.num_completed_envs}/{self.total_target_envs}")
        print(f"  Failed:          {self.num_failed_envs}")
        print(f"  Average Rate:    {avg_rate:.1f} envs/hour")
        print(f"")
        ...
        for category, stats in category_stats.items():
            ...
            avg_attempts = stats["total_attempts"] / stats["completed"] if stats["completed"] > 0 else 0
            
            print(f"    {category:12s}: {stats['completed']:3d} completed, {stats['failed']:3d} failed ({success_rate:.1f}% success)")
            if stats["completed"] > 0:
                print(f"                 Avg time: {avg_time:.1f}s, Avg attempts: {avg_attempts:.1f}")
