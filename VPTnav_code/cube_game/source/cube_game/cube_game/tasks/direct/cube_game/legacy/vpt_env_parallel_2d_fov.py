...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        # For LOS checker
        self.env_bounds = (self.boundary_limits[0][0], self.boundary_limits[1][0])
        self.env_config = {
            "env_bounds": self.env_bounds,
            "fov_deg": 30.0,
            "sample_count": 360,
            "clearance": 0.2,
            "min_valid_spacing": self.min_viewpoint_distance,
        }
    ...
    def _get_object_info_list(self, env_ids: Sequence[int] | None = None) -> List[Dict[str, Any]]:
        """Returns a list of object info dictionaries, one for each env_id."""
        ...
        # --- 1. Get all batched data first ---
        
        # Get scalar/constant config values
        goal_radius = self._goal.cfg.spawn.radius
        agent_size = self._agent.cfg.spawn.size
        
        # Get batched tensor data (Shape: (N, 3))
        goal_pos_batch = self._goal.data.root_pos_w[env_ids]
        agent_pos_batch = self._agent.data.root_pos_w[env_ids]
        camera_obj_pos_batch = self._camera_obj.data.root_pos_w[env_ids]
        
        # --- 2. Pre-process VPT object data ---
        
        # Get all VPT object positions for the selected envs
        # Shape: (N, num_objs, 3)
        all_vpt_pos_batch =  self._vpt_objects.data.object_pos_w[env_ids]

        # Get the static config data (size) for each VPT object just once
        vpt_obj_cfgs = []
        vpt_obj_keys = list(self.cfg.vpt_objects.rigid_objects.keys())
        for obj_idx in range(self.num_objs):
            vpt_spawn_cfg = self.cfg.vpt_objects.rigid_objects[vpt_obj_keys[obj_idx]].spawn
            vpt_obj_cfgs.append({
                "key": f"object_{obj_idx}",
                "size": vpt_spawn_cfg.size
            })

        # --- 3. Build the list with a comprehension ---
        
        # This loop iterates N times (where N = num_envs_to_process)
        object_info_list = [
            {
                "target": {
                    "position": goal_pos_batch[i], # Slices env i
                    "radius": goal_radius
                },
                "agent": {
                    "position": agent_pos_batch[i], # Slices env i
                    "size": agent_size
                },
                "camera_object": {
                    "position": camera_obj_pos_batch[i] # Slices env i
                },
                "vpt_objects": {
                    # Inner dict comprehension builds the dict for this env (i)
                    cfg["key"]: {
                        "position": all_vpt_pos_batch[i][obj_idx], # Slices env i, obj obj_idx
                        "size": cfg["size"]
                    }
                    for obj_idx, cfg in enumerate(vpt_obj_cfgs)
                }
            }
    ...
    def find_valid_view_points(self, object_info, config):
        """
        Calculates valid observation points from a single environment's object data.

        This function is adapted to take a dictionary of object information from a
        batched simulation environment and computes valid viewpoints in 2D by
        projecting the 3D data onto the x-y plane.

        - The observation circle is based on the "target" and "camera_object".
        - Line-of-sight is checked only against "vpt_objects".
        - The "agent" must maintain clearance from "vpt_objects", the "target",
        and the "camera_object".

        Parameters
        ----------
        object_info : dict
            A dictionary containing the state of all relevant objects for a single
            environment. It is expected to have keys like "target", "camera_object",
            "agent" (cube of given size), and "vpt_objects". The "camera_object"
            is assumed to be a 1x1x1 cube.
        config : dict
            A dictionary containing simulation parameters. Must include:
            - "env_bounds": (min_coord, max_coord) for the environment.
            - "fov_deg": The camera's horizontal field of view in degrees.
            - "sample_count": Number of points to sample on the observation circle.
            - "clearance": Base distance to maintain from any obstacle.
            - "min_valid_spacing": Minimum distance between valid observation points.

        Returns
        -------
        tuple of (np.ndarray, int)
            A tuple containing:
            - An array of valid [x, y] observation points. Returns an empty
            array if no valid points are found.
            - The integer count of valid points.
        """
        # --- 1. Extract and Format Data from Input Dictionaries ---
        env_min, env_max = config["env_bounds"]

        # Target position and shape
        red_pos_np = object_info["target"]["position"].cpu().numpy()[:2]
        target_radius = object_info["target"]["radius"]
        target_shape = Point(red_pos_np).buffer(target_radius)

        # Camera Object position and shape (assumed to be a 1x1 cube)
        green_pos_np = object_info["camera_object"]["position"].cpu().numpy()[:2]
        camera_obj_shape = _rect_polygon(green_pos_np, 1.0, 1.0)

        # Agent size, used to calculate clearance from obstacles
        agent_size = object_info["agent"]["size"]
        agent_radius = agent_size[0] / 2.0
        total_clearance = config["clearance"] + agent_radius

        # Obstacles for LOS check are only the vpt_objects
        vpt_obstacle_specs = [
            (
                data["position"].cpu().numpy()[0], data["position"].cpu().numpy()[1],
                data["size"][0], data["size"][1]
            )
            for data in object_info["vpt_objects"].values()
        ]
        vpt_obstacles = [_rect_polygon(s[:2], s[2], s[3]) for s in vpt_obstacle_specs]

        # Obstacles for clearance check include vpt_objects, target, and camera_object
        clearance_obstacles = vpt_obstacles + [target_shape, camera_obj_shape]

        # --- 2. Calculate Observation Circle ---
        d = np.linalg.norm(red_pos_np - green_pos_np)
        if d == 0:
            return np.array([]), 0

        circle_center = (red_pos_np + green_pos_np) / 2
        tan_half_fov = np.tan(np.deg2rad(config["fov_deg"] / 2))

        if tan_half_fov == 0:
            return np.array([]), 0

        circle_radius = 1.3 * (d / (2 * tan_half_fov))
        angles = np.deg2rad(np.linspace(0, 360, config["sample_count"], endpoint=False))
        sample_points = circle_center + circle_radius * \
            np.array([np.cos(angles), np.sin(angles)]).T

        # --- 3. Filter points based on constraints ---
        # In-bounds Filter
        in_bounds_mask = np.all((sample_points >= env_min.cpu().numpy()) & (sample_points <= env_max.cpu().numpy()), axis=1)
        valid_points = sample_points[in_bounds_mask]

        # Clearance Filter (checks against all other objects)
        clearance_mask = [
            all(Point(pt).distance(obs) >= total_clearance for obs in clearance_obstacles)
            for pt in valid_points
        ]
        valid_points = valid_points[clearance_mask]

        # Line-of-Sight (LOS) Filter (checks only against vpt_objects)
        los_mask = [
            not any(LineString([pt, red_pos_np]).intersects(obs) or
                    LineString([pt, green_pos_np]).intersects(obs) for obs in vpt_obstacles)
            for pt in valid_points
        ]
        valid_points = valid_points[los_mask]

        # Spacing Filter
        final_points = _greedy_filter(valid_points, config["min_valid_spacing"])

        return final_points, len(final_points)
    ...
    def _reset_idx_internal(self, env_ids: Sequence[int] | None, randomize_objects: bool = True,
        ...
        for spawn_attempt in range(max_spawn_attempts):
            if not envs_need_spawn_retry.any():
                break
            print(f"  🔄 Spawn Attempt {spawn_attempt + 1}/{max_spawn_attempts} for {envs_need_spawn_retry.sum().item()} environment(s)...")
            ...
            # LOS Checks for all final valid envs
            los_valid_mask = torch.ones(len(final_valid_indices), dtype=torch.bool, device=device)
            data_dict = self._get_object_info_list(env_ids=final_valid_env_ids)
            for local_idx, env_idx in enumerate(final_valid_indices):
                env_bounds = (-self.center_to_boundary, self.center_to_boundary)
                env_bounds_w_origin = (
                    env_bounds[0] + self.scene.env_origins[env_idx][0].cpu().numpy().item(),
                    env_bounds[1] + self.scene.env_origins[env_idx][1].cpu().numpy().item()
                )
                points, num_valid = self.find_valid_view_points(
                    object_info=data_dict[local_idx],
                    config={
                        "env_bounds": env_bounds_w_origin,
                        "fov_deg": 30.0,
                        "sample_count": 360,
                        "clearance": 0.2,
                        "min_valid_spacing": self.min_viewpoint_distance
                ...
                # If not at least self.images_per_env found, mark for retry
                if num_valid < self.images_per_env:
                    los_valid_mask[local_idx] = False
                    envs_need_spawn_retry[env_idx] = True
                    if self.verbose >= 1:
                        print(f"    ❌ Env {final_valid_env_ids[local_idx].item()} (folder {folder_indices[env_idx]}): LOS viewpoint check FAILED ({num_valid}/{self.images_per_env} valid points)")
                elif num_valid >= self.images_per_env:
                    if self.verbose >= 2:
                        print(f"    ✅ Env {final_valid_env_ids[local_idx].item()} (folder {folder_indices[env_idx]}): LOS viewpoint check PASSED ({num_valid}/{self.images_per_env} valid points)")
            
            final_valid_mask[final_valid_indices] = los_valid_mask
            if not final_valid_mask.any():
                continue
    ...
    def generate_valid_circle_points(self, env_ids: torch.Tensor, angle_step: float = 2.0,
        ...
        LOGIC:
        1.  Fast geometric check on a circle (original Step 1).
        2.  Fast displacement filter on circle points (original Step 2).
        3.  If (1) and (2) pass, call `find_valid_view_points` (the LOS checker).
        4.  If (3) passes, USE THE POINTS FROM `find_valid_view_points`
            and add them to the batch for the final Agent FOV check.
        5.  Run Agent FOV check (original Step 3) on the collected points.
        6.  Collect results (original Step 4).
        """
        device = self.device
        num_envs = len(env_ids)
        ...
        # Step 1: Geometric validation (Fast Pre-check)
        geometric_valid = self._is_point_valid_batch(
            points=all_points_batch,
            env_ids=env_ids_batch,
            check_agent_fov=False
        ...
        if self.verbose >= 2:
            for i, env_id in enumerate(env_ids):
                env_id_item = env_id.item()
                print(f"   Env {env_id_item}: {geometric_valid_per_env[i].sum().item()} geometric candidates (pre-check)")
        
        # Pre-fetch object info for all envs for the new LOS check
        try:
            data_dict_all_envs = self._get_object_info_list(env_ids=env_ids)
        except Exception as e:
            print(f"   ❌ Error calling _get_object_info_list: {e}. Returning empty for all.")
            return [torch.zeros((0, 2), device=device) for _ in range(num_envs)]

        # Step 2 & 2.5: Displacement Filter (Pre-check) & LOS Check (Point Generation)
        final_candidate_points = []
        final_candidate_env_ids = []
        final_candidate_indices = []
        
        MIN_CANDIDATES_FOR_FOV = 60  # Min candidates for *displacement filter* to pass
        ...
        for i, env_id in enumerate(env_ids):
            ...
            if not valid_mask.any():
                if self.verbose >= 2:
                    print(f"   Env {env_id_item}: Failed geometric pre-check (0 candidates)")
                continue
            ...
            num_after_displacement = selected_mask.sum().item()
            
            if num_after_displacement < MIN_CANDIDATES_FOR_FOV:
                if self.verbose >= 1:
                    print(f"   Env {env_id_item}: ❌ Only {num_after_displacement} candidates after displacement. Skipping LOS/FOV check.")
                continue
            
            if self.verbose >= 2:
                print(f"   Env {env_id_item}: Passed displacement pre-check ({num_after_displacement} candidates). Running LOS check...")
            
            # --- Step 2.5: NEW LOS Check & Point REPLACEMENT ---
            # Pre-checks passed. Now run the real viewpoint generator.
            
            env_bounds = (-self.center_to_boundary, self.center_to_boundary)
            env_bounds_w_origin = (
                env_bounds[0] + self.scene.env_origins[env_id][0].cpu().numpy().item(),
                env_bounds[1] + self.scene.env_origins[env_id][1].cpu().numpy().item()
            )
            
            try:
                # Call the function to get the *actual* points
                points_from_los_check, num_valid = self.find_valid_view_points(
                    object_info=data_dict_all_envs[i], # Use pre-fetched info
                    config={
                        "env_bounds": env_bounds_w_origin,
                        "fov_deg": 30.0,
                        "sample_count": 360,
                ...
                if num_valid < self.images_per_env:
                    if self.verbose >= 1:
                        print(f"   Env {env_id_item}: ❌ LOS viewpoint check FAILED ({num_valid}/{self.images_per_env} valid points). Skipping FOV check.")
                    continue # Env fails
                else:
                    if self.verbose >= 2:
                        print(f"   Env {env_id_item}: ✅ LOS viewpoint check PASSED ({num_valid}/{self.images_per_env}). Using these {num_valid} points.")
                    
                    # --- THIS IS THE KEY CHANGE ---
                    # We append the points *from the LOS checker*, not the circle.
                    points_tensor = torch.tensor(points_from_los_check, dtype=torch.float32, device=device)
                    final_candidate_points.append(points_tensor)
                    final_candidate_env_ids.extend([env_id.item()] * num_valid)
                    final_candidate_indices.extend([i] * num_valid)
                    # --- END KEY CHANGE ---
            
            except Exception as e:
                print(f"   Env {env_id_item}: ❌ Exception during find_valid_view_points: {e}. Skipping FOV check.")
                continue # Env fails
        
        # --- END OF STEP 2 & 2.5 LOOP ---
        

        if len(final_candidate_points) == 0:
            if self.verbose >= 1:
                print(f"   ❌ No candidates passed pre-checks and LOS filter for any environment")
            return [torch.zeros((0, 2), device=device) for _ in range(num_envs)]
        
        # Concatenate all candidates that passed all checks
        all_candidates = torch.cat(final_candidate_points, dim=0)
        all_candidates_env_ids = torch.tensor(final_candidate_env_ids, dtype=torch.long, device=device)
        all_candidates_indices = torch.tensor(final_candidate_indices, dtype=torch.long, device=device)
        
        if self.verbose >= 2:
            print(f"   💡 Total {len(all_candidates)} points from {len(final_candidate_points)} envs proceeding to final Agent FOV check.")
        ...
        # Step 3: FOV check on the points generated by find_valid_view_points
        ...
        for i, env_id in enumerate(env_ids):
            ...
            if num_candidates == 0:
                ...
                # Verbose message was already printed during the LOS check
                continue
            ...
            if not env_fov_valid.any():
                all_valid_points.append(torch.zeros((0, 2), device=device))
                if self.verbose >= 1:
                    print(f"   Env {env_id_item}: ❌ 0/{num_candidates} passed FINAL Agent FOV check")
                continue
            ...
            if len(valid_points_tensor) >= MIN_REQUIRED_POINTS:
                ...
                if self.verbose >= 2:
                    print(f"   Env {env_id_item}: ✅ {len(valid_points_tensor)}/{num_candidates} passed FINAL Agent FOV ({fov_rejection_rate:.1f}% rejected)")
            else:
                all_valid_points.append(torch.zeros((0, 2), device=device))
                if self.verbose >= 1:
...
def _rect_polygon(center, w, h):
    """Creates a shapely Polygon for a rectangle."""
    cx, cy = center
    return Polygon([
...
def _greedy_filter(points, min_dist):
    """Selects a subset of points that are at least min_dist apart."""
    if len(points) == 0:
        return np.array([])
    selected = [points[0]]
    for p in points[1:]:
