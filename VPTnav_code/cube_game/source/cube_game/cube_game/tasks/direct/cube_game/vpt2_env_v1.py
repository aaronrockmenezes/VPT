...
from .vpt2_env_cfg_v1 import VPTEnvCfg
...
class VPTEnv(DirectRLEnv):
    ...
    def __init__(self,
        ...
        self.verbose = 0
        self.goal_pixel_threshold_agent = 50  # Minimum pixels for goal visibility
        self.goal_pixel_threshold_cam = 150  # Minimum pixels for goal visibility
        # self.camera_pixel_threshold = 500  # Minimum pixels for camera visibility
        self.camera_pixel_threshold_agent = 100  # Minimum pixels for camera visibility
        self.ref_obj_pixel_threshold_agent = 30
        self.ref_obj_pixel_threshold_cam = 150
        ...
        base = os.getenv("BASE_PATH", "/oscar/scratch/arock3/VPT2_DATA/v2")
    ...
    def _setup_scene(self):
        ...
        self._ref_obj = RigidObject(self.cfg.reference_obj)
        ...
        self.scene.rigid_objects["ref_obj"] = self._ref_obj
    ...
    def check_batch_object_visibility(
        ...
        """
        Checks the visibility of specific objects (goal, camera, and reference object) 
        within the camera's field of view for a batch of environments.

        It uses semantic segmentation images to count the number of pixels matching 
        exact color masks:
        - Red (255, 0, 0): Represents the goal object.
        - Green (0, 255, 0): Represents the camera.
        - Pink (255, 0, 255): Represents the reference object.

        It also saves debug images of the segmentation output to the local disk.

        Args:
            env_ids (torch.Tensor): A tensor containing the environment indices to check.

        Returns:
            tuple[torch.Tensor, torch.Tensor, torch.Tensor]: Three boolean tensors indicating 
            whether the goal, camera, and reference objects meet their respective 
            pixel count thresholds for visibility in each environment.
        """
        
        # Self-contained counter initialization
        ...
        for idx in range(sem_imgs.shape[0]):
            img_tensor = sem_imgs[idx]
            
            # Move to CPU, convert to numpy, and ensure uint8 dtype
            img_np = img_tensor.cpu().numpy().astype(np.uint8)
            
            # Convert RGBA (Isaac Lab default) to BGRA for OpenCV saving
            img_bgr = cv2.cvtColor(img_np, cv2.COLOR_RGBA2BGRA)
            
            filename = os.path.join(save_dir, f"img_{self._debug_img_counter}.png")
        ...
        # Exact match for Pink (255, 0, 255, 255) corresponding to diffuse_color=(1.0, 0.0, 1.0)
        ...
        # print(red_counts, green_counts, pink_counts)
    ...
    def _reset_idx_internal(self,
        ...
        ref_obj_state = self._ref_obj.data.default_root_state[env_ids].clone()
    ...
    def write_pose_to_sim(self,
        ...
        if ref_obj_default_state is not None:
            ref_obj_poses = ref_obj_default_state[indices, :7]
            self._ref_obj.write_root_com_pose_to_sim(ref_obj_poses, env_ids)
            self._ref_obj.write_root_com_velocity_to_sim(
                torch.zeros((len(env_ids), 6), device=device), env_ids)
